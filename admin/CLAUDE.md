...
│   ├── api/v1/controller/      # API v1 控制器（URL 版本 /api/v1）
...
版本固化在 URL 路径前缀 `/api/v1`（如 `POST /api/v1/auth/login`、`POST /api/v1/captcha/generate`），URL 不带版本前缀即 404：

```bash
curl http://localhost:8787/api/v1/auth/login
```

新增版本时在 `config/route.php` 注册 `/api/v{n}` 路由组，`v()` 解析函数按组前缀 `{version}` 定位 `app/api/{version}/controller/` 目录下的控制器。
