...
### 仪表盘
登录后首页查看实时统计（用户数/订单总数/技师数/服务订单数）与趋势折线图（订单量/金额/新增用户/活跃度），快速导航直达待处理模块，站内消息接收新订单/退款通知。

### 数据报表
报表页查看 3 类报表（对应 `GET /admin/reports/orders|technicians|distribution`，Redis 缓存 300s）：
- **订单统计** — 订单量/金额按日期范围统计
- **技师业绩** — 按技师汇总服务单量与业绩
- **门店分布** — 按门店汇总订单分布
