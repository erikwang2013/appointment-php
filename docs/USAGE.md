...
### 仪表盘
登录后首页查看 7 张统计卡（用户总数/今日新增/活跃用户/操作日志/今日预约/待审核提现/待审技师，动态渲染）、近 30 天趋势折线图（订单量/金额/新增用户/活跃度）、用户状态分布饼图（启用/禁用）与最近 10 条操作日志（Redis `svc:dashboard` 缓存 300s）；快速导航直达待处理模块，站内消息接收新订单/退款通知。

### 数据报表
报表页查看 3 类报表（7/30 天范围，对应 `GET /admin/reports/orders|technicians|distribution`，Redis 缓存 300s）：
- **订单统计** — 汇总（单量/支付金额/退款/净收入）+ 按天趋势
- **技师业绩** — 技师 TOP10（单量/营收/评分，姓名脱敏，按单量或营收排序）
- **渠道分布** — 支付渠道分布（微信/支付宝/余额）+ 订单状态分布

另可查看**销售统计**（`svc:sales_stats`：区间订单汇总/门店/服务类型维度）与**财务统计**（`svc:finance_stats`：收入/退款/提现/佣金区间汇总）。
...
APP注册设备: `POST /api/v1/user/device/register {"platform":"ios","device_token":"..."}`
