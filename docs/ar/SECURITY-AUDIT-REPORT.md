...
| البند | محتوى الإصلاح |
|----|---------|
| منع تلاعب الطلب | أسعار بنود OrderController::store() تُحسب دائمًا من سجلات قاعدة البيانات (service→erik_service، product→erik_product)، وأسعار العميل لا تدخل في الحساب؛ target_type غير معروف 422؛ target_id يجب أن يكون hashid (raw id يُفك إلى 0 → 422 «المنتج غير موجود أو غير متاح»)؛ أسعار المجموعة/البيع المفاجئ كذلك حسب DB |
