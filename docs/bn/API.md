# API ডকুমেন্টেশন
> **Languages**: [中文](../API.md) · [English](../en/API.md) · [한국어](../ko/API.md) · [Русский](../ru/API.md) · [Deutsch](../de/API.md) · [Français](../fr/API.md) · [Español](../es/API.md) · [Português](../pt/API.md) · [हिन्दी](../hi/API.md) · [العربية](../ar/API.md) · [Bahasa Indonesia](../id/API.md) · [日本語](../ja/API.md)
...
- **ভার্সন কন্ট্রোল**: ভার্সন URL পাথ প্রিফিক্স `/api/v1`-এ স্থির করা হয়েছে (যেমন `POST /api/v1/auth/login`); URL-এ ভার্সন প্রিফিক্স না থাকলে 404
...
**`POST /api/v1/captcha/send`** — SMS ভেরিফিকেশন কোড পাঠানো
...
**`POST /api/v1/auth/register`** — ফোন নম্বর দিয়ে রেজিস্ট্রেশন
...
**`POST /api/v1/auth/login`** — পাসওয়ার্ড লগইন
...
**`POST /api/v1/auth/login-by-code`** — ভেরিফিকেশন কোড লগইন
...
**`POST /api/v1/auth/forget-password`** — পাসওয়ার্ড ভুলে গেলে
...
**`POST /api/v1/wechat/mini-login`** — মিনি-প্রোগ্রাম লগইন

রিকোয়েস্ট: `{"code":"微信登录code"}`
বিবরণ: প্রথম লগইনের পর ফোন নম্বর বাঁধতে `/api/v1/wechat/phone` কল করতে হবে।

---

**`POST /api/v1/wechat/phone`** — ফোন নম্বর বাঁধা

রিকোয়েস্ট: `{"code":"微信手机号组件code"}`

---

**`POST /api/v1/wechat/oa-login`** — অফিসিয়াল অ্যাকাউন্ট লগইন
...
**`GET /api/v1/common/config`** — পাবলিক কনফিগ

রেসপন্স: চুক্তি টেক্সট (ব্যবহারকারী চুক্তি/প্রাইভেসি চুক্তি/সার্ভিস চুক্তি), আমাদের সম্পর্কে তথ্য, ভার্সন নম্বর থাকে।

---

**`GET /api/v1/common/area`** — শহর/এলাকা তালিকা

---

#### 1.5 সার্ভিস কোয়েরি

**`GET /api/v1/service/categories`** — ক্যাটাগরি তালিকা

প্যারামিটার: `?parent_id=0`

---

**`GET /api/v1/service/items`** — সার্ভিস আইটেম তালিকা

প্যারামিটার: `?category_id=&page=1&per_page=10&sort=sales`

---

**`GET /api/v1/service/detail/{id}`** — সার্ভিস ডিটেইল

রেসপন্সে থাকে: ছবি/নাম/দাম/স্পেক/সময়কাল/বিক্রয় সংখ্যা/রিভিউ তালিকা।

---

**`GET /api/v1/service/products`** — পণ্য তালিকা

**`GET /api/v1/service/stores`** — স্টোর তালিকা
...
**`GET /api/v1/technician/list`** — টেকনিশিয়ান তালিকা
...
**`GET /api/v1/technician/detail/{id}`** — টেকনিশিয়ান ডিটেইল

রেসপন্সে থাকে: ছবি/নাম/ইন্ট্রো/স্কোর/দূরত্ব/সার্ভিসযোগ্য আইটেম তালিকা/রিভিউ।

---

**`GET /api/v1/technician/schedule/{id}`** — টেকনিশিয়ান শিডিউল
...
**`GET /api/v1/content/banners`** — ক্যারোসেল

প্যারামিটার: `?position=home`

**`GET /api/v1/content/articles`** — নোটিশ/আর্টিকেল তালিকা

প্যারামিটার: `?type=announcement&page=1`

**`GET /api/v1/content/article/{id}`** — আর্টিকেল ডিটেইল

---

#### 1.8 LBS

**`GET /api/v1/lbs/nearby-stores`** — কাছের স্টোর

প্যারামিটার: `?lat=&lng=&radius=5000`

**`GET /api/v1/lbs/geocode`** — রিভার্স জিওকোডিং
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/user/profile` | ব্যক্তিগত তথ্য পাওয়া |
| PUT | `/api/v1/user/profile` | নিকনেম/অবতার/লিঙ্গ আপডেট |
| POST | `/api/v1/user/change-password` | পাসওয়ার্ড পরিবর্তন (old_password/new_password/confirm_password) |
| POST | `/api/v1/user/change-phone` | ফোন নম্বর পরিবর্তন (old_code/new_phone/new_code) |
| POST | `/api/v1/user/cancel-account` | অ্যাকাউন্ট বাতিল (পাসওয়ার্ড ভেরিফাই করতে হবে) |
| POST | `/api/v1/user/logout` | লগআউট (token ব্ল্যাকলিস্টে) |
| POST | `/api/v1/user/switch-role` | পরিচয় স্যুইচ (role: customer/technician) |
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/user/addresses` | ঠিকানা তালিকা |
| POST | `/api/v1/user/addresses` | নতুন ঠিকানা (contact_name/contact_phone/province/city/district/detail/lat/lng/is_default) |
| GET | `/api/v1/user/addresses/{id}` | ঠিকানা ডিটেইল |
| PUT | `/api/v1/user/addresses/{id}` | ঠিকানা আপডেট |
| DELETE | `/api/v1/user/addresses/{id}` | ঠিকানা ডিলিট |
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/user/favorites` | ফেভারিট তালিকা (?type=service/technician) |
| POST | `/api/v1/user/favorites` | ফেভারিট যোগ (target_type/target_id) |
| DELETE | `/api/v1/user/favorites/{id}` | ফেভারিট বাতিল |

#### 2.4 মতামত ও ফিডব্যাক

`POST /api/v1/user/feedback` — ফিডব্যাক জমা (content + images অ্যারে)

#### 2.5 প্রমোশন ও রেফারেল

| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/user/referral` | প্রমোশন তথ্য (রেফারেল কোড/রেফার করা সংখ্যা/প্রথম অর্ডার সংখ্যা/পাওয়া পয়েন্ট) |
| GET | `/api/v1/user/referral/qrcode` | প্রমোশন QR কোড (রেফারেল কোড + ইনভাইট লিংক) |
| GET | `/api/v1/user/referral/referred-users` | রেফার করা ইউজার তালিকা |
| GET | `/api/v1/user/referral/earnings` | ডিস্ট্রিবিউশন কমিশন বিবরণ (পেজিনেশন: রেফার করা ব্যক্তির নিকনেম/অবতার/অর্ডার নম্বর/পরিমাণ/দেওয়ার সময়) |

**ডিস্ট্রিবিউশন কমিশন**: রেফার করা ব্যক্তির প্রথম অর্ডার completed হওয়ার পর দেওয়া হয়, পরিমাণ = paid_amount × reward_rate (appointment_system_config referral.reward_rate, ডিফল্ট 0.05, অবৈধ মান হলে কনস্ট্যান্টে ফিরে যায়)। রো লক + rewarded_at নাল চেক + প্রথম অর্ডার পুনঃপরীক্ষা — তিন স্তরের আইডেমপোটেন্সি; ওয়ালেটে WalletTxn type=referral_reward হিসেবে জমা হয়।

#### 2.6 পয়েন্ট ট্রান্সফার (রাউন্ড ১৯)

| মেথড | পাথ | বিবরণ |
|------|------|------|
| POST | `/api/v1/user/points/transfer` | পয়েন্ট ট্রান্সফার (to_user_id hashid/points) |
| GET | `/api/v1/user/points/transfers` | ট্রান্সফার রেকর্ড (?direction=sent/received&page=1) |
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/technician/profile` | টেকনিশিয়ান প্রোফাইল পাওয়া |
| PUT | `/api/v1/technician/profile` | প্রোফাইল আপডেট (avatar/intro/real_name/gender/id_card/id_card_front/id_card_back) |
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/technician/schedule` | শিডিউল কোয়েরি (?start_date=&end_date=) |
| PUT | `/api/v1/technician/schedule` | শিডিউল সেট (date/time_slots/status), সময় স্লট ওভারল্যাপ ৪২২「与已有排班时间冲突」 |
| POST | `/api/v1/technician/schedule/batch` | ব্যাচ শিডিউল (রাউন্ড ২৩): তারিখ পরিসর ≤৭ দিন + weekdays ফিল্টার, ইতিমধ্যে শিডিউল করা দিন স্কিপ, রেসপন্সে created/skipped |

#### 3.3 টেকনিশিয়ান অর্ডার

`GET /api/v1/technician/orders` — অর্ডার তালিকা (?status=&page=1)

#### 3.4 আয়

`GET /api/v1/technician/earnings` — আয় ওভারভিউ (today_income/pending_settlement/balance + লেনদেন তালিকা)

#### 3.5 উত্তোলন

`POST /api/v1/technician/withdraw` — উত্তোলন আবেদন (amount)
...
`POST /api/v1/technician/review/reply/{order_id}` — টেকনিশিয়ান রিভিউ রিপ্লাই (reply)। রিভিউ না থাকলে/নিজের না হলে ইউনিফাইড ৪০৪ (অস্তিত্ব লিক হয় না); আগে থেকে রিপ্লাই থাকলে ৪২২ (আইডেমপোটেন্ট রিজেক্ট, ওভাররাইট হয় না); খালি রিপ্লাই ৪২২। রিপ্লাই সফল হলে ইউজারকে স্টেশন-ইন নোটিফিকেশন (type='review_reply')।

#### 3.7 ওয়ার্কবেঞ্চ

| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/technician/work/today` | আজকের টাস্ক তালিকা |
| GET | `/api/v1/technician/work/records` | সম্পন্ন রেকর্ড পেজিনেশন |
| POST | `/api/v1/technician/work/{id}/start` | সার্ভিস শুরু |
| POST | `/api/v1/technician/work/{id}/complete` | সার্ভিস সম্পন্ন |
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| POST | `/api/v1/order` | অর্ডার তৈরি (order_type/items/store_id/technician_id/service_time/coupon_id/user_coupon_id/promotion_id/remark) |
| GET | `/api/v1/order/list` | অর্ডার তালিকা (?status=&page=1) |
| GET | `/api/v1/order/detail/{id}` | অর্ডার ডিটেইল |
| POST | `/api/v1/order/cancel/{id}` | অর্ডার বাতিল (reason) |
| POST | `/api/v1/order/pay/{id}` | পেমেন্ট শুরু (pay_channel: wechat/balance, use_points: ঐচ্ছিক পয়েন্ট ক্যাশব্যাক) |
| POST | `/api/v1/order/refund/{id}` | রিফান্ড আবেদন |
| POST | `/api/v1/order/verify/{id}` | ভেরিফিকেশন (code: QR কোড মান) |
| POST | `/api/v1/order/reschedule/{id}` | অ্যাপয়েন্টমেন্ট পুনঃনির্ধারণ (new_service_time বাধ্যতামূলক/reason ঐচ্ছিক) |
| GET | `/api/v1/order/logistics/{id}` | লজিস্টিকস ট্র্যাকিং (রাউন্ড ১৯, product অর্ডার) |
| POST | `/api/v1/order/review/{order_id}` | রিভিউ জমা (rating 1-5/content/images)（রাউন্ড ১৯补注册） |
| POST | `/api/v1/order/review/{order_id}/append` | রিভিউ ফলো-আপ (content/images কমা-বিভক্ত)（রাউন্ড ১৯） |
...
**দাম টেম্পারিং সুরক্ষা (২০২৬-০৮-২৬)**: অর্ডার আইটেমের পরিমাণ সবসময় ডেটাবেস রেকর্ড অনুযায়ী (target_type=service হলে appointment_service, product হলে appointment_product), ক্লায়েন্ট পাঠানো দাম হিসাবে ধরা হয় না; অজানা target_type ৪২২; target_id অবশ্যই hashid এনকোড করা মান দিতে হবে (raw id দিলে ডিকোড 0 → ৪২২「商品不存在或已下架」); গ্রুপ বাই/সেকিল দামও DB অনুযায়ী।
...
**পয়েন্ট ক্যাশব্যাক**: পেমেন্ট রিকোয়েস্ট বডিতে ঐচ্ছিক `use_points` (পূর্ণসংখ্যা)। SUM অ্যাগ্রিগেশন দিয়ে পয়েন্ট ব্যালেন্স ভেরিফাই করা হয় (appointment_user_points এর balance কলাম এককালীন ইনক্রিমেন্ট স্ন্যাপশট, সরাসরি ব্যালেন্স হিসেবে ধরা যায় না), কাটতি = floor(use_points / config('app.points_rate', 100)) ইউয়ান, প্রকৃত পরিমাণ = মূল পরিশোধযোগ্য − কাটতি (নিম্নসীমা 0.01, পরিশোধযোগ্যের বেশি হলে পরিশোধযোগ্য অনুযায়ী পয়েন্ট নষ্ট হয় না)। সফল হলে type=consume/source=points_offset কনজিউম লেনদেন লেখা হয় (আইডেমপোটেন্ট, রিট্রাইতে ডাবল কাটা হয় না)। ব্যালেন্স কম ৪২২।
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/promotions` | অ্যাক্টিভিটি তালিকা (?type=group_buy；flash_sale ফিল্টার হয়ে রিটার্ন হয় না) |
| GET | `/api/v1/promotions/{id}` | অ্যাক্টিভিটি ডিটেইল (অংশগ্রহণকারী সংখ্যা/গ্রুপ হয়েছে কি না সহ; flash_sale টাইপ ৪০০) |
| GET | `/api/v1/promotions/{id}/participants` | অংশগ্রহণকারী তালিকা |
| POST | `/api/v1/promotions/join/{id}` | অ্যাক্টিভিটিতে অংশগ্রহণ (রাউন্ড ১৫ পরিপূর্ণ: রেসপন্সে discount_percent/original_price/group_price; flash_sale টাইপ ৪০০) |
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/marketing/coupons` | কুপন তালিকা (?status=available/used/expired) |
| POST | `/api/v1/marketing/coupons/receive` | কুপন গ্রহণ (coupon_id) |
| GET | `/api/v1/marketing/cards` | মেম্বার কার্ড তালিকা |
| POST | `/api/v1/marketing/cards/buy` | মেম্বার কার্ড কেনা (card_id) |
| GET | `/api/v1/marketing/cards/my` | আমার টাইম কার্ড তালিকা |
| POST | `/api/v1/marketing/cards/use` | টাইম কার্ড ভেরিফাই (user_card_id/service_id/remark?) |
| GET | `/api/v1/marketing/gift-cards` | গিফট কার্ড তালিকা |
| GET | `/api/v1/marketing/gift-cards/my` | আমার গিফট কার্ড (redeem রেকর্ড) |
| POST | `/api/v1/marketing/gift-cards/redeem` | গিফট কার্ড রিডিম (cash টাইপ রিডিমের পর ওয়ালেট ব্যালেন্স রিচার্জ) |
| GET | `/api/v1/marketing/points` | পয়েন্ট লেনদেন (?type=earn/use/expire&source=order/referral/gift_card/check_in/admin) |
| GET | `/api/v1/marketing/points-exchange` | পয়েন্ট এক্সচেঞ্জ পণ্য তালিকা (আপ + রিয়েল-টাইম অবশিষ্ট স্টক + এক্সচেঞ্জ সংখ্যা) |
| POST | `/api/v1/marketing/points-exchange/{id}` | এক্সচেঞ্জ (type=coupon কুপন দেওয়া / wallet জমা / gift_card কার্ড কোড রিটার্ন) |
| POST | `/api/v1/marketing/coupons/transfer` | ট্রান্সফার কোড জেনারেট (user_coupon_id: ৮-অক্ষরের ইউনিক কোড/৭ দিন বৈধ) |
| POST | `/api/v1/marketing/coupons/claim` | ট্রান্সফারড কুপন গ্রহণ (code) |
| GET | `/api/v1/marketing/coupons/transfers` | ট্রান্সফার রেকর্ড (পাঠানো pending/claimed/expired + গ্রহণ claimed) |
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/notification` | নোটিফিকেশন তালিকা (?type=order/system&page=1) |
| PUT | `/api/v1/notification/read/{id}` | পড়া হয়েছে চিহ্নিত |
| PUT | `/api/v1/notification/read-all` | সব পড়া হয়েছে |
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/wallet` | ওয়ালেট ব্যালেন্স + লেনদেন পেজিনেশন |
| POST | `/api/v1/wallet/recharge` | রিচার্জ অর্ডার তৈরি (amount: ইউয়ান) |
| POST | `/api/v1/wallet/recharge/{id}/pay` | রিচার্জ অর্ডার পেমেন্ট (উইচ্যাট) |
| POST | `/api/v1/wallet/transfer` | ব্যালেন্স ট্রান্সফার (to_user_id hashid/amount/remark ঐচ্ছিক/client_token ঐচ্ছিক)（রাউন্ড ১৯） |
| GET | `/api/v1/wallet/transfers` | ট্রান্সফার রেকর্ড (?direction=out/in&page=1)（রাউন্ড ১৯） |
| GET | `/api/v1/wallet/transfers/{id}` | ট্রান্সফার ডিটেইল (শুধু দুই পক্ষ দেখতে পারে, অন্যের ৪০৪)（রাউন্ড ১৯） |
...
**রিচার্জ**: `POST /api/v1/wallet/recharge` এ amount (ইউয়ান) দিয়ে রিচার্জ অর্ডার তৈরি হয়, রিচার্জ অর্ডার hashid রিটার্ন। `POST /api/v1/wallet/recharge/{id}/pay` উইচ্যাট পেমেন্ট শুরু করে, রেসপন্সে sign_params থাকে (অর্ডার পেমেন্ট মোডের মতো); পেমেন্ট কলব্যাকে R প্রিফিক্সের out_trade_no দিয়ে রিচার্জ অর্ডার ও অর্ডার আলাদা করা হয়।
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/store-manager/overview` | আজকের ওভারভিউ (আজকের অর্ডার সংখ্যা/আজকের আয়/চলমান/টেকনিশিয়ান সংখ্যা/ভেরিফাই সংখ্যা) |
| GET | `/api/v1/store-manager/orders` | স্টোর অর্ডার তালিকা (?status=&page=&limit=) |
| GET | `/api/v1/store-manager/technicians` | টেকনিশিয়ান তালিকা (আজকের শিডিউল সহ) |
| GET | `/api/v1/store-manager/revenue` | সাম্প্রতিক ৭ দিনের আয় অ্যাগ্রিগেশন |

**store_id আইসোলেশন**: requireStoreId() বর্তমান ইউজারকে স্টোর বাঁধতে বাধ্য করে (appointment_user.store_id), স্টোর না থাকলে ৪০৩; সব কোয়েরি store_id দিয়ে ফিল্টার।
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/growth` | বর্তমান গ্রোথ ওভারভিউ (balance/লেভেল/পরের ধাপের পার্থক্য/লেভেল নাম) |
| GET | `/api/v1/growth/records` | গ্রোথ ভ্যালু লেনদেন পেজিনেশন (?page=&limit=) |
| GET | `/api/v1/growth/levels` | ধাপ তালিকা (পাবলিক, লগইন লাগে না) |
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| POST | `/api/v1/invoices` | ইনভয়েস আবেদন (order_id hashid/order_type: service=服务/points_exchange=积分兑换/order_type ডিফল্ট service; পরিমাণ ও টাইটেল সার্ভার সাইড থেকে আনা হয়, টেম্পার করা যায় না) |
| GET | `/api/v1/invoices` | ইনভয়েস তালিকা (?status=&page=) |
| GET | `/api/v1/invoices/{id}` | ইনভয়েস ডিটেইল (শুধু নিজের) |
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| POST | `/api/v1/tickets` | টিকিট জমা (title/content বাধ্যতামূলক) |
| GET | `/api/v1/tickets` | টিকিট তালিকা (?status=open/closed&page=) |
| GET | `/api/v1/tickets/{id}` | টিকিট ডিটেইল (শুধু নিজের, অন্যের ৪০৪) |
| POST | `/api/v1/tickets/{id}/close` | টিকিট বন্ধ (শুধু নিজের/শুধু open; ঐচ্ছিক rating ১-৫ সন্তুষ্টি স্কোর, সীমার বাইরে/অ-পূর্ণসংখ্যা ৪২২, না দিলে NULL) |

### 12. অ্যাপয়েন্টমেন্ট ক্যালেন্ডার ইন্টারফেস (JWT অথেনটিকেশন প্রয়োজন, রাউন্ড ২০)

| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/calendar/technician/{id}` | মাস ভিউ (?month=YYYY-MM): শিডিউল time_slots ঘণ্টা স্লটে বিস্তার + বুক করা বাদ |
| GET | `/api/v1/calendar/technician/{id}/day` | দিন ভিউ (?date=YYYY-MM-DD): সেদিনের বুকযোগ্য/বুক করা/বুক করা যাবে না স্লট বিবরণ |

### 13. ইনভয়েস টাইটেল ইন্টারফেস (JWT অথেনটিকেশন প্রয়োজন, রাউন্ড ২১)

| মেথড | পাথ | বিবরণ |
|------|------|------|
| POST | `/api/v1/invoice-titles` | টাইটেল সেভ (title_type: personal/company; company হলে tax_no বাধ্যতামূলক; একই ইউজারের একই টাইটেল ডুপ্লিকেট ৪২২; প্রথমটি অটো ডিফল্ট) |
| GET | `/api/v1/invoice-titles` | টাইটেল তালিকা (ডিফল্ট প্রথমে) |
| PUT | `/api/v1/invoice-titles/{id}` | টাইটেল এডিট (শুধু নিজের) |
| DELETE | `/api/v1/invoice-titles/{id}` | টাইটেল ডিলিট (শুধু নিজের; ডিফল্ট ডিলিট করলে অটো সবচেয়ে পুরনোটি ডিফল্ট) |
| POST | `/api/v1/invoice-titles/{id}/default` | ডিফল্ট সেট (ট্রানজেকশনে একই ইউজারের অন্য সারি শূন্য) |

**আবেদন লিংকেজ**: POST /api/v1/invoices ঐচ্ছিক title_id সমর্থন করে — টাইটেল পার্স করে অটো invoice_title/tax_no/title_type পূরণ, title_id না দিলে আগের ম্যানুয়াল পথ।

### 14. ব্রাউজ হিস্ট্রি ইন্টারফেস (JWT অথেনটিকেশন প্রয়োজন, রাউন্ড ২১)

| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/browse-history` | সাম্প্রতিক দেখা সার্ভিস (সার্ভিস নাম/কভার/দাম/মূল দাম join, viewed_at উল্টো ক্রমে, per_page ডিফল্ট ১৫ সর্বোচ্চ ৫০) |
| DELETE | `/api/v1/browse-history/{item_id}` | একটি ডিলিট (শুধু নিজের, অবৈধ/অন্যের ৪০৪) |
| DELETE | `/api/v1/browse-history` | হিস্ট্রি ক্লিয়ার (শুধু নিজের) |
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/full-reduction-activities` | চলমান ফুল-রিডাকশন অ্যাক্টিভিটি তালিকা (status=1 এবং সময় বৈধ পরিসরে, কাটতি পরিমাণ উল্টো ক্রমে; পাবলিক ইন্টারফেস) |
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/order/ics` | ৯০ দিনের মধ্যে বৈধ অর্ডার (pending/paid/confirmed/serving) iCal (RFC5545) হিসেবে এক্সপোর্ট |
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| POST | `/api/v1/technician/attendance/check-in` | কাজে ঢোকার চেক-ইন (সেদিন ডুপ্লিকেট ৪২২, ইউনিক ইনডেক্স কনকারেন্সি ব্যাকআপ; >১০:০০ হলে লেট চিহ্নিত) |
| POST | `/api/v1/technician/attendance/check-out` | কাজ শেষের চেক-আউট (চেক-ইন না/চেক-আউট হয়েছে ৪২২, রো লক কনকারেন্সি) |
| GET | `/api/v1/technician/attendance` | চলতি মাসের অ্যাটেন্ডেন্স তালিকা + উপস্থিত দিন/মোট ঘণ্টা/গড় ঘণ্টা সামারি (?month=YYYY-MM, অবৈধ ৪২২) |

### 18. প্রাইভেসি কমপ্লায়েন্স ইন্টারফেস (JWT অথেনটিকেশন প্রয়োজন, রাউন্ড ২২)

| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/privacy/data` | ডেটা এক্সপোর্ট (personal/orders/points/wallet_txns/reviews/addresses/invoices গ্রুপ JSON; সার্ভার লগে শুধু মাস্কড ফোন নম্বর + সংখ্যা) |
| POST | `/api/v1/privacy/close-request` | অ্যাকাউন্ট বন্ধের আবেদন (ব্যালেন্স ০ নয় / অসম্পূর্ণ অর্ডার / চলমান টিকিট ৪২২; close_status=1 + close_requested_at সেট) |
| POST | `/api/v1/privacy/close-cancel` | বন্ধের আবেদন বাতিল (close_status ১→০) |
| POST | `/api/v1/privacy/close-confirm` | বন্ধ কনফার্ম (৭২ ঘণ্টা পূর্ণ হলে; close_status=2 + close_at + phone/nickname user{id} তে অ্যানোনিমাইজ + status=0) |
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/health-profile` | আমার হেলথ প্রোফাইল কোয়েরি (প্রোফাইল না থাকলে খালি অবজেক্ট) |
| PUT | `/api/v1/health-profile` | তৈরি/আপডেট (upsert, একজনের একটি; allergies/health_notes সর্বোচ্চ ৫০০ অক্ষর, preferred_technician_id অস্তিত্ব ভেরিফাই; শুধু দেওয়া ফিল্ড আপডেট, রেসপন্স hashid এনকোড) |
| DELETE | `/api/v1/health-profile` | আমার প্রোফাইল ডিলিট (শুধু নিজের) |
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| POST | `/api/v1/wallet/pay-password/set` | পেমেন্ট পাসওয়ার্ড সেট (৬ অঙ্কের `\d{6}`; সেট করা থাকলে পুরনো পাসওয়ার্ড চাওয়া হয় ৪২২) |
| POST | `/api/v1/wallet/pay-password/verify` | পেমেন্ট পাসওয়ার্ড ভেরিফাই (সঠিক/ভুল বুলিয়ান রিটার্ন, সংরক্ষণ হয় না) |
| POST | `/api/v1/wallet/pay-password/check` | সেট করা আছে কি না কোয়েরি (set: true/false) |
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/order/{id}/timeline` | অর্ডার স্ট্যাটাস পরিবর্তনের টাইমলাইন (উল্টো ক্রমে; শুধু নিজের, অন্যের অর্ডার ৪০৪ অস্তিত্ব লিক হয় না) |
...
| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/wheel/prizes` | হুইল প্রাইজ তালিকা (weight/stock সেনসিটিভ ফিল্ড লুকানো) |
| POST | `/api/v1/wheel/spin` | একবার ড্র (Redis NX + রো লক কনকারেন্সি রোধ; random_int ওয়েট ড্র; পয়েন্ট→earn লেনদেন এক্সপায়ারি টাইম সহ, ব্যালেন্স→lockForUpdate জমা, কুপন→pending ম্যানুয়াল বিতরণ, কিছু না→lose; client_token আইডেমপোটেন্সি) |
| GET | `/api/v1/wheel/records` | আমার ড্র রেকর্ড (পেজিনেশন) |

### 23. গেস্ট মোড ইন্টারফেস (রাউন্ড ২৪)

| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/guest/home` | হোমপেজ অ্যাগ্রিগেশন (ক্যারোসেল/নোটিশ/সার্ভিস ক্যাটাগরি/হট সার্ভিস, Redis ক্যাশ svc:guest:home ৩০০s) |
| GET | `/api/v1/guest/services` | সার্ভিস তালিকা (?category_id=hashid&sort=newest\|sales\|price&page/per_page≤50) |
| GET | `/api/v1/guest/services/{id}` | সার্ভিস ডিটেইল (না থাকলে ৪০৪) |
| GET | `/api/v1/guest/stores` | স্টোর তালিকা |
| GET | `/api/v1/guest/technicians` | টেকনিশিয়ান তালিকা (শুধু অনুমোদিত; ?service_id=hashid ফিল্টার; স্কোর উল্টো ক্রমে) |

অথেনটিকেশন ছাড়াই (পাবলিক ইন্টারফেস) লগইন-মুক্ত ব্রাউজিং এন্ট্রি।

### 24. সেকিল ইন্টারফেস (JWT অথেনটিকেশন প্রয়োজন, রাউন্ড ২৪)

| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/seckill` | সেকিল অ্যাক্টিভিটি তালিকা (status=1 এবং টাইম উইন্ডোর মধ্যে; বিক্রি হওয়া সংখ্যা = appointment_order.seckill_id অর্ডার সংখ্যা, অবশিষ্ট স্টক সহ) |
| GET | `/api/v1/seckill/{id}` | অ্যাক্টিভিটি ডিটেইল (state=not_started/ongoing/ended) |
| POST | `/api/v1/seckill/{id}/buy` | সেকিল অর্ডার (client_token আইডেমপোটেন্সি + Redis NX ৩০s কনকারেন্সি রোধ + অ্যাক্টিভিটি ভ্যালিডেশন; স্টক আগে থেকে কাটা হয় না) |

**অর্ডার নিয়ম (২০২৬-০৮-২৬ থেকে)**: স্টক ইউনিফাইডভাবে `/api/v1/order store()` ট্রানজেকশনে রো লক দিয়ে কাটা হয়, buy শুধু এন্ট্রি ভ্যালিডেশন/আইডেমপোটেন্সি; সেকিল দাম = seckill_price (DB অনুযায়ী), কুপন/পয়েন্ট/মেম্বার কার্ড স্ট্যাক হয় না; অর্ডার বাতিলে স্টক ফেরত নয়; সরাসরি `/api/v1/order` এ seckill_id দিলেও স্টক কাটে।

### 25. APP ভার্সন চেক ইন্টারফেস (রাউন্ড ২৪)

| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/api/v1/app/version?platform=android\|ios` | সর্বশেষ ভার্সন চেক (platform অবৈধ ৪২২; ভার্সন না থাকলে খালি অবজেক্ট; পাবলিক ইন্টারফেস) |

রেসপন্স: id/platform/version_code/version_name/force_update（1=强制）/changelog/download_url।

### 26. নন-ভার্সনড ইন্টারফেস (/api/v1 প্রিফিক্স ছাড়া)

| মেথড | পাথ | বিবরণ |
|------|------|------|
| GET | `/` | ব্র্যান্ড ল্যান্ডিং পেজ (HTML; ব্রাউজার সরাসরি API পোর্টে গেলে দেখানো হয়, API ডকুমেন্ট ও হেলথ চেকের লিংক সহ) |
| GET | `/health` | হেলথ চেক (JSON; docker-compose healthcheck / লোড ব্যালেন্সার লাইভনেস প্রোব ব্যবহার করে) |
| GET | `/api/docs` | বিজনেস API সম্পূর্ণ স্পেসিফিকেশন (OpenAPI 3.0 JSON) |
| POST | `/payment/wechat-notify` | উইচ্যাট পেমেন্ট কলব্যাক (স্বাক্ষর যাচাই + আইডেমপোটেন্সি) |

> ক্লায়েন্ট বিজনেস ইন্টারফেস সবসময় ভার্সন প্রিফিক্স `/api/v1/...` সহ আসে; প্রিফিক্স ছাড়া খালি `/api/*` পাথ ৪০৪ রিটার্ন করে।

---

## 二、ম্যানেজমেন্ট ব্যাকএন্ড API (admin/ :8787)

রিকোয়েস্ট হেডার: `Authorization: Bearer <admin_token>`; পাবলিক অথেনটিকেশন ইন্টারফেসের ভার্সন URL পাথ প্রিফিক্স `/api/v1` অনুযায়ী
...
**অটো অ্যাসেসমেন্ট**: TierRatingService::evaluate রিয়েল-টাইম হিসাব (appointment_order completed অর্ডার সংখ্যা + রিভিউ গড় স্কোর, দশমিক ১ ঘর রাউন্ড) profile.order_count/rating এ ফিরে লেখে, appointment_technician_tier_config (min_orders/min_rating) অনুযায়ী উঁচু থেকে নিচু ম্যাচ, ম্যাচ না হলে সর্বনিম্ন লেভেল। শুধু আপগ্রেড, ডাউনগ্রেড নয় (ডাউনগ্রেড কমিশন রেট ও প্রাইস কোএফিসিয়েন্টে প্রভাব ফেলে, ব্যাকএন্ড ম্যানুয়াল ব্যাকআপ; allowDowngrade=true ম্যানুয়াল রি-অ্যাসেসমেন্টের জন্য); আইডেমপোটেন্ট (লেভেল এক হলে শুধু পরিসংখ্যান সিঙ্ক); পরিবর্তন appointment_technician_tier_log + স্টেশন-ইন নোটিফিকেশন। ট্রিগার পয়েন্ট: WorkController::complete / ReviewController রিভিউ লেখা / ProfileController প্রোফাইল দেখার লেজি চেক।
...
পারমিশন ID: 394। সার্ভার সাইড লজিক: appointment_system_config group=profit_sharing (enabled/receiver_ratio); নিষ্ক্রিয় হলে disabled ডিগ্রেড শুধু লগ; সক্ষম হলে পেমেন্ট সফলের পর অটো প্রফিট শেয়ারিং রিকোয়েস্ট (পরিমাণ=প্রকৃত পরিশোধ×receiver_ratio ডিফল্ট 0.7, একই অর্ডারে pending/success আইডেমপোটেন্ট স্কিপ); ক্রেডেনশিয়াল না থাকলে HTTP এক্সিকিউট হয় না, রিকোয়েস্ট স্ট্রাকচার লগ করা হয়।
...
পারমিশন ID: 412-414। রিওয়ার্ড নিয়ম: ইউজার একই টেকনিশিয়ানে ৩০ দিনের মধ্যে ২য় বার খরচ (অর্ডার সম্পন্ন) হলে বোনাস = প্রকৃত পরিশোধ × ratio (ডিফল্ট 0.05), appointment_technician_earnings-এ (type=return_customer, status=pending) কমিশন সেটেলমেন্ট চেইনের সাথে ইউনিফাইড সেটেল; একই অর্ডার আইডেমপোটেন্ট ডুপ্লিকেট বিতরণ হয় না।
...
পারমিশন ID: 407-411、420। বিক্রি হওয়া সংখ্যা = appointment_order.seckill_id অর্ডার সংখ্যা; স্টক রো লক কাটা, সোল্ড আউট ব্লক।
...
পারমিশন ID: 416-419। আপডেট চেক ইন্টারফেস /api/v1/app/version status=1-এর মধ্যে সর্বশেষ (updated_at/id সর্বোচ্চ) ভার্সন নেয়।
