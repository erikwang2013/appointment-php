...
| ডেটাবেস | MySQL 8.0 | টেবিল প্রিফিক্স `appointment_` |
...
| অ্যাডমিন ব্যাকএন্ড UI সম্পূর্ণ | Flutter Web ২১ পেজ: dashboard/ব্যবহারকারী/রোল/কনফিগ/লগ/ভেরিফিকেশন/শিডিউল/সার্ভিস/টেকনিশিয়ান/অর্ডার/কুপন/মেম্বার/টাইম কার্ড/বিজ্ঞপ্তি/FAQ/উত্তোলন/রিভিউ/রিপোর্ট/আফটার-সেলস/স্টোর ম্যানেজার/পার্সোনাল সেন্টার |
| ড্যাশবোর্ড রিয়েল-টাইম স্ট্যাটস | অ্যাডমিন হোম ৭টি স্ট্যাট কার্ড ডায়নামিক রেন্ডার (মোট ইউজার/আজ নতুন/সক্রিয় ইউজার/অপারেশন লগ/আজকের বুকিং/পেন্ডিং উত্তোলন/পেন্ডিং টেকনিশিয়ান) + ৩০-দিনের ট্রেন্ড চার্ট (অর্ডার ভলিউম/অ্যামাউন্ট/নতুন ইউজার/অ্যাক্টিভিটি) + ইউজার স্ট্যাটাস ডিস্ট্রিবিউশন পাই + সাম্প্রতিক অপারেশন লগ, Redis svc:dashboard ক্যাশ 300s |
| ডেটা রিপোর্ট | ReportController ৩ এন্ডপয়েন্ট: অর্ডার স্ট্যাটস / টেকনিশিয়ান TOP10 / চ্যানেল ডিস্ট্রিবিউশন (GET /admin/reports/orders\|technicians\|distribution, 7/30-দিন রেঞ্জ, Redis ক্যাশ 300s) + সেলস স্ট্যাটস (svc:sales_stats) + ফাইন্যান্স স্ট্যাটস (svc:finance_stats রেভিনিউ/রিফান্ড/উত্তোলন/কমিশন) |
...
| মেম্বার ম্যানেজমেন্ট | appointment_user.member_level কলাম (মাইগ্রেশন 000008); অ্যাডমিন পাশে মেম্বার কার্ড সম্পূর্ণ CRUD (পারমিশন ৩৬৫-৩৬৯) |
...
| স্টোর ম্যানেজার ওয়ার্কবেঞ্চ | service /api/v1/store-manager ৪টি API (overview/orders/technicians/revenue) store_id বাধ্যতামূলক আইসোলেশন (শাখা নেই ৪০৩); admin শাখা ওয়ার্কবেঞ্চ ওভারভিউ + অর্ডার store_id ফিল্টার + Flutter পেজ + পারমিশন ৩৭২ |
...
| অ্যাপয়েন্টমেন্ট রিশিডিউল | POST /api/v1/order/reschedule/{id} একই টেকনিশিয়ানে সময় পরিবর্তন; শুধু pending/paid/confirmed এবং মূল সার্ভিস শুরু থেকে ≥৬ ঘণ্টা আগে; order_lock + নতুন স্লট টেকনিশিয়ান লক SETNX(180s) কনকারেন্সি ওভারসেল প্রতিরোধ + B2 শিডিউল সংঘর্ষ যাচাই; appointment_order_reschedule + SCENE_RESCHEDULE সাবস্ক্রিপশন মেসেজ |
...
| টেকনিশিয়ান লেভেল অটো অ্যাসেসমেন্ট | TierRatingService রিয়েল-টাইম অর্ডার কাউন্ট+গড় স্কোর profile-এ ফেরত লেখা, tier_config অনুযায়ী উঁচু থেকে নিচু ম্যাচ; শুধু আপগ্রেড (allowDowngrade ম্যানুয়াল পুনঃমূল্যায়নের জন্য); পরিবর্তন appointment_technician_tier_log + সাইট-ইন নোটিফিকেশন; admin লগ দেখা (পারমিশন ৩৮০) |
| সেকিল অর্ডার বন্ধনী | /api/v1/seckill অ্যাক্টিভিটি + buy আইডেম্পোটেন্ট/কনকারেন্সি প্রতিরোধ, অর্ডারে seckill_id ইনজেক্ট করে store() পুনরায় ব্যবহার, স্টক ট্রানজেকশনের ভেতরে সারি লকে কাটা (সেকিল প্রাইস = seckill_price DB-ই প্রামাণ্য), সোল্ড আউট ৪২২ "সব শেষ", বাতিলে স্টক ফেরত নেই; পুরনো promotion flash_sale চ্যানেল বন্ধ |
...
| টেকনিশিয়ান রিভিউ রিপ্লাই | POST /api/v1/technician/review/reply/{order_id}: অ-নিজস্ব ৪০৪, পুনরাবৃত্তি রিপ্লাই ৪২২, রিপ্লাই সফলে ব্যবহারকারীকে সাইট-ইন নোটিফিকেশন; appointment_order_review-এ replied_at; admin রিপ্লাই ডিটেইল (পারমিশন ৩৮১) |
...
| ব্যালেন্স ট্রান্সফার | POST /api/v1/wallet/transfer ব্যবহারকারী-মধ্যবর্তী ট্রান্সফার: পরিমাণ 0.01-1000/ট্রানজেকশন + দৈনিক ৫০০০ সীমা; Redis NX লক + দুই পক্ষের ওয়ালেট সারি লক (user_id অ্যাসেন্ডিং ডেডলক প্রতিরোধ) + client_token ২৪ ঘণ্টা আইডেম্পোটেন্ট; WalletTxn transfer_out/transfer_in দ্বৈত লেনদেন balance_after স্ন্যাপশট সহ; প্রাপকের সাইট-ইন নোটিফিকেশন type='balance_received' |
| পয়েন্ট ট্রান্সফার | POST /api/v1/user/points/transfer ব্যবহারকারী-মধ্যবর্তী ট্রান্সফার: ১-১০০০০ পয়েন্ট + দৈনিক মোট ১০০০০ সীমা; Redis NX লক + দুই পক্ষের শেষ লেনদেন lockForUpdate (অ্যাসেন্ডিং ডেডলক প্রতিরোধ) + লকের ভেতরে পুনঃযাচাই; প্রেরকের consume/প্রাপকের earn দ্বৈত লেনদেন (প্রাপকে expires_at সহ স্বাভাবিক এক্সপায়ার); প্রাপকের সাইট-ইন নোটিফিকেশন type='points_received' |
| রিভিউ অ্যাপেন্ড | POST /api/v1/order/review/{order_id}/append: অ-নিজস্ব ৪০৪/পুনরাবৃত্তি ৪২২/খালি কনটেন্ট ৪২২/অ-কমপ্লিটেড ৪২২, সফলে টেকনিশিয়ানকে সাইট-ইন নোটিফিকেশন type='review_append'; appointment_order_review-এ append_content/append_images(JSON)/append_at; সাথে রেজিস্টার্ড ব্যবহারকারীর রিভিউ রুট (আগে store রুট ছিল না) এবং এর লুকানো TypeError ফিক্স |
| ব্যবহারকারী লজিস্টিক ট্র্যাকিং | GET /api/v1/order/logistics/{id}: শুধু নিজস্ব product অর্ডার (৪০৪ অ-নিজস্ব/অ-পণ্য/অ-শিপড); order.remark JSON পড়ে (shipping_company/tracking_no/shipped_at, admin শিপমেন্টে লেখে); প্রাপকের ফোন মাস্কড 138****5678 |
| মেসেজ পছন্দ সেটিং | appointment_user_notify_setting টেবিল (uk_user_type ইউনিক কী, ডিফল্ট সারি = সব চালু); GET/PUT /api/v1/user/notify-settings; ৫ ধরনের সুইচ service_reminder/card_expiry/points_expiry/marketing/system (system চিরকাল চালু বন্ধ করা যায় না); notifySettingEnabled গেটে ৩টি টাইমার + সাবস্ক্রিপশন ইভেন্ট, বন্ধ থাকলে সাইট-ইন নোটিফিকেশন ও সাবস্ক্রিপশন মেসেজ দুটোই বাদ |
| অ্যাপয়েন্টমেন্ট ক্যালেন্ডার | GET /api/v1/calendar/technician/{id} (মাস ভিউ) + /day (দিন ভিউ): time_slots JSON ঘণ্টা স্লটে বিস্তৃত, appointment_order-এ আগে থেকে বুক করা স্লট বাদ; শাখা শিডিউল ভিজুয়ালাইজড সময় নির্বাচন |
| ব্যবহারকারী গ্রোথ লেভেল | appointment_user_growth + appointment_growth_level (ব্রোঞ্জ ০/সিলভার ১০০/গোল্ড ৫০০/প্লাটিনাম ২০০০/ডায়মন্ড ৫০০০); চেক-ইন +১০, রিভিউ +২০, খরচ প্রতি ১ ইউয়ানে ১ পয়েন্ট (বিদ্যমান স্টেট পুনঃযাচাই প্রাকৃতিক আইডেম্পোটেন্ট); GET /api/v1/growth (ওভারভিউ/records/levels পাবলিক লেভেল) |
| ইলেকট্রনিক ইনভয়েস | POST/GET /api/v1/invoices (আবেদন/লিস্ট/ডিটেইল): uk_order_type(order_id,order_type) পুনরাবৃত্তি আবেদন প্রতিরোধ, পরিমাণ সার্ভার-সাইড; admin ইনভয়েস/প্রত্যাখ্যান (পারমিশন ৩৮২-৩৮৪) |
| কাস্টমার টিকেট | POST/GET /api/v1/tickets + /{id}/close: ব্যবহারকারী জমা/লিস্ট/ডিটেইল/ক্লোজ; admin রিপ্লাই (পারমিশন ৩৮৫/৩৮৭) |
...
| ইনভয়েস হেডার ম্যানেজমেন্ট | appointment_invoice_title সাধারণ হেডার লাইব্রেরি: সেভ/এডিট/ডিলিট/ডিফল্ট (প্রথমটি স্বয়ংক্রিয় ডিফল্ট, ডিফল্ট ডিলিটে স্বয়ংক্রিয় স্থানান্তর, ডিফল্ট সেট ট্রানজেকশন শূন্য); ইনভয়েস আবেদনে title_id অপশনাল, ম্যানুয়াল ফিল কম্প্যাটিবল |
...
| ব্রাউজ হিস্ট্রি | appointment_browse_history (uk_user_item পুনরাবৃত্তি ব্রাউজে শুধু viewed_at আপডেট): সার্ভিস ডিটেইলে হুক (try/catch মূল ফ্লো ব্লক করে না, লগইন না করলে স্কিপ); লিস্ট join সার্ভিস তথ্য + hashid; একক ডিলিট/ক্লিয়ার শুধু নিজস্ব |
...
>
> Round-16 সংযোজন: পয়েন্ট রিডিম (PointsExchangeController, টাইপ consume/source=exchange); গ্রুপ বাই অর্ডার (appointment_order-এ promotion_id/participant_id কলাম); ডিস্ট্রিবিউশন রিবেট (ReferralRewardService হুক WorkController::complete)।
>
> Round-17 সংযোজন: অ্যাপয়েন্টমেন্ট রিশিডিউল (appointment_order_reschedule + reschedule API); কুপন ট্রান্সফার (appointment_user_coupon_transfer + transfer/claim/transfers); পয়েন্ট এক্সপায়ার (expires_at + PointsExpiryTimer প্রসেস); টেকনিশিয়ান লেভেল অটো অ্যাসেসমেন্ট (TierRatingService + appointment_technician_tier_log, পারমিশন ৩৮০)。
>
...
>
> Round-19 সংযোজন: ব্যালেন্স ট্রান্সফার (appointment_wallet_transfer + WalletTransferController, পারমিশনে দ্বৈত সারি লক + client_token আইডেম্পোটেন্সি); পয়েন্ট ট্রান্সফার (appointment_user_points_transfer + PointsTransferController, দৈনিক সীমা + দ্বিমুখী লেনদেন); রিভিউ অ্যাপেন্ড (appointment_order_review append তিন কলাম + append API + store রুট রেজিস্ট্রেশন); ব্যবহারকারী লজিস্টিক ট্র্যাকিং (logistics API + remark JSON পার্স + ফোন মাস্কিং); মেসেজ পছন্দ সেটিং (appointment_user_notify_setting + NotifySettingController + ৩ টাইমার গেট)।
>
> Round-20 সংযোজন: অ্যাপয়েন্টমেন্ট ক্যালেন্ডার (CalendarController মাস/দিন ভিউ + বুকড বাদ); ব্যবহারকারী গ্রোথ লেভেল (appointment_user_growth + appointment_growth_level ৫ লেভেল + চেক-ইন/রিভিউ/খরচ হুক); ইলেকট্রনিক ইনভয়েস (appointment_invoice + uk_order_type ডুপ প্রতিরোধ + ব্যাকএন্ড ইনভয়েস/প্রত্যাখ্যান, পারমিশন ৩৮২-৩৮৪); কাস্টমার টিকেট (appointment_ticket জমা/লিস্ট/ডিটেইল/ক্লোজ + ব্যাকএন্ড রিপ্লাই, পারমিশন ৩৮৫/৩৮৭); মাল্টি-লেভেল ডিস্ট্রিবিউশন - লেভেল ২ রিবেট (payLevel2Reward ট্রানজেকশন সারি লক + uk_order_referred আইডেম্পোটেন্সি, পারমিশন ৩৮৬)。
>
> Round-21 সংযোজন: গ্রোথ লেভেল সুবিধা বাস্তবায়ন (অর্ডারে discount_rate ছাড় + পেমেন্টে points_multiplier পয়েন্ট গুণ, মাইগ্রেশন সিড ৫ লেভেল benefits); ইনভয়েস হেডার ম্যানেজমেন্ট (appointment_invoice_title হেডার লাইব্রেরি + আবেদনে title_id লিংক); টিকেট স্যাটিসফ্যাকশন (ক্লোজে রেটিং rating/rated_at + admin সামারি স্ট্যাট, পারমিশন ৩৮৮); রিভিউ ইমেজ অডিট (ReviewAuditController হাইড/রিস্টোর, পারমিশন ৩৮৯-৩৯১); ব্যবহারকারী ব্রাউজ হিস্ট্রি (appointment_browse_history + ডিটেইল হুক + লিস্ট/ডিলিট/ক্লিয়ার)。
>
> Round-22 সংযোজন: ফুল রিডাকশন অ্যাক্টিভিটি (appointment_full_reduction অটো ছাড় + শর্ত যাচাই, পারমিশন ৩৯৬-৪০০); ICS ক্যালেন্ডার এক্সপোর্ট (RFC5545 আমার অ্যাপয়েন্টমেন্ট); টেকনিশিয়ান চেক-ইন (appointment_technician_attendance চেক-ইন/আউট + লেট মার্ক + admin স্ট্যাট, পারমিশন ৩৯২-৩৯৩); APP পুশ সার্ভিস (কনফিগ-ড্রাইভেন অ্যাবস্ট্রাকশন + ৫টি ইভেন্ট ইন্টিগ্রেশন, appointment_push_log); WeChat অফিসিয়াল প্রফিট শেয়ারিং (appointment_profit_sharing_log কনফিগ-ড্রাইভেন + ডাউনগ্রেড, পারমিশন ৩৯৪); প্রাইভেসি কমপ্লায়েন্স (ডেটা এক্সপোর্ট + অ্যাকাউন্ট ডিলিটেশন ৭২ ঘণ্টা স্টেট মেশিন close_status)。
>
> Round-23 সংযোজন: ব্যবহারকারী হেলথ প্রোফাইল (appointment_user_health_profile); ওয়ালেট পেমেন্ট পাসওয়ার্ড (appointment_user_wallet pay_password সেট/যাচাই); টেকনিশিয়ান ব্যাচ শিডিউল (batch ইমপোর্ট + ওভারল্যাপ সংঘর্ষ ডিটেকশন); অর্ডার স্ট্যাটাস টাইমলাইন (appointment_order_status_log ৮ স্ট্যাটাস ট্রেস + ব্যবহারকারী/ব্যাকএন্ড ডিসপ্লে); পয়েন্ট লাকি হুইল (appointment_lucky_wheel + appointment_wheel_record ওয়েটেড লটারি, পারমিশন ৪০১-৪০৬); পয়েন্ট ভ্যালিডিটি (points.expiry_days কনফিগ + নতুন earn লেনদেনে expires_at)।
>
> Round-24 সংযোজন: গেস্ট মোড (/api/v1/guest/* লগইন ছাড়া রিড-অনলি ব্রাউজ + Redis ক্যাশ); সেকিল (appointment_seckill_activity + Redis NX সারি লক কেনা + appointment_order.seckill_id ইনজেক্ট অর্ডার, পারমিশন ৪০৭-৪১১/৪২০); APP ভার্সন ম্যানেজমেন্ট ও আপডেট ডিটেকশন (appointment_app_version + /api/v1/app/version, পারমিশন ৪১৬-৪১৯); রিটার্ন কাস্টমার রিওয়ার্ড (৩০ দিনে দ্বিতীয় খরচ বোনাস type=return_customer, পারমিশন ৪১২-৪১৪); শিডিউল CSV এক্সপোর্ট (UTF-8 BOM + টাইম স্লট ডিটেইল, পারমিশন ৪১৫)。
>
> 2026-08-26 নিরাপত্তা শক্তিশালীকরণ: অর্ডার API-তে অর্ডার আইটেমের দাম সবসময় ডেটাবেস রেকর্ড অনুযায়ী (ক্লায়েন্ট প্রাইস অবিশ্বস্ত, অজানা target_type ৪২২, target_id অবশ্যই hashid), গ্রুপ বাই/সেকিল প্রাইসও DB-ই প্রামাণ্য; সেকিল স্টক সবসময় /api/v1/order store() ট্রানজেকশনের ভেতরে সারি লকে কাটা (SeckillController::buy আর প্রি-ডিডাক্ট করে না, Redis অ্যাক্টিভিটি লক + client_token আইডেম্পোটেন্সি রেখেছে); টেকনিশিয়ান উত্তোলন আবেদনে ইন-ফ্লাইট রিজার্ভ, অনুমোদন ট্রান্সফারের আগে পুনঃচেক, কনকারেন্ট অনুমোদনে ডাবল পেমেন্ট প্রতিরোধ; WeChat পেমেন্ট কলব্যাক total_fee অর্ডার পাওনাদির সাথে কঠোর তুলনা, Alipay কলব্যাক লগ মাস্কিং; /install ইনস্টল সফলে .install.lock ডাবল-চেক পুনঃইনস্টল প্রতিরোধ; ডিপেন্ডেন্সি ভার্সন কনভার্জেন্স (webman-scout 2.0.5 / opensearch-php ^2.6 / dompdf, security-php, webman-database সুনির্দিষ্ট লক); দুই অ্যাপের phpstan.neon ফিক্স করা চালুযোগ্য (php -d memory_limit=2G)。
