...
> **Languages**: [中文](../API.md) · [English](../en/API.md) · [한국어](../ko/API.md) · [Русский](../ru/API.md) · [Deutsch](../de/API.md) · [Français](../fr/API.md) · [Português](../pt/API.md) · [हिन्दी](../hi/API.md) · [العربية](../ar/API.md) · [বাংলা](../bn/API.md) · [Bahasa Indonesia](../id/API.md) · [日本語](../ja/API.md)
...
- **Control de versiones**: la versión queda fijada en el prefijo de ruta de la URL `/api/v1` (p. ej. `POST /api/v1/auth/login`); una URL sin el prefijo de versión devuelve 404
...
**`POST /api/v1/captcha/send`** — envía el código de verificación SMS
...
**`POST /api/v1/auth/login`** — inicio de sesión con contraseña
...
**`POST /api/v1/auth/login-by-code`** — inicio de sesión con código de verificación
...
**`POST /api/v1/auth/forget-password`** — contraseña olvidada
...
**`POST /api/v1/wechat/mini-login`** — inicio de sesión del mini programa

Solicitud: `{"code":"código de inicio de sesión de WeChat"}`
Nota: el primer inicio de sesión requiere llamar después a `/api/v1/wechat/phone` para vincular el teléfono.

---

**`POST /api/v1/wechat/phone`** — vincular teléfono

Solicitud: `{"code":"código del componente de teléfono de WeChat"}`

---

**`POST /api/v1/wechat/oa-login`** — inicio de sesión de la cuenta oficial
...
**`GET /api/v1/common/config`** — configuración pública
...
**`GET /api/v1/common/area`** — lista de ciudades y regiones

---

#### 1.5 Consulta de servicios

**`GET /api/v1/service/categories`** — lista de categorías

Parámetros: `?parent_id=0`

---

**`GET /api/v1/service/items`** — lista de servicios

Parámetros: `?category_id=&page=1&per_page=10&sort=sales`

---

**`GET /api/v1/service/detail/{id}`** — detalle del servicio

La respuesta incluye: imagen/nombre/precio/especificaciones/duración/ventas/lista de evaluaciones.

---

**`GET /api/v1/service/products`** — lista de productos

**`GET /api/v1/service/stores`** — lista de tiendas
...
**`GET /api/v1/technician/list`** — lista de técnicos
...
**`GET /api/v1/content/articles`** — lista de anuncios/artículos

Parámetros: `?type=announcement&page=1`

**`GET /api/v1/content/article/{id}`** — detalle del artículo

---

#### 1.8 LBS

**`GET /api/v1/lbs/nearby-stores`** — tiendas cercanas

Parámetros: `?lat=&lng=&radius=5000`

**`GET /api/v1/lbs/geocode`** — geocodificación inversa
...
|------|------|------|
| GET | `/api/v1/user/profile` | Obtener información personal |
| PUT | `/api/v1/user/profile` | Actualizar apodo/avatar/género |
| POST | `/api/v1/user/change-password` | Cambiar contraseña (old_password/new_password/confirm_password) |
| POST | `/api/v1/user/change-phone` | Cambiar teléfono (old_code/new_phone/new_code) |
| POST | `/api/v1/user/cancel-account` | Cancelar cuenta (requiere verificar contraseña) |
| POST | `/api/v1/user/logout` | Cerrar sesión (el token entra en la lista negra) |
| POST | `/api/v1/user/switch-role` | Cambiar identidad (role: customer/technician) |
...
| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/user/addresses` | Lista de direcciones |
| POST | `/api/v1/user/addresses` | Agregar dirección (contact_name/contact_phone/province/city/district/detail/lat/lng/is_default) |
| GET | `/api/v1/user/addresses/{id}` | Detalle de la dirección |
...
| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/user/favorites` | Lista de favoritos (?type=service/technician) |
| POST | `/api/v1/user/favorites` | Agregar favorito (target_type/target_id) |
| DELETE | `/api/v1/user/favorites/{id}` | Quitar favorito |

#### 2.4 Comentarios

`POST /api/v1/user/feedback` — enviar comentario (content + array images)

#### 2.5 Promoción y recomendación

| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/user/referral` | Información de promoción (código de recomendación/número de recomendados/número de primer pedido/puntos obtenidos) |
| GET | `/api/v1/user/referral/qrcode` | Código QR de promoción (código de recomendación + enlace de invitación) |
| GET | `/api/v1/user/referral/referred-users` | Lista de usuarios recomendados |
| GET | `/api/v1/user/referral/earnings` | Detalle de comisiones de distribución (paginado: apodo/avatar del recomendado/número de pedido/importe/hora de emisión) |
...
| Método | Ruta | Descripción |
|------|------|------|
| POST | `/api/v1/user/points/transfer` | Transferencia de puntos (to_user_id hashid/points) |
| GET | `/api/v1/user/points/transfers` | Registros de transferencia (?direction=sent/received&page=1) |
...
| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/user/notify-settings` | Consultar los interruptores de notificación (los 5 tipos completos) |
| PUT | `/api/v1/user/notify-settings` | Actualizar interruptores por lotes (types: {service_reminder: 0/1, ...}) |
...
| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/technician/profile` | Obtener el archivo del técnico |
| PUT | `/api/v1/technician/profile` | Actualizar el archivo (avatar/intro/real_name/gender/id_card/id_card_front/id_card_back) |
...
| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/technician/schedule` | Consulta de horarios (?start_date=&end_date=) |
| PUT | `/api/v1/technician/schedule` | Configurar horarios (date/time_slots/status), intervalos superpuestos 422 «conflicto con el horario existente» |
| POST | `/api/v1/technician/schedule/batch` | Horarios masivos (Ronda 23): tramo de fechas ≤7 días + filtro weekdays, los días con horarios existentes se saltan, respuesta created/skipped |

#### 3.3 Pedidos del técnico

`GET /api/v1/technician/orders` — lista de pedidos (?status=&page=1)

#### 3.4 Ganancias

`GET /api/v1/technician/earnings` — resumen de ganancias (today_income/pending_settlement/balance + lista de historial)

#### 3.5 Retiro

`POST /api/v1/technician/withdraw` — solicitar retiro (amount)
...
`POST /api/v1/technician/review/reply/{order_id}` — respuesta del técnico a la evaluación (reply). Evaluación inexistente/no propia unificada en 404 (no filtra la existencia); respuesta existente 422 (rechazo idempotente sin sobrescribir); respuesta vacía 422. Al responder correctamente, notificación interna al usuario (type='review_reply').

#### 3.6 Panel de trabajo

| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/technician/work/today` | Lista de tareas de hoy |
| GET | `/api/v1/technician/work/records` | Registros completados paginados |
| POST | `/api/v1/technician/work/{id}/start` | Iniciar servicio |
| POST | `/api/v1/technician/work/{id}/complete` | Completar servicio |
...
| Método | Ruta | Descripción |
|------|------|------|
| POST | `/api/v1/order` | Crear pedido (order_type/items/store_id/technician_id/service_time/coupon_id/user_coupon_id/promotion_id/remark) |
| GET | `/api/v1/order/list` | Lista de pedidos (?status=&page=1) |
| GET | `/api/v1/order/detail/{id}` | Detalle del pedido |
| POST | `/api/v1/order/cancel/{id}` | Cancelar pedido (reason) |
| POST | `/api/v1/order/pay/{id}` | Iniciar pago (pay_channel: wechat/balance, use_points: descuento con puntos opcional) |
| POST | `/api/v1/order/refund/{id}` | Solicitar reembolso |
| POST | `/api/v1/order/verify/{id}` | Verificación (code: valor del código QR) |
| POST | `/api/v1/order/reschedule/{id}` | Reprogramación de reserva (new_service_time obligatorio/reason opcional) |
| GET | `/api/v1/order/logistics/{id}` | Seguimiento logístico (Ronda 19, pedidos product) |
| POST | `/api/v1/order/review/{order_id}` | Enviar evaluación (rating 1-5/content/images) (registro completado en la Ronda 19) |
| POST | `/api/v1/order/review/{order_id}/append` | Evaluación complementaria (content/images separados por comas) (Ronda 19) |
...
| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/promotions` | Lista de actividades (?type=group_buy; flash_sale filtrado y no devuelto) |
| GET | `/api/v1/promotions/{id}` | Detalle de la actividad (incluye número de participantes/si se formó el grupo; tipo flash_sale 400) |
| GET | `/api/v1/promotions/{id}/participants` | Lista de participantes |
| POST | `/api/v1/promotions/join/{id}` | Participar en la actividad (completado en la Ronda 15: la respuesta incluye discount_percent/original_price/group_price; tipo flash_sale 400) |
...
| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/marketing/coupons` | Lista de cupones (?status=available/used/expired) |
| POST | `/api/v1/marketing/coupons/receive` | Recibir cupón (coupon_id) |
| GET | `/api/v1/marketing/cards` | Lista de tarjetas de membresía |
| POST | `/api/v1/marketing/cards/buy` | Comprar tarjeta de membresía (card_id) |
| GET | `/api/v1/marketing/cards/my` | Lista de mis tarjetas por uso |
| POST | `/api/v1/marketing/cards/use` | Verificar tarjeta por uso (user_card_id/service_id/remark?) |
| GET | `/api/v1/marketing/gift-cards` | Lista de tarjetas regalo |
| GET | `/api/v1/marketing/gift-cards/my` | Mis tarjetas regalo (registros redeem) |
| POST | `/api/v1/marketing/gift-cards/redeem` | Canjear tarjeta regalo (el tipo cash recarga el saldo de la billetera tras el canje) |
| GET | `/api/v1/marketing/points` | Historial de puntos (?type=earn/use/expire&source=order/referral/gift_card/check_in/admin) |
| GET | `/api/v1/marketing/points-exchange` | Lista de productos de canje de puntos (en venta + stock restante en tiempo real + cantidad canjeada) |
| POST | `/api/v1/marketing/points-exchange/{id}` | Canjear (type=coupon emite cupón / wallet ingresa saldo / gift_card devuelve tarjeta con código) |
| POST | `/api/v1/marketing/coupons/transfer` | Generar código de transferencia (user_coupon_id: código único de 8 caracteres/válido 7 días) |
| POST | `/api/v1/marketing/coupons/claim` | Recibir cupón transferido (code) |
| GET | `/api/v1/marketing/coupons/transfers` | Registros de transferencia (emitidos pending/claimed/expired + recibidos claimed) |
...
| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/notification` | Lista de notificaciones (?type=order/system&page=1) |
| PUT | `/api/v1/notification/read/{id}` | Marcar leída |
| PUT | `/api/v1/notification/read-all` | Marcar todas leídas |
...
| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/wallet` | Saldo de la billetera + historial paginado |
| POST | `/api/v1/wallet/recharge` | Crear orden de recarga (amount: yuanes) |
| POST | `/api/v1/wallet/recharge/{id}/pay` | Iniciar pago de la orden de recarga (WeChat) |
| POST | `/api/v1/wallet/transfer` | Transferencia de saldo (to_user_id hashid/amount/remark opcional/client_token opcional) (Ronda 19) |
| GET | `/api/v1/wallet/transfers` | Registros de transferencia (?direction=out/in&page=1) (Ronda 19) |
| GET | `/api/v1/wallet/transfers/{id}` | Detalle de la transferencia (solo visible para ambas partes, ajeno 404) (Ronda 19) |
...
**Recarga**: `POST /api/v1/wallet/recharge` pasando amount (yuanes) crea la orden de recarga y devuelve el hashid de la orden. `POST /api/v1/wallet/recharge/{id}/pay` inicia el pago de WeChat, la respuesta incluye sign_params (mismo patrón que el pago de pedidos); la devolución de llamada de pago distingue las órdenes de recarga de los pedidos con el out_trade_no de prefijo R.
...
| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/store-manager/overview` | Resumen de hoy (pedidos de hoy/ingresos de hoy/en curso/número de técnicos/número de verificaciones) |
| GET | `/api/v1/store-manager/orders` | Lista de pedidos de la tienda (?status=&page=&limit=) |
| GET | `/api/v1/store-manager/technicians` | Lista de técnicos (incluye horarios de hoy) |
| GET | `/api/v1/store-manager/revenue` | Agregación de ingresos de los últimos 7 días |
...
| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/growth` | Resumen de crecimiento actual (balance/nivel/diferencia al siguiente nivel/nombre del nivel) |
| GET | `/api/v1/growth/records` | Historial de crecimiento paginado (?page=&limit=) |
| GET | `/api/v1/growth/levels` | Lista de niveles (pública, sin necesidad de inicio de sesión) |
...
| Método | Ruta | Descripción |
|------|------|------|
| POST | `/api/v1/invoices` | Solicitar factura (order_id hashid/order_type: service=servicio/points_exchange=canje de puntos/order_type predeterminado service; el importe y el título los aporta el servidor, no manipulables) |
| GET | `/api/v1/invoices` | Lista de facturas (?status=&page=) |
| GET | `/api/v1/invoices/{id}` | Detalle de la factura (solo propio) |
...
| Método | Ruta | Descripción |
|------|------|------|
| POST | `/api/v1/tickets` | Enviar ticket (title/content obligatorios) |
| GET | `/api/v1/tickets` | Lista de tickets (?status=open/closed&page=) |
| GET | `/api/v1/tickets/{id}` | Detalle del ticket (solo propio, ajeno 404) |
| POST | `/api/v1/tickets/{id}/close` | Cerrar ticket (solo propio/solo open; rating opcional 1-5 de satisfacción, fuera de rango/no entero 422, si no se proporciona compatible NULL) |

### 12. Interfaces de calendario de reservas (requieren autenticación JWT, Ronda 20)

| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/calendar/technician/{id}` | Vista mensual (?month=YYYY-MM): time_slots de horarios expandidos en intervalos horarios + exclusión de lo ya reservado |
| GET | `/api/v1/calendar/technician/{id}/day` | Vista diaria (?date=YYYY-MM-DD): detalle de intervalos disponibles/reservados/no disponibles de ese día |

### 13. Interfaces de títulos de factura (requieren autenticación JWT, Ronda 21)

| Método | Ruta | Descripción |
|------|------|------|
| POST | `/api/v1/invoice-titles` | Guardar título (title_type: personal/company; company requiere tax_no; título duplicado del mismo usuario 422; la primera entrada es predeterminada automáticamente) |
| GET | `/api/v1/invoice-titles` | Lista de títulos (el predeterminado arriba) |
| PUT | `/api/v1/invoice-titles/{id}` | Editar título (solo propio) |
| DELETE | `/api/v1/invoice-titles/{id}` | Eliminar título (solo propio; al eliminar el predeterminado se asigna automáticamente la más antigua) |
| POST | `/api/v1/invoice-titles/{id}/default` | Establecer predeterminado (transacción limpia las otras filas del mismo usuario) |

**Vinculación en la solicitud**: POST /api/v1/invoices soporta title_id opcional — resuelve el título e incorpora automáticamente invoice_title/tax_no/title_type, sin title_id se conserva la ruta de relleno manual original.

### 14. Interfaces de historial de navegación (requieren autenticación JWT, Ronda 21)

| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/browse-history` | Servicios vistos recientemente (join del nombre del servicio/portada/precio/precio original, viewed_at descendente, per_page por defecto 15 máximo 50) |
| DELETE | `/api/v1/browse-history/{item_id}` | Eliminar uno (solo propio, inválido/ajeno 404) |
| DELETE | `/api/v1/browse-history` | Vaciar el historial (solo propio) |
...
| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/full-reduction-activities` | Lista de actividades de reducción de importe activas (status=1 y dentro del período de validez, ordenadas por importe de reducción descendente; interfaz pública) |
...
| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/order/ics` | Exportar los pedidos vigentes de los últimos 90 días (pending/paid/confirmed/serving) como iCal (RFC5545) |
...
| Método | Ruta | Descripción |
|------|------|------|
| POST | `/api/v1/technician/attendance/check-in` | Registro de entrada (duplicado del mismo día 422, índice único de respaldo contra concurrencia; >10:00 se marca retraso) |
| POST | `/api/v1/technician/attendance/check-out` | Registro de salida (sin fichar/fichado de salida 422, bloqueo de fila contra concurrencia) |
| GET | `/api/v1/technician/attendance` | Lista de asistencia del mes actual + resumen de días de asistencia/horas totales/horas medias (?month=YYYY-MM, inválido 422) |

### 18. Interfaces de cumplimiento de privacidad (requieren autenticación JWT, Ronda 22)

| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/privacy/data` | Exportación de datos (JSON agrupado por personal/orders/points/wallet_txns/reviews/addresses/invoices; el log del servidor solo registra el teléfono desidentificado + la cantidad) |
| POST | `/api/v1/privacy/close-request` | Solicitar cancelación (saldo distinto de 0 / pedidos sin completar / tickets en curso 422; pone close_status=1 + close_requested_at) |
| POST | `/api/v1/privacy/close-cancel` | Cancelar la solicitud de cancelación (close_status 1→0) |
| POST | `/api/v1/privacy/close-confirm` | Confirmar la cancelación (solo tras 72 h; close_status=2 + close_at + teléfono/apodo anonimizados a user{id} + status=0) |
...
| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/health-profile` | Consultar mi perfil de salud (sin perfil devuelve objeto vacío) |
| PUT | `/api/v1/health-profile` | Crear/actualizar (upsert, uno por persona; allergies/health_notes con límite de 500 caracteres, preferred_technician_id valida existencia; solo actualiza los campos proporcionados, respuesta con codificación hashid) |
| DELETE | `/api/v1/health-profile` | Eliminar mi perfil (solo propio) |
...
| Método | Ruta | Descripción |
|------|------|------|
| POST | `/api/v1/wallet/pay-password/set` | Establecer contraseña de pago (6 dígitos `\d{6}`; si ya está establecida, requiere la contraseña antigua 422 de bloqueo) |
| POST | `/api/v1/wallet/pay-password/verify` | Validar la contraseña de pago (devuelve booleano correcto/incorrecto, sin escribir) |
| POST | `/api/v1/wallet/pay-password/check` | Consultar si está establecida (set: true/false) |
...
| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/order/{id}/timeline` | Línea de tiempo de cambios de estado del pedido (descendente; solo propio, pedido ajeno 404 sin filtrar la existencia) |
...
| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/wheel/prizes` | Lista de premios de la ruleta (oculta los campos sensibles weight/stock) |
| POST | `/api/v1/wheel/spin` | Sorteo de una vez (Redis NX + bloqueo de fila contra concurrencia; extracción ponderada con random_int; puntos → historial earn con fecha de caducidad, saldo → ingreso con lockForUpdate, cupón → pending de emisión manual, sin premio → lose; idempotencia client_token) |
| GET | `/api/v1/wheel/records` | Mis registros de sorteos (paginado) |

### 23. Interfaces de modo invitado (Ronda 24)

| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/guest/home` | Agregación del inicio (banners/anuncios/categorías de servicios/servicios populares, caché Redis svc:guest:home 300 s) |
| GET | `/api/v1/guest/services` | Lista de servicios (?category_id=hashid&sort=newest\|sales\|price&page/per_page≤50) |
| GET | `/api/v1/guest/services/{id}` | Detalle del servicio (inexistente 404) |
| GET | `/api/v1/guest/stores` | Lista de tiendas |
| GET | `/api/v1/guest/technicians` | Lista de técnicos (solo aprobados; ?service_id=hashid filtro; puntuación descendente) |

Entradas de navegación sin inicio de sesión que no requieren autenticación (interfaz pública).

### 24. Interfaces de oferta flash (requieren autenticación JWT, Ronda 24)

| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/seckill` | Lista de actividades de oferta flash (status=1 y dentro de la ventana de tiempo; incluye vendidos = pedidos con appointment_order.seckill_id, stock restante) |
| GET | `/api/v1/seckill/{id}` | Detalle de la actividad (state=not_started/ongoing/ended) |
| POST | `/api/v1/seckill/{id}/buy` | Pedido flash (idempotencia client_token + Redis NX 30 s contra concurrencia + validación de la actividad; ya no reserva stock de antemano) |

**Reglas de pedido (desde 2026-08-26)**: el stock se reduce uniformemente con bloqueo de fila dentro de la transacción de `/api/v1/order store()`, buy solo hace la validación de entrada/idempotencia; precio flash = seckill_price (según DB), sin superposición de cupones/puntos/tarjetas de membresía; la cancelación del pedido no repone el stock; llamar directamente a `/api/v1/order` con seckill_id también descuenta stock.

### 25. Interfaces de comprobación de versión de APP (Ronda 24)

| Método | Ruta | Descripción |
|------|------|------|
| GET | `/api/v1/app/version?platform=android|ios` | Comprobación de la versión más reciente (platform inválido 422; sin versión devuelve objeto vacío; interfaz pública) |
...
Cabecera de solicitud: `Authorization: Bearer <admin_token>`; la versión de los extremos públicos de autenticación sigue el prefijo de URL `/api/v1`
...
IDs de permiso: 416-419. La interfaz de detección de actualizaciones /api/v1/app/version toma la más reciente (mayor updated_at/id) de las de status=1.
