...
│   ├── app/                   #   admin (controladores del panel)/api/v1/model/middleware/process/view
...
| UI completa del panel de administración | Flutter Web 21 páginas: dashboard/usuarios/roles/configuración/logs/verificación/horarios/servicios/técnicos/pedidos/cupones/membresías/tarjetas por uso/anuncios/FAQ/retiros/evaluaciones/informes/posventa/panel del gerente de tienda/centro personal |
| Estadísticas en tiempo real del panel | La página de inicio del admin renderiza dinámicamente 7 tarjetas de estadísticas (total usuarios/nuevos hoy/activos/registro de operaciones/reservas de hoy/retiros pendientes/técnicos pendientes de revisión) + gráficos de tendencia de 30 días (volumen de pedidos/importe/nuevos usuarios/actividad) + diagrama circular de estado de usuarios + últimos registros de operaciones, caché Redis svc:dashboard 300s |
| Informes de datos | 3 puntos de conexión de ReportController: estadísticas de pedidos / TOP10 técnicos / distribución de canales (GET /admin/reports/orders\|technicians\|distribution, rango 7/30 días, caché Redis 300s) + estadísticas de ventas (svc:sales_stats) + estadísticas financieras (svc:finance_stats ingresos/reembolsos/retiros/comisiones) |
...
| Panel de trabajo del gerente de tienda | service /api/v1/store-manager con 4 interfaces (overview/orders/technicians/revenue) con aislamiento forzado por store_id (403 sin tienda); vista general del panel de trabajo de tienda en admin + filtro por store_id en pedidos + página Flutter + permiso 372 |
...
| Reprogramación de reservas | POST /api/v1/order/reschedule/{id} cambia de hora con el mismo técnico; solo pending/paid/confirmed y con ≥6 h antes del inicio del servicio original; order_lock + bloqueo del técnico en el nuevo horario con SETNX(180s) contra sobreventa concurrente + validación de conflicto de horarios B2; se registra en appointment_order_reschedule + mensaje de suscripción SCENE_RESCHEDULE |
...
| Respuesta del técnico a las evaluaciones | POST /api/v1/technician/review/reply/{order_id}: 404 si no es su propia evaluación, 422 por respuesta repetida, notificación interna al usuario al responder; appointment_order_review añade replied_at; detalle de respuesta en admin (permiso 381) |
