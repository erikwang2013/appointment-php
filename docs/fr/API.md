...
- **Gestion des versions** : la version est figée dans le préfixe de chemin de l'URL `/api/v1` (p. ex. `POST /api/v1/auth/login`) ; une URL sans préfixe de version renvoie 404
...
**`POST /api/v1/captcha/send`** — Envoi d'un code de vérification SMS
...
**`POST /api/v1/auth/register`** — Inscription par numéro de téléphone
...
**`POST /api/v1/auth/login`** — Connexion par mot de passe
...
**`POST /api/v1/auth/login-by-code`** — Connexion par code de vérification
...
**`POST /api/v1/wechat/mini-login`** — Connexion au mini-programme

Requête : `{"code":"微信登录code"}`
Note : lors de la première connexion, il faut ensuite appeler `/api/v1/wechat/phone` pour lier le numéro de téléphone.

---

**`POST /api/v1/wechat/phone`** — Liaison du numéro de téléphone

Requête : `{"code":"微信手机号组件code"}`

---

**`POST /api/v1/wechat/oa-login`** — Connexion au compte officiel
...
**`GET /api/v1/common/config`** — Configuration publique
...
**`GET /api/v1/common/area`** — Liste des villes et régions

---

#### 1.5 Consultation des prestations

**`GET /api/v1/service/categories`** — Liste des catégories

Paramètre : `?parent_id=0`

---

**`GET /api/v1/service/items`** — Liste des prestations

Paramètres : `?category_id=&page=1&per_page=10&sort=sales`

---

**`GET /api/v1/service/detail/{id}`** — Détail d'une prestation

La réponse contient : images/nom/prix/spécifications/durée/volume de ventes/liste des évaluations.

---

**`GET /api/v1/service/products`** — Liste des produits

**`GET /api/v1/service/stores`** — Liste des boutiques
...
**`GET /api/v1/technician/list`** — Liste des techniciens
...
**`GET /api/v1/technician/detail/{id}`** — Détail d'un technicien

La réponse contient : images/nom/présentation/note/distance/liste des prestations proposées/évaluations.

---

**`GET /api/v1/technician/schedule/{id}`** — Planning du technicien
...
**`GET /api/v1/content/banners`** — Bannières

Paramètre : `?position=home`

**`GET /api/v1/content/articles`** — Liste des annonces/articles

Paramètres : `?type=announcement&page=1`

**`GET /api/v1/content/article/{id}`** — Détail d'un article

---

#### 1.8 LBS

**`GET /api/v1/lbs/nearby-stores`** — Boutiques à proximité

Paramètres : `?lat=&lng=&radius=5000`

**`GET /api/v1/lbs/geocode`** — Géocodage inverse
...
|------|------|------|
| GET | `/api/v1/user/profile` | Récupération des informations personnelles |
| PUT | `/api/v1/user/profile` | Mise à jour du surnom/avatar/sexe |
| POST | `/api/v1/user/change-password` | Modification du mot de passe (old_password/new_password/confirm_password) |
| POST | `/api/v1/user/change-phone` | Changement de téléphone (old_code/new_phone/new_code) |
| POST | `/api/v1/user/cancel-account` | Suppression du compte (vérification du mot de passe requise) |
| POST | `/api/v1/user/logout` | Déconnexion (token ajouté à la liste noire) |
| POST | `/api/v1/user/switch-role` | Changement d'identité (role: customer/technician) |
...
| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/user/addresses` | Liste des adresses |
| POST | `/api/v1/user/addresses` | Nouvelle adresse (contact_name/contact_phone/province/city/district/detail/lat/lng/is_default) |
| GET | `/api/v1/user/addresses/{id}` | Détail d'une adresse |
| PUT | `/api/v1/user/addresses/{id}` | Mise à jour d'une adresse |
| DELETE | `/api/v1/user/addresses/{id}` | Suppression d'une adresse |
...
| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/user/favorites` | Liste des favoris (?type=service/technician) |
| POST | `/api/v1/user/favorites` | Ajout d'un favori (target_type/target_id) |
| DELETE | `/api/v1/user/favorites/{id}` | Retrait d'un favori |

#### 2.4 Retours d'expérience

`POST /api/v1/user/feedback` — Soumission d'un retour (content + tableau images)

#### 2.5 Parrainage

| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/user/referral` | Informations de parrainage (code de parrainage/nombre de recommandés/nombre de premières commandes/points obtenus) |
| GET | `/api/v1/user/referral/qrcode` | QR code de parrainage (code de parrainage + lien d'invitation) |
| GET | `/api/v1/user/referral/referred-users` | Liste des utilisateurs recommandés |
| GET | `/api/v1/user/referral/earnings` | Détail des commissions de distribution (pagé : surnom/avatar du recommandé/numéro de commande/montant/heure d'émission) |

**Commission de distribution** : versée après la première commande completed du recommandé, montant = paid_amount × reward_rate (appointment_system_config referral.reward_rate, défaut 0.05, repli sur la constante en cas de valeur invalide). Triple idempotence : verrou de ligne + contrôle rewarded_at non vide + re-vérification de la première commande ; crédit WalletTxn type=referral_reward.

#### 2.6 Transfert de points (tour 19)

| Méthode | Chemin | Description |
|------|------|------|
| POST | `/api/v1/user/points/transfer` | Transfert de points (to_user_id hashid/points) |
| GET | `/api/v1/user/points/transfers` | Enregistrements de transfert (?direction=sent/received&page=1) |
...
| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/technician/profile` | Récupération du dossier du technicien |
| PUT | `/api/v1/technician/profile` | Mise à jour du dossier (avatar/intro/real_name/gender/id_card/id_card_front/id_card_back) |
...
| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/technician/schedule` | Consultation du planning (?start_date=&end_date=) |
| PUT | `/api/v1/technician/schedule` | Définition du planning (date/time_slots/status), chevauchement de créneaux 422 « en conflit avec un planning existant » |
| POST | `/api/v1/technician/schedule/batch` | Planning par lots (tour 23) : période ≤ 7 jours + filtre weekdays, jours déjà planifiés ignorés, réponse created/skipped |

#### 3.3 Commandes du technicien

`GET /api/v1/technician/orders` — Liste des commandes (?status=&page=1)

#### 3.4 Revenus

`GET /api/v1/technician/earnings` — Aperçu des revenus (today_income/pending_settlement/balance + liste des flux)

#### 3.5 Retrait

`POST /api/v1/technician/withdraw` — Demande de retrait (amount)
...
`POST /api/v1/technician/review/reply/{order_id}` — Réponse du technicien à une évaluation (reply). Évaluation inexistante/non personnelle : 404 unifié (sans fuite d'existence) ; réponse existante 422 (refus idempotent sans écrasement) ; réponse vide 422. Après succès, notification interne à l'utilisateur (type='review_reply').

#### 3.6 Poste de travail

| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/technician/work/today` | Liste des tâches du jour |
| GET | `/api/v1/technician/work/records` | Enregistrements terminés paginés |
| POST | `/api/v1/technician/work/{id}/start` | Début du service |
| POST | `/api/v1/technician/work/{id}/complete` | Fin du service |
...
| Méthode | Chemin | Description |
|------|------|------|
| POST | `/api/v1/order` | Création d'une commande (order_type/items/store_id/technician_id/service_time/coupon_id/user_coupon_id/promotion_id/remark) |
| GET | `/api/v1/order/list` | Liste des commandes (?status=&page=1) |
| GET | `/api/v1/order/detail/{id}` | Détail d'une commande |
| POST | `/api/v1/order/cancel/{id}` | Annulation d'une commande (reason) |
| POST | `/api/v1/order/pay/{id}` | Lancement du paiement (pay_channel: wechat/balance, use_points: réduction en points optionnelle) |
| POST | `/api/v1/order/refund/{id}` | Demande de remboursement |
| POST | `/api/v1/order/verify/{id}` | Vérification (code: valeur du QR code) |
| POST | `/api/v1/order/reschedule/{id}` | Report de réservation (new_service_time obligatoire/reason facultatif) |
| GET | `/api/v1/order/logistics/{id}` | Suivi logistique (tour 19, commandes product) |
| POST | `/api/v1/order/review/{order_id}` | Soumission d'une évaluation (rating 1-5/content/images) (enregistrement complété au tour 19) |
| POST | `/api/v1/order/review/{order_id}/append` | Évaluation complémentaire (content/images séparés par des virgules) (tour 19) |
...
**Anti-falsification des prix (2026-08-26)** : les montants des articles de commande se réfèrent toujours aux enregistrements de la base (target_type=service interroge appointment_service, product interroge appointment_product), les prix envoyés par le client ne participent pas au calcul ; target_type inconnu 422 ; target_id doit être une valeur encodée en hashids (envoyer un id brut le décode à 0 → 422 « produit inexistant ou retiré ») ; les prix des achats groupés/flash sont également déterminés par la base.
...
**Commande d'achat groupé (tour 16)** : promotion_id facultatif (hashid) à la création de la commande. Validation : uniquement type group_buy, activité dans sa période de validité, l'appelant est participant, pas complet (groupe constitué verrouillé 422), la prestation de la commande correspond à l'activité ; prix groupé = prix d'origine × discount_percent/100, bons/cartes à forfait/points désactivés en cumul (tout envoi de l'un d'eux → 422). La commande stocke promotion_id/participant_id ; le paiement réutilise entièrement `POST /api/v1/order/pay/{id}`, avec au pay une évaluation paresseuse de la clôture de l'activité (expirée sans groupe constitué) → la commande est automatiquement annulée et le verrou du technicien libéré.

**Commande flash (tour 18, mis hors ligne)** : ~~promotion_id (type flash_sale) à la création de la commande~~ — depuis 2026-08, l'ancien canal promotionnel FLASH_SALE est supprimé, la branche promotionnelle de store() ne conserve que l'achat groupé GROUP_BUY (promotion non groupé 422) ; le flash passe unifié par le canal `/api/v1/seckill` du tour 24 (seckill_id injecté dans la transaction de store avec verrou de ligne sur le stock), PromotionController::index filtre flash_sale, show/join lui retourne 400, la constante `Promotion::TYPE_FLASH_SALE` est conservée pour la compatibilité des données historiques.

**Report de réservation (tour 17)** : `POST /api/v1/order/reschedule/{id}` avec new_service_time (obligatoire) + reason (facultatif), changement d'horaire avec le même technicien. Règles : commande personnelle uniquement (non personnel 404), uniquement type appointment et statut pending/paid/confirmed modifiable (autres 422), ≥ 6 heures avant le début du service d'origine (aligné sur la fenêtre de remboursement complet) pour reporter. Protection contre la concurrence : B1 order_lock (même famille d'exclusion mutuelle que pay/cancel/refund) → verrou Redis SETNX EX 180 du technicien sur le nouveau créneau (prévention des surventes en report concurrent) → relecture sous verrou de ligne dans la transaction + B2 validation DB des conflits de planning (en excluant la commande en cours) → mise à jour de service_time + enregistrement appointment_order_reschedule → libération du verrou de l'ancien créneau, le nouveau verrou étant détenu par cette commande → message d'abonnement SCENE_RESCHEDULE (dégradé en notification interne si non configuré). En cas d'échec, la transaction est annulée et le verrou du nouveau créneau est également libéré.

**Suivi logistique (tour 19)** : `GET /api/v1/order/logistics/{id}` — uniquement les commandes product personnelles (non personnel/non produit/non expédié : 404 unifié). Lecture du JSON order.remark (shipping_company/tracking_no/shipped_at, écrit à l'expédition par admin MallOrderController::ship()), double analyse parseShippingInfo/parseReceiver pour les anciens formats ; numéro de téléphone du destinataire masqué 138****5678.

**Évaluation (tour 19)** : `POST /api/v1/order/review/{order_id}` soumission d'une évaluation (rating obligatoire 1-5, content/images facultatifs) : non personnel 404, non completed 422, évaluation en double 400. `POST /api/v1/order/review/{order_id}/append` évaluation complémentaire (content obligatoire, images séparées par des virgules) : évaluation inexistante/non personnelle : 404 unifié, non completed 422, complément en double 422, contenu vide 422 ; en cas de succès, écriture de append_content/append_images(JSON)/append_at et notification interne au technicien type='review_append', la réponse expose le champ append.

### 4.1 Interfaces S.A.V. (authentification JWT requise)

| Méthode | Chemin | Description |
|------|------|------|
| POST | `/api/v1/aftersales` | Demande de S.A.V. (order_id hashid/type: refund|exchange/reason), validation commande personnelle 404, uniquement statut paid+completed 422, S.A.V. en cours sur la même commande dédoublonnée 422 |
| GET | `/api/v1/aftersales` | Liste de mes demandes de S.A.V. (?status=&page=1&limit=) |
| GET | `/api/v1/aftersales/{id}` | Détail d'une demande de S.A.V. (contrôle de propriété 404) |

**Statuts de S.A.V.** : pending(en attente d'audit) → approved(approuvée) / rejected(rejetée). approved ne fait que faire évoluer le statut, l'action de remboursement réutilise `POST /api/v1/order/refund/{id}`.
...
| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/promotions` | Liste des activités (?type=group_buy ; flash_sale filtré et non retourné) |
| GET | `/api/v1/promotions/{id}` | Détail d'une activité (nombre de participants/groupe constitué ; type flash_sale 400) |
| GET | `/api/v1/promotions/{id}/participants` | Liste des participants |
| POST | `/api/v1/promotions/join/{id}` | Participation à l'activité (complété au tour 15 : réponse avec discount_percent/original_price/group_price ; type flash_sale 400) |
...
| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/marketing/coupons` | Liste des bons (?status=available/used/expired) |
| POST | `/api/v1/marketing/coupons/receive` | Réception d'un bon (coupon_id) |
| GET | `/api/v1/marketing/cards` | Liste des cartes de membre |
| POST | `/api/v1/marketing/cards/buy` | Achat d'une carte de membre (card_id) |
| GET | `/api/v1/marketing/cards/my` | Liste de mes cartes à forfait |
| POST | `/api/v1/marketing/cards/use` | Utilisation d'une carte à forfait (user_card_id/service_id/remark?) |
| GET | `/api/v1/marketing/gift-cards` | Liste des cartes cadeaux |
| GET | `/api/v1/marketing/gift-cards/my` | Mes cartes cadeaux (enregistrements redeem) |
| POST | `/api/v1/marketing/gift-cards/redeem` | Échange d'une carte cadeau (le type cash crédite le solde du portefeuille après échange) |
| GET | `/api/v1/marketing/points` | Flux de points (?type=earn/use/expire&source=order/referral/gift_card/check_in/admin) |
| GET | `/api/v1/marketing/points-exchange` | Liste des produits d'échange à points (en ligne + stock restant en temps réel + quantité déjà échangée) |
| POST | `/api/v1/marketing/points-exchange/{id}` | Échange (type=coupon émission de bon / wallet crédit / gift_card retour de code) |
| POST | `/api/v1/marketing/coupons/transfer` | Génération d'un code de transfert (user_coupon_id : code unique de 8 caractères/valable 7 jours) |
| POST | `/api/v1/marketing/coupons/claim` | Réclamation du bon transféré (code) |
| GET | `/api/v1/marketing/coupons/transfers` | Enregistrements de transfert (émis pending/claimed/expired + reçus claimed) |
...
| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/notification` | Liste des notifications (?type=order/system&page=1) |
| PUT | `/api/v1/notification/read/{id}` | Marquage comme lu |
| PUT | `/api/v1/notification/read-all` | Tout marquer comme lu |
...
| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/wallet` | Solde du portefeuille + flux paginés |
| POST | `/api/v1/wallet/recharge` | Création d'un bon de recharge (amount: yuans) |
| POST | `/api/v1/wallet/recharge/{id}/pay` | Lancement du paiement du bon de recharge (WeChat) |
| POST | `/api/v1/wallet/transfer` | Transfert de solde (to_user_id hashid/amount/remark facultatif/client_token facultatif) (tour 19) |
| GET | `/api/v1/wallet/transfers` | Enregistrements de transfert (?direction=out/in&page=1) (tour 19) |
| GET | `/api/v1/wallet/transfers/{id}` | Détail d'un transfert (visible uniquement par les deux parties, autrui 404) (tour 19) |
...
**Recharge** : `POST /api/v1/wallet/recharge` avec amount (yuans) crée un bon de recharge et retourne son hashid. `POST /api/v1/wallet/recharge/{id}/pay` lance le paiement WeChat, la réponse contient sign_params (même schéma que le paiement de commande) ; le callback de paiement distingue bons de recharge et commandes par le préfixe R de out_trade_no.
...
| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/store-manager/overview` | Aperçu du jour (commandes du jour/chiffre d'affaires du jour/en cours/nombre de techniciens/vérifications) |
| GET | `/api/v1/store-manager/orders` | Liste des commandes de la boutique (?status=&page=&limit=) |
| GET | `/api/v1/store-manager/technicians` | Liste des techniciens (avec le planning du jour) |
| GET | `/api/v1/store-manager/revenue` | Agrégation du chiffre d'affaires des 7 derniers jours |

**Isolation store_id** : requireStoreId() impose que l'utilisateur actuel soit lié à une boutique (appointment_user.store_id), sans boutique 403 ; toutes les requêtes sont filtrées par store_id.
...
| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/growth` | Aperçu de croissance actuel (balance/niveau/écart au niveau suivant/nom du niveau) |
| GET | `/api/v1/growth/records` | Flux de croissance paginé (?page=&limit=) |
| GET | `/api/v1/growth/levels` | Liste des niveaux (publique, sans connexion) |
...
| Méthode | Chemin | Description |
|------|------|------|
| POST | `/api/v1/invoices` | Demande de facture (order_id hashid/order_type: service=prestation/points_exchange=échange de points/order_type par défaut service ; montant et intitulé portés côté serveur, non falsifiables) |
| GET | `/api/v1/invoices` | Liste des factures (?status=&page=) |
| GET | `/api/v1/invoices/{id}` | Détail d'une facture (personnelle uniquement) |
...
| Méthode | Chemin | Description |
|------|------|------|
| POST | `/api/v1/tickets` | Soumission d'un ticket (title/content obligatoires) |
| GET | `/api/v1/tickets` | Liste des tickets (?status=open/closed&page=) |
| GET | `/api/v1/tickets/{id}` | Détail d'un ticket (personnel uniquement, autrui 404) |
| POST | `/api/v1/tickets/{id}/close` | Clôture d'un ticket (personnel uniquement/open uniquement ; rating 1-5 facultatif pour la satisfaction, hors limites/non entier 422, absent → NULL compatible) |

### 12. Interfaces du calendrier de réservation (authentification JWT requise, tour 20)

| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/calendar/technician/{id}` | Vue mensuelle (?month=YYYY-MM) : time_slots du planning dépliés en créneaux horaires + exclusion des réservations existantes |
| GET | `/api/v1/calendar/technician/{id}/day` | Vue journalière (?date=YYYY-MM-DD) : détail des créneaux réservables/déjà réservés/non réservables |

### 13. Interfaces d'intitulés de facture (authentification JWT requise, tour 21)

| Méthode | Chemin | Description |
|------|------|------|
| POST | `/api/v1/invoice-titles` | Enregistrement d'un intitulé (title_type: personal/company ; company doit avoir tax_no ; même intitulé pour le même utilisateur 422 ; la première entrée devient par défaut) |
| GET | `/api/v1/invoice-titles` | Liste des intitulés (par défaut en tête) |
| PUT | `/api/v1/invoice-titles/{id}` | Modification d'un intitulé (personnel uniquement) |
| DELETE | `/api/v1/invoice-titles/{id}` | Suppression d'un intitulé (personnel uniquement ; suppression du défaut → désignation automatique de la première entrée) |
| POST | `/api/v1/invoice-titles/{id}/default` | Passage en défaut (mise à zéro des autres lignes du même utilisateur dans une transaction) |

**Liaison à la demande** : POST /api/v1/invoices accepte title_id facultatif — l'intitulé est analysé et porté automatiquement vers invoice_title/tax_no/title_type, sans title_id le chemin de saisie manuelle est conservé.

### 14. Interfaces d'historique de navigation (authentification JWT requise, tour 21)

| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/browse-history` | Prestations récemment consultées (join nom/couverture/prix/prix d'origine de la prestation, trié par viewed_at décroissant, per_page défaut 15 max 50) |
| DELETE | `/api/v1/browse-history/{item_id}` | Suppression d'une entrée (personnelle uniquement, invalide/autrui 404) |
| DELETE | `/api/v1/browse-history` | Vidage de l'historique (personnel uniquement) |
...
| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/full-reduction-activities` | Liste des opérations de remise en vigueur (status=1 et dans la période de validité, triées par montant de réduction décroissant ; interface publique) |
...
| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/order/ics` | Export des commandes valides des 90 derniers jours (pending/paid/confirmed/serving) en iCal (RFC5545) |
...
| Méthode | Chemin | Description |
|------|------|------|
| POST | `/api/v1/technician/attendance/check-in` | Pointage d'arrivée (doublon du jour 422, index unique en secours contre la concurrence ; >10:00 marqué en retard) |
| POST | `/api/v1/technician/attendance/check-out` | Pointage de départ (non arrivé/déjà parti 422, verrou de ligne anti-concurrence) |
| GET | `/api/v1/technician/attendance` | Liste des pointages du mois + synthèse jours de présence/temps total/temps moyen (?month=YYYY-MM, invalide 422) |

### 18. Interfaces de conformité vie privée (authentification JWT requise, tour 22)

| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/privacy/data` | Export de données (groupes JSON personal/orders/points/wallet_txns/reviews/addresses/invoices ; les journaux serveur n'enregistrent que le téléphone masqué + le nombre) |
| POST | `/api/v1/privacy/close-request` | Demande de suppression de compte (solde non nul / commandes non terminées / tickets ouverts 422 ; définit close_status=1 + close_requested_at) |
| POST | `/api/v1/privacy/close-cancel` | Annulation de la demande de suppression (close_status 1→0) |
| POST | `/api/v1/privacy/close-confirm` | Confirmation de suppression (72 h requises ; close_status=2 + close_at + anonymisation phone/nickname en user{id} + status=0) |
...
| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/health-profile` | Consultation de mon dossier de santé (objet vide sans dossier) |
| PUT | `/api/v1/health-profile` | Création/mise à jour (upsert, un seul dossier par personne ; allergies/health_notes max 500 caractères, preferred_technician_id avec validation d'existence ; seuls les champs fournis sont mis à jour, réponse encodée en hashids) |
| DELETE | `/api/v1/health-profile` | Suppression de mon dossier (personnel uniquement) |
...
| Méthode | Chemin | Description |
|------|------|------|
| POST | `/api/v1/wallet/pay-password/set` | Définition du mot de passe de paiement (6 chiffres `\d{6}` ; déjà défini : ancien mot de passe requis, sinon 422) |
| POST | `/api/v1/wallet/pay-password/verify` | Vérification du mot de passe de paiement (retourne un booléen correct/incorrect, non enregistré) |
| POST | `/api/v1/wallet/pay-password/check` | Vérification de l'état (set: true/false) |
...
| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/order/{id}/timeline` | Chronologie des changements de statut de commande (ordre décroissant ; personnelle uniquement, commande d'autrui 404 sans fuite d'existence) |
...
| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/wheel/prizes` | Liste des prix de la roue (champs sensibles weight/stock masqués) |
| POST | `/api/v1/wheel/spin` | Un tirage (Redis NX + verrou de ligne anti-concurrence ; tirage pondéré random_int ; points→flux earn avec date d'expiration, solde→crédit lockForUpdate, bon→émission manuelle pending, rien→lose ; client_token idempotent) |
| GET | `/api/v1/wheel/records` | Mes enregistrements de tirage (pagés) |

### 23. Interfaces du mode invité (tour 24)

| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/guest/home` | Agrégation d'accueil (bannières/annonces/catégories de prestations/prestations populaires, cache Redis svc:guest:home 300s) |
| GET | `/api/v1/guest/services` | Liste des prestations (?category_id=hashid&sort=newest|sales|price&page/per_page≤50) |
| GET | `/api/v1/guest/services/{id}` | Détail d'une prestation (inexistante 404) |
| GET | `/api/v1/guest/stores` | Liste des boutiques |
| GET | `/api/v1/guest/technicians` | Liste des techniciens (uniquement approuvés ; ?service_id=hashid filtrage ; note décroissante) |

Entrée de navigation non connectée, sans authentification (interfaces publiques).

### 24. Interfaces de flash (authentification JWT requise, tour 24)

| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/seckill` | Liste des activités flash (status=1 et dans la fenêtre de temps ; contient la quantité vendue = nombre de commandes appointment_order.seckill_id, stock restant) |
| GET | `/api/v1/seckill/{id}` | Détail d'une activité (state=not_started/ongoing/ended) |
| POST | `/api/v1/seckill/{id}/buy` | Commande flash (client_token idempotent + Redis NX 30s anti-concurrence + validation d'activité ; plus de pré-déduction du stock) |

**Règles de commande (depuis 2026-08-26)** : le stock est déduit de manière unifiée par verrou de ligne dans la transaction `/api/v1/order store()`, buy ne fait que la validation d'entrée/l'idempotence ; prix flash = seckill_price (déterminé par la base), pas de cumul de bons/points/cartes de membre ; l'annulation de commande ne restitue pas le stock ; appeler directement `/api/v1/order` avec seckill_id déduit également le stock.

### 25. Interfaces de vérification de version APP (tour 24)

| Méthode | Chemin | Description |
|------|------|------|
| GET | `/api/v1/app/version?platform=android|ios` | Vérification de la dernière version (platform invalide 422 ; sans version, objet vide ; interface publique) |
...
En-têtes : `Authorization: Bearer <admin_token>` ; la version des interfaces d'authentification publiques suit le préfixe d'URL `/api/v1`
...
**Évaluation automatique** : TierRatingService::evaluate calcule en temps réel (nombre de commandes appointment_order completed + note moyenne des évaluations, arrondie à 1 décimale) et réécrit profile.order_count/rating, appariement selon appointment_technician_tier_config (min_orders/min_rating) du plus haut au plus bas, sans correspondance → niveau le plus bas. Uniquement promotion, pas de déclassement (le déclassement affecte le taux de commission et le coefficient de prix, géré manuellement en back-office ; allowDowngrade=true pour la réévaluation manuelle) ; idempotent (niveau identique → simple synchronisation des statistiques) ; les changements sont enregistrés dans appointment_technician_tier_log + notification interne. Points de déclenchement : WorkController::complete / écriture d'évaluation ReviewController / lecture paresseuse du profil ProfileController.
...
ID de permission : 394. Logique serveur : appointment_system_config groupe=profit_sharing (enabled/receiver_ratio) ; non activé → dégradé disabled avec simple journalisation ; activé → demande automatique de partage après paiement réussi (montant = paiement réel×receiver_ratio défaut 0.7, même commande pending/success sautée idempotente) ; sans justificatifs, pas d'HTTP, la structure de la demande est journalisée.
...
ID de permission : 412-414. Règle de récompense : 2e consommation (commande terminée) du même utilisateur chez le même technicien dans les 30 jours → prime = paiement réel × ratio (défaut 0.05), enregistrée dans appointment_technician_earnings (type=return_customer, status=pending), réglée avec la chaîne de règlement des commissions ; idempotent, pas de double versement pour la même commande.
...
ID de permission : 407-411, 420. Quantité vendue = nombre de commandes appointment_order.seckill_id ; déduction du stock sous verrou de ligne, blocage à l'épuisement.
...
ID de permission : 416-419. L'interface de vérification de mise à jour /api/v1/app/version prend la version la plus récente (updated_at/id maximal) parmi status=1.
