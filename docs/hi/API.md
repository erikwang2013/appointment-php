...
- **संस्करण नियंत्रण**: संस्करण URL पाथ प्रीफ़िक्स `/api/v1` में निहित है (जैसे `POST /api/v1/auth/login`); URL में संस्करण प्रीफ़िक्स न होने पर 404
...
**`POST /api/v1/captcha/send`** — SMS वेरिफिकेशन कोड भेजें
...
**`POST /api/v1/auth/register`** — मोबाइल नंबर से पंजीकरण
...
**`POST /api/v1/auth/login`** — पासवर्ड से लॉगिन
...
**`POST /api/v1/auth/login-by-code`** — वेरिफिकेशन कोड से लॉगिन
...
**`POST /api/v1/auth/forget-password`** — पासवर्ड भूलना
...
**`POST /api/v1/auth/refresh`** — Token रिफ्रेश करें
...
**`POST /api/v1/wechat/mini-login`** — मिनी प्रोग्राम लॉगिन

अनुरोध: `{"code":"वीचैट लॉगिन code"}`
विवरण: पहली बार लॉगिन के बाद `/api/v1/wechat/phone` कॉल कर मोबाइल नंबर बाइंड करना आवश्यक है।

---

**`POST /api/v1/wechat/phone`** — मोबाइल नंबर बाइंड करें

अनुरोध: `{"code":"वीचैट मोबाइल नंबर घटक code"}`

---

**`POST /api/v1/wechat/oa-login`** — ऑफिशियल अकाउंट लॉगिन
...
**`GET /api/v1/common/config`** — सार्वजनिक कॉन्फ़िगरेशन

प्रतिक्रिया: समझौता टेक्स्ट (उपयोगकर्ता समझौता/गोपनीयता समझौता/सेवा समझौता), हमारे बारे में जानकारी, संस्करण संख्या शामिल है।

---

**`GET /api/v1/common/area`** — शहर/क्षेत्र सूची

---

#### 1.5 सेवा खोज

**`GET /api/v1/service/categories`** — श्रेणी सूची

पैरामीटर: `?parent_id=0`

---

**`GET /api/v1/service/items`** — सेवा आइटम सूची

पैरामीटर: `?category_id=&page=1&per_page=10&sort=sales`

---

**`GET /api/v1/service/detail/{id}`** — सेवा विवरण

प्रतिक्रिया में शामिल: छवियाँ/नाम/कीमत/विनिर्देश/अवधि/बिक्री/समीक्षा सूची।

---

**`GET /api/v1/service/products`** — उत्पाद सूची

**`GET /api/v1/service/stores`** — स्टोर सूची
...
**`GET /api/v1/technician/list`** — तकनीशियन सूची
...
**`GET /api/v1/technician/detail/{id}`** — तकनीशियन विवरण

प्रतिक्रिया में शामिल: छवियाँ/नाम/परिचय/रेटिंग/दूरी/सेवा आइटम सूची/समीक्षाएँ।

---

**`GET /api/v1/technician/schedule/{id}`** — तकनीशियन शेड्यूल
...
**`GET /api/v1/content/banners`** — कैरोसेल

पैरामीटर: `?position=home`

**`GET /api/v1/content/articles`** — घोषणा/लेख सूची

पैरामीटर: `?type=announcement&page=1`

**`GET /api/v1/content/article/{id}`** — लेख विवरण

---

#### 1.8 LBS

**`GET /api/v1/lbs/nearby-stores`** — आस-पास के स्टोर

पैरामीटर: `?lat=&lng=&radius=5000`

**`GET /api/v1/lbs/geocode`** — रिवर्स जियोकोडिंग
...
| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/user/profile` | व्यक्तिगत जानकारी प्राप्त करें |
| PUT | `/api/v1/user/profile` | निकनेम/अवतार/लिंग अपडेट करें |
| POST | `/api/v1/user/change-password` | पासवर्ड बदलें (old_password/new_password/confirm_password) |
| POST | `/api/v1/user/change-phone` | मोबाइल बदलें (old_code/new_phone/new_code) |
| POST | `/api/v1/user/cancel-account` | खाता हटाएँ (पासवर्ड सत्यापन आवश्यक) |
| POST | `/api/v1/user/logout` | लॉगआउट (token ब्लैकलिस्ट में जाता है) |
| POST | `/api/v1/user/switch-role` | भूमिका स्विच करें (role: customer/technician) |
...
| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/user/addresses` | पता सूची |
| POST | `/api/v1/user/addresses` | नया पता (contact_name/contact_phone/province/city/district/detail/lat/lng/is_default) |
| GET | `/api/v1/user/addresses/{id}` | पता विवरण |
| PUT | `/api/v1/user/addresses/{id}` | पता अपडेट करें |
| DELETE | `/api/v1/user/addresses/{id}` | पता हटाएँ |
...
| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/user/favorites` | पसंदीदा सूची (?type=service/technician) |
| POST | `/api/v1/user/favorites` | पसंदीदा जोड़ें (target_type/target_id) |
| DELETE | `/api/v1/user/favorites/{id}` | पसंदीदा रद्द करें |

#### 2.4 फ़ीडबैक

`POST /api/v1/user/feedback` — फ़ीडबैक सबमिट करें (content + images ऐरे)

#### 2.5 प्रमोशन/रेफरल

| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/user/referral` | प्रमोशन जानकारी (रेफरल कोड/रेफरल संख्या/पहले ऑर्डर वाले संख्या/प्राप्त पॉइंट्स) |
| GET | `/api/v1/user/referral/qrcode` | प्रमोशन QR कोड (रेफरल कोड + आमंत्रण लिंक) |
| GET | `/api/v1/user/referral/referred-users` | रेफर किए गए उपयोगकर्ताओं की सूची |
| GET | `/api/v1/user/referral/earnings` | डिस्ट्रीब्यूशन कमीशन विवरण (पेजिंग: रेफर किए गए व्यक्ति का निकनेम/अवतार/ऑर्डर नंबर/राशि/जारी करने का समय) |

**डिस्ट्रीब्यूशन कमीशन**: रेफर किए गए व्यक्ति के पहले ऑर्डर के completed होने पर जारी, राशि = paid_amount × reward_rate (appointment_system_config referral.reward_rate, डिफ़ॉल्ट 0.05, अवैध मान पर कॉन्स्टेंट पर वापसी)। रो-लॉक + rewarded_at रिक्त जाँच + पहले ऑर्डर की पुनर्जाँच तीन-स्तरीय आइडेम्पोटेंसी; WalletTxn type=referral_reward में जमा।

#### 2.6 पॉइंट्स ट्रांसफर (राउंड 19)

| विधि | पथ | विवरण |
|------|------|------|
| POST | `/api/v1/user/points/transfer` | पॉइंट्स ट्रांसफर (to_user_id hashid/points) |
| GET | `/api/v1/user/points/transfers` | ट्रांसफर रिकॉर्ड (?direction=sent/received&page=1) |
...
| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/technician/profile` | तकनीशियन प्रोफ़ाइल प्राप्त करें |
| PUT | `/api/v1/technician/profile` | प्रोफ़ाइल अपडेट करें (avatar/intro/real_name/gender/id_card/id_card_front/id_card_back) |
...
| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/technician/schedule` | शेड्यूल क्वेरी (?start_date=&end_date=) |
| PUT | `/api/v1/technician/schedule` | शेड्यूल सेट करें (date/time_slots/status), समय स्लॉट ओवरलैप 422「मौजूदा शेड्यूल समय से टकराव」 |
| POST | `/api/v1/technician/schedule/batch` | बैच शेड्यूल (राउंड 23): दिनांक सीमा ≤7 दिन + weekdays फ़िल्टर, मौजूदा शेड्यूल वाले दिन छोड़े जाते हैं, प्रतिक्रिया created/skipped |

#### 3.3 तकनीशियन ऑर्डर

`GET /api/v1/technician/orders` — ऑर्डर सूची (?status=&page=1)

#### 3.4 आय

`GET /api/v1/technician/earnings` — आय अवलोकन (today_income/pending_settlement/balance + लेन-देन सूची)

#### 3.5 विड्रॉल

`POST /api/v1/technician/withdraw` — विड्रॉल आवेदन (amount)
...
`POST /api/v1/technician/review/reply/{order_id}` — तकनीशियन समीक्षा उत्तर (reply)। समीक्षा मौजूद नहीं/स्वयं की नहीं → एकीकृत 404 (अस्तित्व लीक नहीं होता); मौजूदा उत्तर 422 (आइडेम्पोटेंट अस्वीकृति, ओवरराइट नहीं); खाली उत्तर 422। उत्तर सफल होने पर उपयोगकर्ता को इन-ऐप नोटिफिकेशन (type='review_reply')।

#### 3.7 वर्कबेंच

| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/technician/work/today` | आज के कार्य सूची |
| GET | `/api/v1/technician/work/records` | पूर्ण रिकॉर्ड पेजिंग |
| POST | `/api/v1/technician/work/{id}/start` | सेवा शुरू करें |
| POST | `/api/v1/technician/work/{id}/complete` | सेवा पूर्ण करें |
...
| विधि | पथ | विवरण |
|------|------|------|
| POST | `/api/v1/order` | ऑर्डर बनाएँ (order_type/items/store_id/technician_id/service_time/coupon_id/user_coupon_id/promotion_id/remark) |
| GET | `/api/v1/order/list` | ऑर्डर सूची (?status=&page=1) |
| GET | `/api/v1/order/detail/{id}` | ऑर्डर विवरण |
| POST | `/api/v1/order/cancel/{id}` | ऑर्डर रद्द करें (reason) |
| POST | `/api/v1/order/pay/{id}` | भुगतान शुरू करें (pay_channel: wechat/balance, use_points: वैकल्पिक पॉइंट्स कैश) |
| POST | `/api/v1/order/refund/{id}` | रिफंड आवेदन |
| POST | `/api/v1/order/verify/{id}` | वेरिफिकेशन (code: QR कोड मान) |
| POST | `/api/v1/order/reschedule/{id}` | अपॉइंटमेंट रीशेड्यूल (new_service_time अनिवार्य/reason वैकल्पिक) |
| GET | `/api/v1/order/logistics/{id}` | लॉजिस्टिक्स ट्रैकिंग (राउंड 19, product ऑर्डर) |
| POST | `/api/v1/order/review/{order_id}` | समीक्षा सबमिट करें (rating 1-5/content/images) (राउंड 19 पंजीकरण) |
| POST | `/api/v1/order/review/{order_id}/append` | समीक्षा फॉलो-अप (content/images अल्पविराम से अलग) (राउंड 19) |
...
**कीमत टैम्पर-रोधी (2026-08-26)**: ऑर्डर आइटम राशि हमेशा डेटाबेस रिकॉर्ड के अनुसार (target_type=service appointment_service में, product appointment_product में), क्लाइंट द्वारा भेजी गई कीमत गणना में भाग नहीं लेती; अज्ञात target_type 422; target_id hashid एन्कोडेड मान होना चाहिए (raw id भेजने पर डिकोड 0 → 422「उत्पाद मौजूद नहीं या अनलिस्टेड」); ग्रुप बाय/सेकिल कीमत भी DB पर आधारित।
...
**ग्रुप बाय ऑर्डर (राउंड 16)**: ऑर्डर बनाते समय वैकल्पिक `promotion_id` (hashid) भेजें। जाँच: केवल group_buy प्रकार, गतिविधि वैध अवधि में, कॉलर भागीदार है, पूर्ण नहीं (पूर्ण हुआ लॉक 422), ऑर्डर सेवा गतिविधि से मेल खाती है; ग्रुप बाय कीमत = मूल कीमत × discount_percent/100, कूपन/सेशन कार्ड/पॉइंट्स स्टैकिंग निषिद्ध (कोई भी भेजने पर 422)। ऑर्डर में promotion_id/participant_id लिखा जाता है; भुगतान पूरी तरह `POST /api/v1/order/pay/{id}` दोहराता है, pay पर लेज़ी निर्धारण कि गतिविधि बंद हो गई (समय समाप्त पर पूर्ण नहीं) → ऑर्डर स्वतः रद्द और तकनीशियन लॉक रिलीज़।

**सेकिल ऑर्डर (राउंड 18, बंद)**: ~~ऑर्डर बनाते समय `promotion_id` (flash_sale प्रकार) भेजें~~ — 2026-08 से पुराना प्रमोशन FLASH_SALE चैनल हटा दिया गया, store() प्रमोशन शाखा में केवल ग्रुप बाय GROUP_BUY बचा (गैर-ग्रुप बाय promotion 422); सेकिल एकीकृत रूप से राउंड 24 के `/api/v1/seckill` चैनल से होता है (seckill_id store ट्रांज़ैक्शन में रो-लॉक इन्वेंट्री कटौती में इंजेक्ट होता है), PromotionController::index flash_sale फ़िल्टर करता है, show/join उस पर 400 लौटाता है, `Promotion::TYPE_FLASH_SALE` कॉन्स्टेंट ऐतिहासिक डेटा संगतता के लिए रिटेन किया गया है।

**अपॉइंटमेंट रीशेड्यूल (राउंड 17)**: `POST /api/v1/order/reschedule/{id}` पर new_service_time (अनिवार्य) + reason (वैकल्पिक) भेजें, उसी तकनीशियन के साथ समय बदलें। नियम: केवल स्वयं का ऑर्डर (स्वयं का नहीं 404), केवल appointment प्रकार और स्थिति pending/paid/confirmed में बदला जा सकता है (बाकी 422), मूल सेवा शुरुआत से ≥ 6 घंटे (पूर्ण रिफंड विंडो के समान) पर ही बदला जा सकता है। समवर्ती सुरक्षा: B1 order_lock (pay/cancel/refund के साथ एक ही म्यूटेक्स परिवार) → नए समय स्लॉट के लिए तकनीशियन लॉक Redis SETNX EX 180 (समवर्ती रीशेड्यूल पर ओवरसेलिंग रोकता है) → ट्रांज़ैक्शन में रो-लॉक पुनर्पठन + B2 शेड्यूल टकराव DB जाँच (यह ऑर्डर छोड़कर) → service_time अपडेट + appointment_order_reschedule रिकॉर्ड → मूल स्लॉट लॉक रिलीज़, नया स्लॉट लॉक इस ऑर्डर के पास → SCENE_RESCHEDULE सब्सक्रिप्शन मैसेज (कॉन्फ़िगर न होने पर इन-ऐप नोटिफिकेशन डिग्रेड)। विफल पथ पर ट्रांज़ैक्शन रोलबैक के साथ नया स्लॉट लॉक भी रिलीज़।

**लॉजिस्टिक्स ट्रैकिंग (राउंड 19)**: `GET /api/v1/order/logistics/{id}` — केवल स्वयं का product ऑर्डर देख सकता है (स्वयं का नहीं/उत्पाद नहीं/शिप नहीं हुआ एकीकृत 404)। order.remark JSON पढ़ता है (shipping_company/tracking_no/shipped_at, admin MallOrderController::ship() शिप करते समय लिखता है), parseShippingInfo/parseReceiver दोहरी पार्सिंग पुराने फ़ॉर्मैट के लिए फॉलबैक; प्राप्तकर्ता मोबाइल नंबर मास्क 138****5678।

**समीक्षा (राउंड 19)**: `POST /api/v1/order/review/{order_id}` समीक्षा सबमिट (rating अनिवार्य 1-5, content/images वैकल्पिक): स्वयं का नहीं 404, non-completed 422, दोहरी समीक्षा 400। `POST /api/v1/order/review/{order_id}/append` फॉलो-अप (content अनिवार्य, images अल्पविराम से अलग): समीक्षा मौजूद नहीं/स्वयं की नहीं एकीकृत 404, non-completed 422, दोहरा फॉलो-अप 422, खाली सामग्री 422; सफल होने पर append_content/append_images(JSON)/append_at लिखता है और तकनीशियन को इन-ऐप नोटिफिकेशन type='review_append', प्रतिक्रिया में append फ़ील्ड दिखती है।

### 4.1 आफ्टर-सेल इंटरफ़ेस (JWT प्रमाणीकरण आवश्यक)

| विधि | पथ | विवरण |
|------|------|------|
| POST | `/api/v1/aftersales` | आफ्टर-सेल आवेदन (order_id hashid/type: refund|exchange/reason), स्वयं के ऑर्डर की जाँच 404, स्थिति paid+completed पर ही आवेदन 422, उसी ऑर्डर की चालू आफ्टर-सेल डेडुप 422 |
| GET | `/api/v1/aftersales` | मेरे आफ्टर-सेल सूची (?status=&page=1&limit=) |
| GET | `/api/v1/aftersales/{id}` | आफ्टर-सेल विवरण (स्वामित्व जाँच 404) |

**आफ्टर-सेल स्थिति**: pending(ऑडिट शेष) → approved(पास) / rejected(अस्वीकृत)। approved केवल स्थिति परिवर्तन है, रिफंड क्रिया `POST /api/v1/order/refund/{id}` उपयोग करती है।
...
| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/promotions` | गतिविधि सूची (?type=group_buy; flash_sale फ़िल्टर होकर वापस नहीं आती) |
| GET | `/api/v1/promotions/{id}` | गतिविधि विवरण (भागीदारी संख्या/पूर्ण हुई या नहीं सहित; flash_sale प्रकार 400) |
| GET | `/api/v1/promotions/{id}/participants` | भागीदार सूची |
| POST | `/api/v1/promotions/join/{id}` | गतिविधि में भाग लें (राउंड 15 पूर्णता: प्रतिक्रिया में discount_percent/original_price/group_price; flash_sale प्रकार 400) |
...
| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/marketing/coupons` | कूपन सूची (?status=available/used/expired) |
| POST | `/api/v1/marketing/coupons/receive` | कूपन प्राप्त करें (coupon_id) |
| GET | `/api/v1/marketing/cards` | सदस्यता कार्ड सूची |
| POST | `/api/v1/marketing/cards/buy` | सदस्यता कार्ड खरीदें (card_id) |
| GET | `/api/v1/marketing/cards/my` | मेरे सेशन कार्ड सूची |
| POST | `/api/v1/marketing/cards/use` | सेशन कार्ड वेरिफाई करें (user_card_id/service_id/remark?) |
| GET | `/api/v1/marketing/gift-cards` | गिफ्ट कार्ड सूची |
| GET | `/api/v1/marketing/gift-cards/my` | मेरे गिफ्ट कार्ड (redeem रिकॉर्ड) |
| POST | `/api/v1/marketing/gift-cards/redeem` | गिफ्ट कार्ड रिडीम करें (cash प्रकार रिडीम पर वॉलेट बैलेंस में जमा) |
| GET | `/api/v1/marketing/points` | पॉइंट्स लेन-देन (?type=earn/use/expire&source=order/referral/gift_card/check_in/admin) |
| GET | `/api/v1/marketing/points-exchange` | पॉइंट्स एक्सचेंज उत्पाद सूची (लिस्टेड + वास्तविक समय शेष स्टॉक + एक्सचेंज की गई संख्या) |
| POST | `/api/v1/marketing/points-exchange/{id}` | एक्सचेंज (type=coupon कूपन जारी / wallet जमा / gift_card कार्ड-पिन वापसी) |
| POST | `/api/v1/marketing/coupons/transfer` | ट्रांसफर कोड बनाएँ (user_coupon_id: 8-अंकीय यूनिक कोड/7 दिन वैध) |
| POST | `/api/v1/marketing/coupons/claim` | ट्रांसफर कूपन प्राप्त करें (code) |
| GET | `/api/v1/marketing/coupons/transfers` | ट्रांसफर रिकॉर्ड (भेजा गया pending/claimed/expired + प्राप्त claimed) |
...
**पॉइंट्स एक्सपायरी (राउंड 17)**: appointment_user_points.expires_at कॉलम (कॉन्फ़िग points.expiry_days, डिफ़ॉल्ट 365 दिन, ≤0 कभी एक्सपायर नहीं), सभी earn लिखते समय वैधता अवधि भरें; PointsExpiryTimer टाइमर प्रोसेस हर 60s कर्सर स्कैन एक्सपायर्ड earn पंक्तियाँ, type=expire नकारात्मक मान कटौती पंक्ति लिखता है (source=expiry + order_id मूल लेन-देन ट्रेस, तीन-स्तरीय आइडेम्पोटेंसी) + एग्रीगेट इन-ऐप नोटिफिकेशन「आपके X पॉइंट्स एक्सपायर हुए」; उपलब्ध बैलेंस SUM गणना में expire नकारात्मक पंक्तियाँ शामिल हैं, एक्सपायर्ड पॉइंट्स कैश/एक्सचेंज में उपयोग नहीं हो सकते।
...
| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/notification` | नोटिफिकेशन सूची (?type=order/system&page=1) |
| PUT | `/api/v1/notification/read/{id}` | पढ़ा हुआ चिह्नित करें |
| PUT | `/api/v1/notification/read-all` | सभी पढ़े हुए |
...
| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/wallet` | वॉलेट बैलेंस + लेन-देन पेजिंग |
| POST | `/api/v1/wallet/recharge` | रिचार्ज ऑर्डर बनाएँ (amount: युआन) |
| POST | `/api/v1/wallet/recharge/{id}/pay` | रिचार्ज ऑर्डर भुगतान शुरू करें (वीचैट) |
| POST | `/api/v1/wallet/transfer` | बैलेंस ट्रांसफर (to_user_id hashid/amount/remark वैकल्पिक/client_token वैकल्पिक) (राउंड 19) |
| GET | `/api/v1/wallet/transfers` | ट्रांसफर रिकॉर्ड (?direction=out/in&page=1) (राउंड 19) |
| GET | `/api/v1/wallet/transfers/{id}` | ट्रांसफर विवरण (केवल दोनों पक्ष देख सकते हैं, अन्य 404) (राउंड 19) |
...
**रिचार्ज**: `POST /api/v1/wallet/recharge` पर amount (युआन) भेजकर रिचार्ज ऑर्डर बनाएँ, रिचार्ज ऑर्डर hashid लौटाता है। `POST /api/v1/wallet/recharge/{id}/pay` वीचैट भुगतान शुरू करता है, प्रतिक्रिया में sign_params (ऑर्डर भुगतान मोड के समान); भुगतान कॉलबैक R उपसर्ग वाले out_trade_no से रिचार्ज ऑर्डर और ऑर्डर अलग करता है।
...
| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/store-manager/overview` | आज का अवलोकन (आज के ऑर्डर संख्या/आज की आय/चालू/तकनीशियन संख्या/वेरिफिकेशन संख्या) |
| GET | `/api/v1/store-manager/orders` | स्टोर ऑर्डर सूची (?status=&page=&limit=) |
| GET | `/api/v1/store-manager/technicians` | तकनीशियन सूची (आज का शेड्यूल सहित) |
| GET | `/api/v1/store-manager/revenue` | हाल के 7 दिनों की आय एग्रीगेशन |

**store_id अलगाव**: requireStoreId() वर्तमान उपयोगकर्ता को स्टोर से बंधने के लिए बाध्य करता है (appointment_user.store_id), स्टोर नहीं 403; सभी क्वेरी store_id से फ़िल्टर होती हैं।
...
| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/growth` | वर्तमान ग्रोथ अवलोकन (balance/स्तर/अगली स्लॉट अंतर/स्तर नाम) |
| GET | `/api/v1/growth/records` | ग्रोथ वैल्यू लेन-देन पेजिंग (?page=&limit=) |
| GET | `/api/v1/growth/levels` | स्लॉट सूची (सार्वजनिक, लॉगिन आवश्यक नहीं) |
...
| विधि | पथ | विवरण |
|------|------|------|
| POST | `/api/v1/invoices` | इनवॉइस आवेदन (order_id hashid/order_type: service=सेवा/points_exchange=पॉइंट्स एक्सचेंज/order_type डिफ़ॉल्ट service; राशि और शीर्षक सर्वर-पक्ष से, टैम्पर नहीं किया जा सकता) |
| GET | `/api/v1/invoices` | इनवॉइस सूची (?status=&page=) |
| GET | `/api/v1/invoices/{id}` | इनवॉइस विवरण (केवल स्वयं) |
...
| विधि | पथ | विवरण |
|------|------|------|
| POST | `/api/v1/tickets` | टिकट सबमिट करें (title/content अनिवार्य) |
| GET | `/api/v1/tickets` | टिकट सूची (?status=open/closed&page=) |
| GET | `/api/v1/tickets/{id}` | टिकट विवरण (केवल स्वयं, अन्य 404) |
| POST | `/api/v1/tickets/{id}/close` | टिकट बंद करें (केवल स्वयं/केवल open; वैकल्पिक rating 1-5 संतुष्टि स्कोर, सीमा से बाहर/गैर-पूर्णांक 422, न दिए जाने पर NULL संगत) |

### 12. अपॉइंटमेंट कैलेंडर इंटरफ़ेस (JWT प्रमाणीकरण आवश्यक, राउंड 20)

| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/calendar/technician/{id}` | माह दृश्य (?month=YYYY-MM): शेड्यूल time_slots घंटे स्लॉट में विस्तार + बुक किए गए बाहर |
| GET | `/api/v1/calendar/technician/{id}/day` | दिन दृश्य (?date=YYYY-MM-DD): उस दिन बुक करने योग्य/बुक किया/बुक नहीं कर सकने वाले स्लॉट विवरण |

### 13. इनवॉइस शीर्षक इंटरफ़ेस (JWT प्रमाणीकरण आवश्यक, राउंड 21)

| विधि | पथ | विवरण |
|------|------|------|
| POST | `/api/v1/invoice-titles` | शीर्षक सहेजें (title_type: personal/company; company पर tax_no अनिवार्य; एक ही उपयोगकर्ता एक ही शीर्षक दोहरा 422; पहला स्वतः डिफ़ॉल्ट) |
| GET | `/api/v1/invoice-titles` | शीर्षक सूची (डिफ़ॉल्ट शीर्ष पर) |
| PUT | `/api/v1/invoice-titles/{id}` | शीर्षक संपादित करें (केवल स्वयं) |
| DELETE | `/api/v1/invoice-titles/{id}` | शीर्षक हटाएँ (केवल स्वयं; डिफ़ॉल्ट हटाने पर सबसे पुराना स्वतः चुना जाता है) |
| POST | `/api/v1/invoice-titles/{id}/default` | डिफ़ॉल्ट सेट करें (ट्रांज़ैक्शन में उसी उपयोगकर्ता की अन्य पंक्तियाँ शून्य) |

**आवेदन एकीकरण**: POST /api/v1/invoices पर वैकल्पिक title_id — शीर्षक पार्स कर invoice_title/tax_no/title_type स्वतः लाता है, title_id न होने पर मूल मैनुअल-भरण पथ बना रहता है।

### 14. ब्राउज़िंग हिस्ट्री इंटरफ़ेस (JWT प्रमाणीकरण आवश्यक, राउंड 21)

| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/browse-history` | हाल की ब्राउज़ की गई सेवाएँ (join सेवा नाम/कवर/कीमत/मूल कीमत, viewed_at उल्टे क्रम, per_page डिफ़ॉल्ट 15 अधिकतम 50) |
| DELETE | `/api/v1/browse-history/{item_id}` | एक पंक्ति हटाएँ (केवल स्वयं, अवैध/अन्य 404) |
| DELETE | `/api/v1/browse-history` | हिस्ट्री खाली करें (केवल स्वयं) |
...
| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/full-reduction-activities` | चालू फुल-रिडक्शन गतिविधि सूची (status=1 और समय वैध अवधि में, रिडक्शन राशि के अनुसार उल्टे क्रम; सार्वजनिक इंटरफ़ेस) |
...
| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/order/ics` | 90 दिनों के भीतर के वैध ऑर्डर (pending/paid/confirmed/serving) को iCal (RFC5545) में निर्यात करें |
...
| विधि | पथ | विवरण |
|------|------|------|
| POST | `/api/v1/technician/attendance/check-in` | काम पर चेक-इन (उसी दिन दोहरा 422, यूनिक इंडेक्स समवर्ती फॉलबैक; >10:00 पर देर से चिह्नित) |
| POST | `/api/v1/technician/attendance/check-out` | काम से चेक-आउट (बिना चेक-इन/पहले चेक-आउट 422, रो-लॉक समवर्ती) |
| GET | `/api/v1/technician/attendance` | उस माह की उपस्थिति सूची + उपस्थिति दिन/कुल घंटे/औसत घंटे सारांश (?month=YYYY-MM, अवैध 422) |

### 18. गोपनीयता अनुपालन इंटरफ़ेस (JWT प्रमाणीकरण आवश्यक, राउंड 22)

| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/privacy/data` | डेटा निर्यात (personal/orders/points/wallet_txns/reviews/addresses/invoices समूह JSON; सर्वर लॉग केवल मास्क मोबाइल + गिनती) |
| POST | `/api/v1/privacy/close-request` | खाता हटाने का आवेदन (बैलेंस 0 नहीं / अधूरे ऑर्डर / चालू टिकट 422; close_status=1 + close_requested_at सेट) |
| POST | `/api/v1/privacy/close-cancel` | हटाने का आवेदन रद्द करें (close_status 1→0) |
| POST | `/api/v1/privacy/close-confirm` | हटाने की पुष्टि (72h पूर्ण होने पर ही; close_status=2 + close_at + phone/nickname user{id} में अनाम + status=0) |
...
| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/health-profile` | मेरी स्वास्थ्य प्रोफ़ाइल क्वेरी (कोई प्रोफ़ाइल न होने पर खाली ऑब्जेक्ट) |
| PUT | `/api/v1/health-profile` | बनाएँ/अपडेट करें (upsert, एक व्यक्ति एक प्रोफ़ाइल; allergies/health_notes अधिकतम 500 अक्षर, preferred_technician_id अस्तित्व जाँच; केवल दिए गए फ़ील्ड अपडेट, प्रतिक्रिया hashid एन्कोडेड) |
| DELETE | `/api/v1/health-profile` | मेरी प्रोफ़ाइल हटाएँ (केवल स्वयं) |
...
| विधि | पथ | विवरण |
|------|------|------|
| POST | `/api/v1/wallet/pay-password/set` | भुगतान पासवर्ड सेट करें (6-अंकीय `\d{6}`; पहले से सेट होने पर पुराना पासवर्ड आवश्यक 422 रोक) |
| POST | `/api/v1/wallet/pay-password/verify` | भुगतान पासवर्ड जाँचें (सही/गलत बूलियन लौटाता है, सहेजा नहीं जाता) |
| POST | `/api/v1/wallet/pay-password/check` | सेट हुआ या नहीं क्वेरी (set: true/false) |
...
| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/order/{id}/timeline` | ऑर्डर स्टेटस परिवर्तन टाइमलाइन (उल्टे क्रम; केवल स्वयं, अन्य का ऑर्डर 404 अस्तित्व लीक नहीं) |
...
| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/wheel/prizes` | व्हील पुरस्कार सूची (weight/stock संवेदनशील फ़ील्ड छिपे) |
| POST | `/api/v1/wheel/spin` | एक बार ड्रॉ (Redis NX + रो-लॉक समवर्ती रोक; random_int वेटेड ड्रॉ; पॉइंट्स→earn लेन-देन एक्सपायरी समय सहित, बैलेंस→lockForUpdate जमा, कूपन→pending मैनुअल जारी, कोई पुरस्कार नहीं→lose; client_token आइडेम्पोटेंट) |
| GET | `/api/v1/wheel/records` | मेरे ड्रॉ रिकॉर्ड (पेजिंग) |

### 23. गेस्ट मोड इंटरफ़ेस (राउंड 24)

| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/guest/home` | होमपेज एग्रीगेशन (कैरोसेल/घोषणा/सेवा श्रेणियाँ/लोकप्रिय सेवाएँ, Redis कैश svc:guest:home 300s) |
| GET | `/api/v1/guest/services` | सेवा सूची (?category_id=hashid&sort=newest|sales|price&page/per_page≤50) |
| GET | `/api/v1/guest/services/{id}` | सेवा विवरण (मौजूद नहीं 404) |
| GET | `/api/v1/guest/stores` | स्टोर सूची |
| GET | `/api/v1/guest/technicians` | तकनीशियन सूची (केवल ऑडिट पास; ?service_id=hashid फ़िल्टर; रेटिंग उल्टे क्रम) |

बिना प्रमाणीकरण (सार्वजनिक इंटरफ़ेस) के लॉगिन-रहित ब्राउज़िंग प्रवेश द्वार।

### 24. सेकिल इंटरफ़ेस (JWT प्रमाणीकरण आवश्यक, राउंड 24)

| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/seckill` | सेकिल गतिविधि सूची (status=1 और समय विंडो में; बिकी मात्रा = appointment_order.seckill_id ऑर्डर संख्या, शेष स्टॉक सहित) |
| GET | `/api/v1/seckill/{id}` | गतिविधि विवरण (state=not_started/ongoing/ended) |
| POST | `/api/v1/seckill/{id}/buy` | सेकिल ऑर्डर (client_token आइडेम्पोटेंट + Redis NX 30s समवर्ती रोक + गतिविधि जाँच; अब स्टॉक पूर्व-कटौती नहीं) |

**ऑर्डर नियम (2026-08-26 से)**: स्टॉक एकीकृत रूप से `/api/v1/order store()` ट्रांज़ैक्शन में रो-लॉक कटौती, buy केवल प्रवेश जाँच/आइडेम्पोटेंसी करता है; सेकिल कीमत = seckill_price (DB के अनुसार), कूपन/पॉइंट्स/सदस्यता कार्ड स्टैक नहीं; ऑर्डर रद्द पर स्टॉक वापस नहीं; सीधे `/api/v1/order` पर seckill_id भेजने पर भी स्टॉक कटता है।

### 25. APP संस्करण जाँच इंटरफ़ेस (राउंड 24)

| विधि | पथ | विवरण |
|------|------|------|
| GET | `/api/v1/app/version?platform=android|ios` | नवीनतम संस्करण जाँच (platform अवैध 422; कोई संस्करण न होने पर खाली ऑब्जेक्ट; सार्वजनिक इंटरफ़ेस) |

प्रतिक्रिया: id/platform/version_code/version_name/force_update (1=अनिवार्य)/changelog/download_url।

### 26. गैर-संस्करणित इंटरफ़ेस (बिना /api/v1 प्रीफ़िक्स)

| विधि | पथ | विवरण |
|------|------|------|
| GET | `/` | ब्रांड लैंडिंग पेज (HTML; ब्राउज़र सीधे API पोर्ट पर पहुँचने पर दिखाया जाता है, API दस्तावेज़ और स्वास्थ्य जाँच के लिंक सहित) |
| GET | `/health` | स्वास्थ्य जाँच (JSON; docker-compose healthcheck / लोड बैलेंसर लाइवनेस प्रोब द्वारा उपयोग) |
| GET | `/api/docs` | बिज़नेस API पूर्ण विनिर्देश (OpenAPI 3.0 JSON) |
| POST | `/payment/wechat-notify` | वीचैट भुगतान कॉलबैक (हस्ताक्षर सत्यापन + आइडेम्पोटेंसी) |

> क्लाइंट बिज़नेस इंटरफ़ेस हमेशा संस्करण प्रीफ़िक्स `/api/v1/...` के साथ आते हैं; बिना प्रीफ़िक्स वाले नंगे `/api/*` पथ 404 लौटाते हैं।

---

## 二、प्रबंधन बैकएंड API (admin/ :8787)

अनुरोध हेडर: `Authorization: Bearer <admin_token>`; सार्वजनिक प्रमाणीकरण इंटरफ़ेस का संस्करण URL प्रीफ़िक्स `/api/v1` के साथ आता है
...
**स्वचालित मूल्यांकन**: TierRatingService::evaluate वास्तविक समय आँकड़े (appointment_order completed ऑर्डर संख्या + समीक्षा औसत, 1 दशमलव स्थान राउंड) profile.order_count/rating में वापस लिखता है, appointment_technician_tier_config (min_orders/min_rating) के अनुसार उच्च से निम्न मिलान, कोई मिलान न होने पर निम्नतम स्तर। केवल अपग्रेड, डाउनग्रेड नहीं (डाउनग्रेड कमीशन दर और कीमत गुणांक को प्रभावित करता है, बैकएंड मैनुअल फॉलबैक; allowDowngrade=true मैनुअल पुनर्मूल्यांकन के लिए); आइडेम्पोटेंट (स्तर समान होने पर केवल आँकड़े सिंक); परिवर्तन appointment_technician_tier_log + इन-ऐप नोटिफिकेशन। ट्रिगर बिंदु: WorkController::complete / ReviewController समीक्षा लिखना / ProfileController प्रोफ़ाइल देखते समय लेज़ी निर्धारण।
...
अनुमति ID: 394। सर्वर-पक्ष लॉजिक: appointment_system_config group=profit_sharing (enabled/receiver_ratio); अक्षम होने पर disabled डिग्रेड केवल लॉग; सक्षम होने पर भुगतान सफलता पर स्वतः प्रॉफिट शेयरिंग अनुरोध (राशि=वास्तविक भुगतान×receiver_ratio डिफ़ॉल्ट 0.7, एक ही ऑर्डर pending/success आइडेम्पोटेंट स्किप); क्रेडेंशियल न होने पर HTTP निष्पादित नहीं, अनुरोध संरचना लॉग।
...
अनुमति ID: 412-414। इनाम नियम: उपयोगकर्ता एक ही तकनीशियन के साथ 30 दिनों में दूसरी बार उपभोग (ऑर्डर पूर्ण) पर बोनस = वास्तविक भुगतान × ratio (डिफ़ॉल्ट 0.05), appointment_technician_earnings में लिखा (type=return_customer, status=pending) कमीशन सेटलमेंट श्रृंखला के साथ एकीकृत सेटल; एक ही ऑर्डर आइडेम्पोटेंट, दोहरा जारी नहीं।
...
अनुमति ID: 407-411、420। बिकी मात्रा = appointment_order.seckill_id ऑर्डर संख्या; स्टॉक रो-लॉक कटौती, सोल्ड-आउट रोक।
...
अनुमति ID: 416-419। अपडेट जाँच इंटरफ़ेस /api/v1/app/version status=1 में सबसे नया (updated_at/id सबसे बड़ा) संस्करण लेता है।
