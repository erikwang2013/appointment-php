...
| Manajemen member | Kolom erik_user.member_level (migrasi 000008); CRUD lengkap kartu member sisi admin (izin 365-369) |
...
| Ubah jadwal janji temu | POST /api/order/reschedule/{id} ganti waktu teknisi yang sama; hanya pending/paid/confirmed dan jarak dari mulai ≥6h bisa ubah; order_lock + kunci teknisi slot baru SETNX(180s) cegah oversell + validasi konflik jadwal B2; catat erik_order_reschedule + pesan langganan SCENE_RESCHEDULE |
...
| Balasan ulasan teknisi | POST /api/technician/review/reply/{order_id}: bukan milik 404, balasan ganda 422, balasan sukses notifikasi internal ke pengguna; erik_order_review tambah replied_at; detail balasan admin (izin 381) |
...
| Ulasan tambahan | POST /api/order/review/{order_id}/append: bukan milik 404/duplikat 422/konten kosong 422/bukan completed 422, sukses tulis notifikasi internal teknisi type='review_append'; erik_order_review tambah append_content/append_images(JSON)/append_at; sekalian daftarkan rute submit ulasan pengguna (store asli tak punya rute tak terjangkau) dan perbaiki TypeError latennya |
...
| Preferensi notifikasi | Tabel erik_user_notify_setting (kunci unik uk_user_type, baris default=nyala); GET/PUT /api/user/notify-settings; 5 saklar service_reminder/card_expiry/points_expiry/marketing/system (system selalu nyala tak bisa mati); notifySettingEnabled gerbang 3 timer + event langganan, mati maka notifikasi internal dan pesan langganan dilewati |
| Kalender janji temu | GET /api/calendar/technician/{id} (tampilan bulan) + /day (tampilan hari): perluas time_slots JSON ke slot jam, kecualikan slot terpesan erik_order; visualisasi jadwal toko pilih waktu |
| Level pertumbuhan pengguna | erik_user_growth + erik_growth_level (Perunggu0/Perak100/Emas500/Platinum2000/Berlian5000); absen +10, ulasan +20, konsumsi 1 yuan 1 poin (reuse verifikasi ulang status yang ada, idempoten alami); GET /api/growth (ikhtisar/records/levels daftar publik) |
...
| Manajemen data faktur | Pustaka erik_invoice_title: simpan/edit/hapus/default (pertama otomatis default, hapus default otomatis pindah, set default transaksi nolkan); ajukan faktur opsional title_id, jalur isi manual dipertahankan |
...
| Riwayat penelusuran | erik_browse_history (uk_user_item penelusuran berulang hanya perbarui viewed_at): detail layanan catat (try/catch tak blokir alur utama, tak login dilewati); daftar join info layanan + hashid; hapus satu/bersihkan hanya milik sendiri |
...
>
> Tambahan Round-16: tukar poin (PointsExchangeController, tipe consume/source=exchange); pemesanan belanja bersama (erik_order tambah kolom promotion_id/participant_id); komisi referral (ReferralRewardService sambung WorkController::complete).
>
> Tambahan Round-17: ubah jadwal janji temu (erik_order_reschedule + antarmuka reschedule); transfer hadiah kupon (erik_user_coupon_transfer + transfer/claim/transfers); kedaluwarsa poin (expires_at + proses PointsExpiryTimer); penilaian otomatis level teknisi (TierRatingService + erik_technician_tier_log, izin 380).
>
...
>
> Tambahan Round-19: transfer saldo (erik_wallet_transfer + WalletTransferController, dua row lock dalam izin + idempoten client_token); transfer hadiah poin (erik_user_points_transfer + PointsTransferController, limit harian + dua transaksi); ulasan tambahan (erik_order_review tambah tiga kolom + antarmuka append + daftarkan rute store); pelacakan logistik sisi pengguna (antarmuka logistics + parse JSON remark + samarkan nomor HP); preferensi notifikasi (erik_user_notify_setting + NotifySettingController + gerbang 3 timer).
>
> Tambahan Round-20: kalender janji temu (CalendarController tampilan bulan/hari + kecualikan terpesan); level pertumbuhan pengguna (erik_user_growth + erik_growth_level 5 tingkat + sambung absen/ulasan/konsumsi); faktur elektronik (erik_invoice + uk_order_type cegah ganda + terbit/tolak backend, izin 382-384); tiket layanan pelanggan (erik_ticket ajukan/daftar/detail/tutup + balas backend, izin 385/387); komisi referral tingkat dua (payLevel2Reward row lock transaksi + uk_order_referred idempoten, izin 386).
>
> Tambahan Round-21: hak level pertumbuhan (diskon discount_rate pemesanan + pengali poin points_multiplier bayar, migrasi seed 5 tingkat benefits); manajemen data faktur (pustaka erik_invoice_title + tautan title_id pengajuan); kepuasan tiket (rating/rated_at skor tutup + statistik ringkasan admin, izin 388); audit gambar ulasan (ReviewAuditController sembunyikan/pulihkan, izin 389-391); riwayat penelusuran pengguna (erik_browse_history + sambung detail + daftar/hapus/bersihkan).
>
> Tambahan Round-22: aktivitas potongan (erik_full_reduction potongan otomatis + validasi ambang, izin 396-400); ekspor kalender ICS (RFC5545 janji temu saya); absen teknisi (erik_technician_attendance masuk/pulang + tanda terlambat + statistik admin, izin 392-393); layanan push APP (abstraksi digerakkan konfigurasi + 5 sambungan event, erik_push_log); pembagian keuntungan resmi WeChat (erik_profit_sharing digerakkan konfigurasi + degradasi, izin 394); kepatuhan privasi (ekspor data + penghapusan akun state machine 72h close_status).
>
> Tambahan Round-23: profil kesehatan pengguna (erik_user_health_profile); kata sandi pembayaran dompet (erik_user_wallet pay_password set/verifikasi); penjadwalan massal teknisi (impor batch + deteksi konflik tumpang tindih); linimasa status pesanan (erik_order_status_log 8 titik embed status + tampilan pengguna/backend); roda keberuntungan poin (erik_lucky_wheel + erik_wheel_record undian berbobot, izin 401-406); masa berlaku poin (konfigurasi points.expiry_days + transaksi earn baru berisi expires_at).
>
> Tambahan Round-24: mode tamu (/api/guest/* penelusuran baca-saja tanpa login + cache Redis); flash sale (erik_seckill_activity + Redis NX row lock pembelian + suntik erik_order.seckill_id pemesanan, izin 407-411/420); manajemen versi APP & deteksi pembaruan (erik_app_version + /api/app/version, izin 416-419); hadiah pelanggan setia (bonus pembelian kedua 30 hari type=return_customer, izin 412-414); ekspor CSV jadwal (UTF-8 BOM + detail slot waktu, izin 415).
>
