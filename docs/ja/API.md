# API 説明ドキュメント
> **Languages**: [中文](../API.md) · [English](../en/API.md) · [한국어](../ko/API.md) · [Русский](../ru/API.md) · [Deutsch](../de/API.md) · [Français](../fr/API.md) · [Español](../es/API.md) · [Português](../pt/API.md) · [हिन्दी](../hi/API.md) · [العربية](../ar/API.md) · [বাংলা](../bn/API.md) · [Bahasa Indonesia](../id/API.md)
...
- **バージョン管理**: バージョンは URL パスプレフィックス `/api/v1` に固定（例: `POST /api/v1/auth/login`）、URL にバージョンプレフィックスがない場合は 404
...
**`POST /api/v1/captcha/send`** — 短信認証コードを送信
...
**`POST /api/v1/auth/register`** — 携帯番号登録
...
**`POST /api/v1/auth/login-by-code`** — 認証コードログイン
...
**`POST /api/v1/auth/forget-password`** — パスワード忘れ
...
**`POST /api/v1/auth/refresh`** — Tokenリフレッシュ
...
**`POST /api/v1/wechat/mini-login`** — 小程序ログイン

リクエスト: `{"code":"微信登录code"}`
説明: 初回ログイン後、後続で `/api/v1/wechat/phone` を呼び出して携帯番号を紐付ける必要があります。

---

**`POST /api/v1/wechat/phone`** — 携帯番号の紐付け

リクエスト: `{"code":"微信手机号组件code"}`

---

**`POST /api/v1/wechat/oa-login`** — 公式アカウントログイン
...
**`GET /api/v1/common/config`** — 共通設定

レスポンス: 規約テキスト（ユーザー規約/プライバシー規約/サービス規約）、会社概要情報、バージョン番号を含む。

---

**`GET /api/v1/common/area`** — 都市・エリアリスト

---

#### 1.5 サービス照会

**`GET /api/v1/service/categories`** — 分類リスト

パラメータ: `?parent_id=0`

---

**`GET /api/v1/service/items`** — サービス項目リスト

パラメータ: `?category_id=&page=1&per_page=10&sort=sales`

---

**`GET /api/v1/service/detail/{id}`** — サービス詳細

レスポンスには: 画像/名称/価格/規格/所要時間/売上/評価リスト を含む。

---

**`GET /api/v1/service/products`** — 商品リスト

**`GET /api/v1/service/stores`** — 店舗リスト
...
**`GET /api/v1/technician/list`** — スタッフリスト
...
**`GET /api/v1/technician/detail/{id}`** — スタッフ詳細

レスポンスには: 画像/名前/紹介/評価/距離/提供可能なサービス項目リスト/評価 を含む。

---

**`GET /api/v1/technician/schedule/{id}`** — スタッフ排班
...
**`GET /api/v1/content/banners`** — カルーセル画像

パラメータ: `?position=home`

**`GET /api/v1/content/articles`** — お知らせ/記事リスト

パラメータ: `?type=announcement&page=1`

**`GET /api/v1/content/article/{id}`** — 記事詳細

---

#### 1.8 LBS

**`GET /api/v1/lbs/nearby-stores`** — 近くの店舗

パラメータ: `?lat=&lng=&radius=5000`

**`GET /api/v1/lbs/geocode`** — 逆ジオコーディング
...
| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/user/profile` | 個人情報の取得 |
| PUT | `/api/v1/user/profile` | ニックネーム/アバター/性別の更新 |
| POST | `/api/v1/user/change-password` | パスワード変更 (old_password/new_password/confirm_password) |
| POST | `/api/v1/user/change-phone` | 携帯番号変更 (old_code/new_phone/new_code) |
| POST | `/api/v1/user/cancel-account` | アカウント削除 (パスワード検証が必要) |
| POST | `/api/v1/user/logout` | ログアウト (tokenをブラックリストへ) |
| POST | `/api/v1/user/switch-role` | 身分切替 (role: customer/technician) |
...
| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/user/addresses` | 住所リスト |
| POST | `/api/v1/user/addresses` | 住所追加 (contact_name/contact_phone/province/city/district/detail/lat/lng/is_default) |
| GET | `/api/v1/user/addresses/{id}` | 住所詳細 |
| PUT | `/api/v1/user/addresses/{id}` | 住所更新 |
| DELETE | `/api/v1/user/addresses/{id}` | 住所削除 |
...
| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/user/favorites` | お気に入りリスト (?type=service/technician) |
| POST | `/api/v1/user/favorites` | お気に入り追加 (target_type/target_id) |
| DELETE | `/api/v1/user/favorites/{id}` | お気に入り解除 |

#### 2.4 意見フィードバック

`POST /api/v1/user/feedback` — フィードバック送信 (content + images配列)

#### 2.5 紹介・推薦

| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/user/referral` | 紹介情報 (紹介コード/紹介人数/初回注文人数/獲得ポイント) |
| GET | `/api/v1/user/referral/qrcode` | 紹介QRコード (紹介コード+招待リンク) |
| GET | `/api/v1/user/referral/referred-users` | 紹介済みユーザーリスト |
| GET | `/api/v1/user/referral/earnings` | 分销返金明細 (ページネーション: 被紹介者ニックネーム/アバター/注文番号/金額/発行時間) |
...
| メソッド | パス | 説明 |
|------|------|------|
| POST | `/api/v1/user/points/transfer` | ポイント転贈 (to_user_id hashid/points) |
| GET | `/api/v1/user/points/transfers` | 転贈記録 (?direction=sent/received&page=1) |
...
| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/user/notify-settings` | 通知スイッチの照会（5 種全量） |
| PUT | `/api/v1/user/notify-settings` | スイッチの一括更新 (types: {service_reminder: 0/1, ...}) |
...
| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/technician/profile` | スタッフカルテの取得 |
| PUT | `/api/v1/technician/profile` | カルテ更新 (avatar/intro/real_name/gender/id_card/id_card_front/id_card_back) |
...
| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/technician/schedule` | 排班照会 (?start_date=&end_date=) |
| PUT | `/api/v1/technician/schedule` | 排班設定 (date/time_slots/status)、時間帯が重複すると 422「与已有排班时间冲突」 |
| POST | `/api/v1/technician/schedule/batch` | 一括排班（第23ラウンド）：日付範囲 ≤7 日 + weekdays フィルタ、既存排班のある日はスキップ、レスポンスは created/skipped |

#### 3.3 スタッフ注文

`GET /api/v1/technician/orders` — 注文リスト (?status=&page=1)

#### 3.4 収益

`GET /api/v1/technician/earnings` — 収益概要 (today_income/pending_settlement/balance + 流水リスト)

#### 3.5 出金

`POST /api/v1/technician/withdraw` — 出金申請 (amount)
...
`POST /api/v1/technician/review/reply/{order_id}` — スタッフが評価に返信 (reply)。評価が存在しない/本人以外は統一 404（存在性を漏らさない）；返信済み 422（冪等拒否で上書きしない）；空返信 422。返信成功で站内通知（type='review_reply'）。

#### 3.6 ワークベンチ

| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/technician/work/today` | 今日のタスクリスト |
| GET | `/api/v1/technician/work/records` | 完了記録のページネーション |
| POST | `/api/v1/technician/work/{id}/start` | サービス開始 |
| POST | `/api/v1/technician/work/{id}/complete` | サービス完了 |
...
| メソッド | パス | 説明 |
|------|------|------|
| POST | `/api/v1/order` | 注文作成 (order_type/items/store_id/technician_id/service_time/coupon_id/user_coupon_id/promotion_id/remark) |
| GET | `/api/v1/order/list` | 注文リスト (?status=&page=1) |
| GET | `/api/v1/order/detail/{id}` | 注文詳細 |
| POST | `/api/v1/order/cancel/{id}` | 注文キャンセル (reason) |
| POST | `/api/v1/order/pay/{id}` | 支払い開始 (pay_channel: wechat/balance, use_points: 任意のポイント充当) |
| POST | `/api/v1/order/refund/{id}` | 返金申請 |
| POST | `/api/v1/order/verify/{id}` | 核销 (code: QRコード値) |
| POST | `/api/v1/order/reschedule/{id}` | 予約改期 (new_service_time 必須/reason 任意) |
| GET | `/api/v1/order/logistics/{id}` | 物流追跡（第19ラウンド、product 注文） |
| POST | `/api/v1/order/review/{order_id}` | 評価送信 (rating 1-5/content/images)（第19ラウンドで登録補完） |
| POST | `/api/v1/order/review/{order_id}/append` | 評価追記 (content/images カンマ区切り)（第19ラウンド） |
...
**拼团注文（第16ラウンド）**: 注文作成時に任意で `promotion_id`（hashid）を渡す。検証：group_buy タイプのみ、キャンペーン有効期間内、呼び出し元が参加者、未満員（成团済みは 422）、注文サービスとキャンペーンの一致；拼团価格 = 原価 × discount_percent/100、クーポン/回数券/ポイントの併用不可（いずれかを渡すと 422）。注文に promotion_id/participant_id を保存；支払いは完全に `POST /api/v1/order/pay/{id}` を再利用、pay 時にキャンペーン終了（期限到来で未成团）を遅延判定 → 注文を自動キャンセルしスタッフロックを解放。

**秒殺注文（第18ラウンド、廃止）**: ~~注文作成時に `promotion_id`（flash_sale タイプ）を渡す~~ —— 2026-08 より旧プロモーション FLASH_SALE チャネルは削除、store() のプロモーション分岐は拼团 GROUP_BUY のみ（非拼团 promotion 422）；秒殺は統一して第24ラウンドの `/api/v1/seckill` チャネルへ（seckill_id を store トランザクション内の行ロックで在庫引き落とし）、PromotionController::index は flash_sale をフィルタ、show/join は 400 を返す、`Promotion::TYPE_FLASH_SALE` 定数は履歴データ互換のため保持。

**予約改期（第17ラウンド）**: `POST /api/v1/order/reschedule/{id}` に new_service_time（必須）+ reason（任意）を渡し、同じスタッフで時間変更。ルール：本人の注文のみ（本人以外 404）、appointment タイプかつ状態 pending/paid/confirmed のみ変更可（その他 422）、元のサービス開始まで ≥ 6 時間（全額返金ウィンドウと一致）でのみ改期可。並行防御：B1 order_lock（pay/cancel/refund と同一の相互排他ファミリー）→ 新時間帯のスタッフロック Redis SETNX EX 180（並行改期での過剰販売防止）→ トランザクション内の行ロック再読み取り + B2 排班衝突の DB 検証（本注文を除外）→ service_time 更新 + appointment_order_reschedule 記録 → 元時間帯ロックを解放、新時間帯ロックは本注文が保持 → SCENE_RESCHEDULE 購読メッセージ（未設定時は站内通知にフォールバック）。失敗時はトランザクションロールバックと同時に新時間帯ロックを解放。

**物流追跡（第19ラウンド）**: `GET /api/v1/order/logistics/{id}` — 本人の product 注文のみ照会可（本人以外/非商品/未発送は統一 404）。order.remark JSON を読み取り（shipping_company/tracking_no/shipped_at、admin MallOrderController::ship() が発送時に書き込み）、parseShippingInfo/parseReceiver の二重パースで旧形式を救済；受取人の携帯番号はマスキング 138****5678。

**評価（第19ラウンド）**: `POST /api/v1/order/review/{order_id}` で評価送信（rating 必須 1-5、content/images 任意）：本人以外 404、非 completed 422、重複評価 400。`POST /api/v1/order/review/{order_id}/append` で追記（content 必須、images カンマ区切り）：評価が存在しない/本人以外は統一 404、非 completed 422、重複追記 422、空内容 422；成功時は append_content/append_images(JSON)/append_at を書き込み、スタッフへ站内通知 type='review_append'、レスポンスに append フィールドを透過。

### 4.1 アフターサービスインターフェース（JWT認証必要）

| メソッド | パス | 説明 |
|------|------|------|
| POST | `/api/v1/aftersales` | アフターサービス申請 (order_id hashid/type: refund|exchange/reason)、本人注文検証 404、状態 paid+completed のみ申請可 422、同一注文の進行中アフターサービス重複 422 |
| GET | `/api/v1/aftersales` | 自分のアフターサービスリスト (?status=&page=1&limit=) |
| GET | `/api/v1/aftersales/{id}` | アフターサービス詳細（帰属検証 404） |

**アフターサービス状態**: pending(審査待ち) → approved(承認) / rejected(却下)。approved は状態遷移のみで、返金アクションは `POST /api/v1/order/refund/{id}` を沿用。
...
| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/promotions` | キャンペーンリスト (?type=group_buy；flash_sale はフィルタされ返らない) |
| GET | `/api/v1/promotions/{id}` | キャンペーン詳細（参加人数/成团可否を含む；flash_sale タイプは 400） |
| GET | `/api/v1/promotions/{id}/participants` | 参加リスト |
| POST | `/api/v1/promotions/join/{id}` | キャンペーン参加（第15ラウンドで拡充：レスポンスに discount_percent/original_price/group_price；flash_sale タイプは 400） |
...
| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/marketing/coupons` | クーポンリスト (?status=available/used/expired) |
| POST | `/api/v1/marketing/coupons/receive` | クーポン受け取り (coupon_id) |
| GET | `/api/v1/marketing/cards` | 会員カードリスト |
| POST | `/api/v1/marketing/cards/buy` | 会員カード購入 (card_id) |
| GET | `/api/v1/marketing/cards/my` | 自分の回数券リスト |
| POST | `/api/v1/marketing/cards/use` | 回数券核销 (user_card_id/service_id/remark?) |
| GET | `/api/v1/marketing/gift-cards` | ギフトカードリスト |
| GET | `/api/v1/marketing/gift-cards/my` | 自分のギフトカード (redeem記録) |
| POST | `/api/v1/marketing/gift-cards/redeem` | ギフトカード交換 (cashタイプは交換後にウォレット残高へ入金) |
| GET | `/api/v1/marketing/points` | ポイント流水 (?type=earn/use/expire&source=order/referral/gift_card/check_in/admin) |
| GET | `/api/v1/marketing/points-exchange` | ポイント交換商品リスト（上架中 + リアルタイム残り在庫 + 交換済み数） |
| POST | `/api/v1/marketing/points-exchange/{id}` | 交換 (type=coupon はクーポン発行 / wallet は入帳 / gift_card はカードシークレット返却) |
| POST | `/api/v1/marketing/coupons/transfer` | 転贈コード生成 (user_coupon_id: 8桁ユニークコード/7日有効) |
| POST | `/api/v1/marketing/coupons/claim` | 転贈クーポン受け取り (code) |
| GET | `/api/v1/marketing/coupons/transfers` | 転贈記録 (発行 pending/claimed/expired + 受領 claimed) |
...
| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/notification` | 通知リスト (?type=order/system&page=1) |
| PUT | `/api/v1/notification/read/{id}` | 既読マーク |
| PUT | `/api/v1/notification/read-all` | 全件既読 |
...
| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/wallet` | ウォレット残高 + 流水のページネーション |
| POST | `/api/v1/wallet/recharge` | チャージオーダー作成 (amount: 元) |
| POST | `/api/v1/wallet/recharge/{id}/pay` | チャージオーダーの支払い開始 (微信) |
| POST | `/api/v1/wallet/transfer` | 残高振込 (to_user_id hashid/amount/remark 任意/client_token 任意)（第19ラウンド） |
| GET | `/api/v1/wallet/transfers` | 振込記録 (?direction=out/in&page=1)（第19ラウンド） |
| GET | `/api/v1/wallet/transfers/{id}` | 振込詳細（双方のみ閲覧可、他人 404）（第19ラウンド） |
...
**チャージ**: `POST /api/v1/wallet/recharge` に amount（元）を渡してチャージオーダーを作成、チャージオーダーの hashid を返す。`POST /api/v1/wallet/recharge/{id}/pay` で微信支払いを開始、レスポンスに sign_params を含む（注文支払いモードと同じ）；支払いコールバックは R プレフィックスの out_trade_no でチャージオーダーと注文を区別。

**残高支払い**: 注文支払いリクエストボディに `pay_channel: "balance"` を渡すとウォレット残高を使用；微信返金と残高返金の両方で金額をウォレット残高に回充。

**残高振込（第19ラウンド）**: `POST /api/v1/wallet/transfer` — 受取人の hashid デコード+存在性 404、自分への振込 422、金額 0.01-1000/回 422（DECIMAL 比較で float 禁止）、残高不足 422、単日累計 5000 元 422。並行/冪等：Redis NX ロック wallet_transfer:{from} 30s で転出側を直列化 → トランザクション内で双方のウォレット行を user_id 昇順で lockForUpdate（固定順序でデッドロック防止）→ 転出側を引き落とし + 受取側を増額 + WalletTxn 二重流水（transfer_out/transfer_in は balance_after スナップショット含む）+ 振込記録 completed + 受取側へ站内通知 type='balance_received'（失敗時はログ記録のみ）。client_token 任意：成功後に SETNX 24h で重複送信防止（失敗リクエストは token を記録しないためリトライ可）。
...
| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/store-manager/overview` | 今日の概要 (今日の注文数/今日の売上/進行中/スタッフ数/核销数) |
| GET | `/api/v1/store-manager/orders` | 店舗注文リスト (?status=&page=&limit=) |
| GET | `/api/v1/store-manager/technicians` | スタッフリスト（今日の排班含む） |
| GET | `/api/v1/store-manager/revenue` | 直近 7 日の売上集計 |
...
| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/growth` | 現在の成長概要（balance/レベル/次段階までの差額/レベル名） |
| GET | `/api/v1/growth/records` | 成長値流水のページネーション (?page=&limit=) |
| GET | `/api/v1/growth/levels` | 段階リスト（公開、ログイン不要） |
...
| メソッド | パス | 説明 |
|------|------|------|
| POST | `/api/v1/invoices` | インボイス申請 (order_id hashid/order_type: service=サービス/points_exchange=ポイント交換/order_type デフォルト service；金額と宛名はサーバー側で導出、改ざん不可) |
| GET | `/api/v1/invoices` | インボイスリスト (?status=&page=) |
| GET | `/api/v1/invoices/{id}` | インボイス詳細（本人のみ） |
...
| メソッド | パス | 説明 |
|------|------|------|
| POST | `/api/v1/tickets` | チケット送信 (title/content 必須) |
| GET | `/api/v1/tickets` | チケットリスト (?status=open/closed&page=) |
| GET | `/api/v1/tickets/{id}` | チケット詳細（本人のみ、他人 404） |
| POST | `/api/v1/tickets/{id}/close` | チケットクローズ（本人のみ/open のみ；任意の rating 1-5 満足度、範囲外/非整数 422、未指定は NULL で互換） |

### 12. 予約月カレンダーインターフェース（JWT認証必要、第20ラウンド）

| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/calendar/technician/{id}` | 月ビュー (?month=YYYY-MM)：排班 time_slots を時間スロットに展開 + 予約済みを除外 |
| GET | `/api/v1/calendar/technician/{id}/day` | 日ビュー (?date=YYYY-MM-DD)：当日の予約可/予約済み/予約不可スロット明細 |

### 13. インボイス宛名インターフェース（JWT認証必要、第21ラウンド）

| メソッド | パス | 説明 |
|------|------|------|
| POST | `/api/v1/invoice-titles` | 宛名保存 (title_type: personal/company；company は tax_no 必須；同一ユーザーの同一宛名重複 422；先頭は自動でデフォルト) |
| GET | `/api/v1/invoice-titles` | 宛名リスト（デフォルトを先頭に） |
| PUT | `/api/v1/invoice-titles/{id}` | 宛名編集（本人のみ） |
| DELETE | `/api/v1/invoice-titles/{id}` | 宛名削除（本人のみ；デフォルト削除後は最古の1件を自動指定） |
| POST | `/api/v1/invoice-titles/{id}/default` | デフォルト設定（トランザクションで同一ユーザーの他行をクリア） |

**申請連携**: POST /api/v1/invoices は任意の title_id をサポート —— 宛名を解決して invoice_title/tax_no/title_type を自動で持ち込み、title_id なしの場合は従来の手入力パスを維持。

### 14. 閲覧履歴インターフェース（JWT認証必要、第21ラウンド）

| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/browse-history` | 最近閲覧したサービス（サービス名/表紙/価格/原価を join、viewed_at 降順、per_page デフォルト 15 上限 50） |
| DELETE | `/api/v1/browse-history/{item_id}` | 単件削除（本人のみ、不正/他人 404） |
| DELETE | `/api/v1/browse-history` | 履歴クリア（本人のみ） |
...
| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/full-reduction-activities` | 有効な満減キャンペーンリスト（status=1 かつ時間が有効期間内、割引額降順；公開インターフェース） |
...
| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/order/ics` | 90 日以内の有効注文（pending/paid/confirmed/serving）を iCal（RFC5545）でエクスポート |
...
| メソッド | パス | 説明 |
|------|------|------|
| POST | `/api/v1/technician/attendance/check-in` | 出勤打刻（当日重複 422、ユニークインデックスで並行を兜底；>10:00 は遅刻マーク） |
| POST | `/api/v1/technician/attendance/check-out` | 退勤打刻（未出勤/既退勤 422、行ロックで並行処理） |
| GET | `/api/v1/technician/attendance` | 当月勤怠リスト + 出勤日数/総労働時間/平均労働時間の集計（?month=YYYY-MM、不正 422） |

### 18. プライバシーコンプライアンスインターフェース（JWT認証必要、第22ラウンド）

| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/privacy/data` | データエクスポート（personal/orders/points/wallet_txns/reviews/addresses/invoices をグループ化した JSON；サーバーログはマスキング済み携帯番号+件数のみ記録） |
| POST | `/api/v1/privacy/close-request` | アカウント削除申請（残高非 0 / 未完了注文 / 進行中チケット 422；close_status=1 + close_requested_at を設定） |
| POST | `/api/v1/privacy/close-cancel` | 削除申請のキャンセル（close_status 1→0） |
| POST | `/api/v1/privacy/close-confirm` | 削除確定（72h 経過後のみ可；close_status=2 + close_at + phone/nickname を user{id} に匿名化 + status=0） |
...
| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/health-profile` | 自分の健康カルテ照会（カルテなしは空オブジェクト） |
| PUT | `/api/v1/health-profile` | 作成/更新（upsert、一人一件；allergies/health_notes 上限 500 字、preferred_technician_id の存在性検証；渡されたフィールドのみ更新、レスポンスは hashid エンコード） |
| DELETE | `/api/v1/health-profile` | 自分のカルテ削除（本人のみ） |
...
| メソッド | パス | 説明 |
|------|------|------|
| POST | `/api/v1/wallet/pay-password/set` | 支払いパスワード設定（6桁数字 `\d{6}`；設定済みの場合は旧パスワード必須 422 で遮断） |
| POST | `/api/v1/wallet/pay-password/verify` | 支払いパスワード検証（正/誤のブール値を返す、保存しない） |
| POST | `/api/v1/wallet/pay-password/check` | 設定済みかどうかの照会（set: true/false） |
...
| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/order/{id}/timeline` | 注文状態変更タイムライン（降順；本人のみ、他人の注文は 404 で存在性を漏らさない） |
...
| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/wheel/prizes` | ルーレット賞品リスト（weight/stock の機微フィールドを非表示） |
| POST | `/api/v1/wheel/spin` | 抽選 1 回（Redis NX + 行ロックで並行防止；random_int 加重抽選；ポイント→earn 流水に失効時間含む、残高→lockForUpdate で入帳、クーポン→pending で手動発行、無賞→lose；client_token 冪等） |
| GET | `/api/v1/wheel/records` | 自分の抽選記録（ページネーション） |

### 23. ゲストモードインターフェース（第24ラウンド）

| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/guest/home` | ホーム集約（カルーセル画像/お知らせ/サービス分類/人気サービス、Redis キャッシュ svc:guest:home 300s） |
| GET | `/api/v1/guest/services` | サービスリスト（?category_id=hashid&sort=newest|sales|price&page/per_page≤50） |
| GET | `/api/v1/guest/services/{id}` | サービス詳細（存在しない 404） |
| GET | `/api/v1/guest/stores` | 店舗リスト |
| GET | `/api/v1/guest/technicians` | スタッフリスト（審査通過のみ；?service_id=hashid フィルタ；評価降順） |

認証不要（公開インターフェース）の未ログイン閲覧エントリ。

### 24. 秒殺インターフェース（JWT認証必要、第24ラウンド）

| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/seckill` | 秒殺キャンペーンリスト（status=1 かつ時間窓内；販売済み量 = appointment_order.seckill_id の注文数、残り在庫を含む） |
| GET | `/api/v1/seckill/{id}` | キャンペーン詳細（state=not_started/ongoing/ended） |
| POST | `/api/v1/seckill/{id}/buy` | 秒殺注文（client_token 冪等 + Redis NX 30s 並行防止 + キャンペーン検証；在庫は事前引き落とししない） |

**注文ルール（2026-08-26 以降）**: 在庫は `/api/v1/order store()` トランザクション内の行ロックで一律引き落とし、buy は入口検証/冪等のみ；秒殺価格 = seckill_price（DB 基準）、クーポン/ポイント/会員カードと併用不可；注文キャンセルで在庫は回補しない；`/api/v1/order` に直接 seckill_id を渡して注文しても同様に在庫を引き落とす。

### 25. APP バージョンチェックインターフェース（第24ラウンド）

| メソッド | パス | 説明 |
|------|------|------|
| GET | `/api/v1/app/version?platform=android|ios` | 最新バージョンチェック（platform 不正 422；バージョンなしは空オブジェクト；公開インターフェース） |
...
リクエストヘッダー: `Authorization: Bearer <admin_token>`；公開認証インターフェースのバージョンは URL プレフィックス `/api/v1` に従う
...
権限ID: 416-419。更新チェックインターフェース /api/v1/app/version は status=1 のうち最新（updated_at/id 最大）バージョンを取得。
