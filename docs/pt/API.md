...
- **Controlo de versões**: a versão está fixada no prefixo do caminho do URL `/api/v1` (por exemplo, `POST /api/v1/auth/login`); um URL sem esse prefixo de versão devolve 404
...
**`POST /api/v1/captcha/send`** — envia código de verificação por SMS
...
**`POST /api/v1/auth/register`** — registo com número de telefone
...
**`POST /api/v1/auth/login`** — login com palavra-passe
...
**`POST /api/v1/auth/login-by-code`** — login com código de verificação
...
**`POST /api/v1/auth/forget-password`** — palavra-passe esquecida
...
**`POST /api/v1/wechat/mini-login`** — login no miniprograma

Pedido: `{"code":"code do login WeChat"}`
Nota: no primeiro login é necessário chamar `/api/v1/wechat/phone` para vincular o número de telefone.

---

**`POST /api/v1/wechat/phone`** — vincular número de telefone

Pedido: `{"code":"code do componente de telefone WeChat"}`

---

**`POST /api/v1/wechat/oa-login`** — login na conta oficial
...
**`GET /api/v1/common/config`** — configuração comum
...
**`GET /api/v1/common/area`** — lista de cidades/regiões

---

#### 1.5 Consulta de serviços

**`GET /api/v1/service/categories`** — lista de categorias

Parâmetros: `?parent_id=0`

---

**`GET /api/v1/service/items`** — lista de projetos de serviço

Parâmetros: `?category_id=&page=1&per_page=10&sort=sales`

---

**`GET /api/v1/service/detail/{id}`** — detalhes do serviço

A resposta inclui: imagens/nome/preço/especificações/duração/volume de vendas/lista de avaliações.

---

**`GET /api/v1/service/products`** — lista de produtos

**`GET /api/v1/service/stores`** — lista de lojas
...
**`GET /api/v1/content/banners`** — carrossel

Parâmetros: `?position=home`

**`GET /api/v1/content/articles`** — lista de anúncios/artigos

Parâmetros: `?type=announcement&page=1`

**`GET /api/v1/content/article/{id}`** — detalhes do artigo

---

#### 1.8 LBS

**`GET /api/v1/lbs/nearby-stores`** — lojas próximas

Parâmetros: `?lat=&lng=&radius=5000`

**`GET /api/v1/lbs/geocode`** — geocódigo inverso
...
|------|------|------|
| GET | `/api/v1/user/profile` | Obter informações pessoais |
| PUT | `/api/v1/user/profile` | Atualizar apelido/avatar/sexo |
| POST | `/api/v1/user/change-password` | Alterar palavra-passe (old_password/new_password/confirm_password) |
| POST | `/api/v1/user/change-phone` | Alterar telefone vinculado (old_code/new_phone/new_code) |
| POST | `/api/v1/user/cancel-account` | Cancelar conta (requer verificação de palavra-passe) |
| POST | `/api/v1/user/logout` | Terminar sessão (token adicionado à lista negra) |
| POST | `/api/v1/user/switch-role` | Alternar identidade (role: customer/technician) |
...
| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/user/addresses` | Lista de endereços |
| POST | `/api/v1/user/addresses` | Novo endereço (contact_name/contact_phone/province/city/district/detail/lat/lng/is_default) |
| GET | `/api/v1/user/addresses/{id}` | Detalhes do endereço |
| PUT | `/api/v1/user/addresses/{id}` | Atualizar endereço |
| DELETE | `/api/v1/user/addresses/{id}` | Eliminar endereço |
...
| POST | `/api/v1/user/favorites` | Adicionar favorito (target_type/target_id) |
| DELETE | `/api/v1/user/favorites/{id}` | Remover favorito |

#### 2.4 Feedback de opiniões

`POST /api/v1/user/feedback` — submeter feedback (content + array images)

#### 2.5 Promoção e recomendação

| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/user/referral` | Informações de promoção (código de recomendação/n.º de recomendados/n.º de primeiros pedidos/pontos obtidos) |
| GET | `/api/v1/user/referral/qrcode` | QR code de promoção (código de recomendação + ligação de convite) |
| GET | `/api/v1/user/referral/referred-users` | Lista de utilizadores recomendados |
| GET | `/api/v1/user/referral/earnings` | Extratos de comissões de distribuição (paginado: apelido/avatar do recomendado/número do pedido/montante/hora de emissão) |
...
| Método | Caminho | Descrição |
|------|------|------|
| POST | `/api/v1/user/points/transfer` | Transferência de pontos (to_user_id hashid/points) |
| GET | `/api/v1/user/points/transfers` | Registos de transferências (?direction=sent/received&page=1) |
...
| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/user/notify-settings` | Consultar interruptores de notificações (5 tipos completos) |
| PUT | `/api/v1/user/notify-settings` | Atualização em lote dos interruptores (types: {service_reminder: 0/1, ...}) |
...
| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/technician/profile` | Obter o perfil do técnico |
| PUT | `/api/v1/technician/profile` | Atualizar o perfil (avatar/intro/real_name/gender/id_card/id_card_front/id_card_back) |
...
| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/technician/schedule` | Consulta de agendamentos (?start_date=&end_date=) |
| PUT | `/api/v1/technician/schedule` | Definir agendamento (date/time_slots/status), sobreposição de períodos 422 "Conflito de horário com agendamento existente" |
| POST | `/api/v1/technician/schedule/batch` | Agendamento em lote (ronda 23): período de datas ≤7 dias + filtro weekdays, dias com agendamento existente ignorados, resposta created/skipped |
...
`GET /api/v1/technician/earnings` — resumo de rendimentos (today_income/pending_settlement/balance + lista de movimentos)

#### 3.5 Levantamento

`POST /api/v1/technician/withdraw` — pedido de levantamento (amount)
...
`POST /api/v1/technician/review/reply/{order_id}` — resposta do técnico a avaliações (reply). Avaliação inexistente/não própria unificados 404 (não revela existência); resposta já existente 422 (rejeição idempotente sem sobrepor); resposta vazia 422. Resposta bem-sucedida envia notificação interna ao utilizador (type='review_reply').

#### 3.6 Bancada de trabalho

| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/technician/work/today` | Lista de tarefas de hoje |
| GET | `/api/v1/technician/work/records` | Registos de conclusões paginados |
| POST | `/api/v1/technician/work/{id}/start` | Iniciar serviço |
| POST | `/api/v1/technician/work/{id}/complete` | Concluir serviço |
...
| Método | Caminho | Descrição |
|------|------|------|
| POST | `/api/v1/order` | Criar pedido (order_type/items/store_id/technician_id/service_time/coupon_id/user_coupon_id/promotion_id/remark) |
| GET | `/api/v1/order/list` | Lista de pedidos (?status=&page=1) |
| GET | `/api/v1/order/detail/{id}` | Detalhes do pedido |
| POST | `/api/v1/order/cancel/{id}` | Cancelar pedido (reason) |
| POST | `/api/v1/order/pay/{id}` | Iniciar pagamento (pay_channel: wechat/balance, use_points: pontos opcionais como pagamento) |
| POST | `/api/v1/order/refund/{id}` | Pedir reembolso |
| POST | `/api/v1/order/verify/{id}` | Verificação (code: valor do QR code) |
| POST | `/api/v1/order/reschedule/{id}` | Remarcar agendamento (new_service_time obrigatório/reason opcional) |
| GET | `/api/v1/order/logistics/{id}` | Rastreio logístico (ronda 19, pedidos de produtos) |
| POST | `/api/v1/order/review/{order_id}` | Submeter avaliação (rating 1-5/content/images) (registo completado na ronda 19) |
| POST | `/api/v1/order/review/{order_id}/append` | Avaliação complementar (content/images separados por vírgulas) (ronda 19) |
...
| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/promotions` | Lista de atividades (?type=group_buy; flash_sale filtrado e não devolvido) |
| GET | `/api/v1/promotions/{id}` | Detalhes da atividade (inclui número de participantes/se grupo formado; tipo flash_sale 400) |
| GET | `/api/v1/promotions/{id}/participants` | Lista de participantes |
| POST | `/api/v1/promotions/join/{id}` | Participar na atividade (ronda 15 aperfeiçoada: resposta inclui discount_percent/original_price/group_price; tipo flash_sale 400) |
...
| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/marketing/coupons` | Lista de cupões (?status=available/used/expired) |
| POST | `/api/v1/marketing/coupons/receive` | Receber cupão (coupon_id) |
| GET | `/api/v1/marketing/cards` | Lista de cartões de membro |
| POST | `/api/v1/marketing/cards/buy` | Comprar cartão de membro (card_id) |
| GET | `/api/v1/marketing/cards/my` | Lista dos meus cartões de vezes |
| POST | `/api/v1/marketing/cards/use` | Verificar cartão de vezes (user_card_id/service_id/remark?) |
| GET | `/api/v1/marketing/gift-cards` | Lista de cartões-presente |
| GET | `/api/v1/marketing/gift-cards/my` | Os meus cartões-presente (registos redeem) |
| POST | `/api/v1/marketing/gift-cards/redeem` | Trocar cartão-presente (o tipo cash após troca recarrega o saldo da carteira) |
| GET | `/api/v1/marketing/points` | Movimentos de pontos (?type=earn/use/expire&source=order/referral/gift_card/check_in/admin) |
| GET | `/api/v1/marketing/points-exchange` | Lista de produtos de troca de pontos (em prateleira + stock restante em tempo real + quantidade trocada) |
| POST | `/api/v1/marketing/points-exchange/{id}` | Troca (type=coupon emite cupão / wallet credita saldo / gift_card devolve código) |
| POST | `/api/v1/marketing/coupons/transfer` | Gerar código de oferta (user_coupon_id: código único de 8 caracteres/válido 7 dias) |
| POST | `/api/v1/marketing/coupons/claim` | Receber cupão ofertado (code) |
| GET | `/api/v1/marketing/coupons/transfers` | Registos de ofertas (emitidas pending/claimed/expired + recebidas claimed) |
...
| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/notification` | Lista de notificações (?type=order/system&page=1) |
| PUT | `/api/v1/notification/read/{id}` | Marcar como lida |
| PUT | `/api/v1/notification/read-all` | Marcar tudo como lido |
...
| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/wallet` | Saldo da carteira + movimentos paginados |
| POST | `/api/v1/wallet/recharge` | Criar ordem de recarga (amount: RMB) |
| POST | `/api/v1/wallet/recharge/{id}/pay` | Iniciar pagamento da ordem de recarga (WeChat) |
| POST | `/api/v1/wallet/transfer` | Transferência de saldo (to_user_id hashid/amount/remark opcional/client_token opcional) (ronda 19) |
| GET | `/api/v1/wallet/transfers` | Registos de transferências (?direction=out/in&page=1) (ronda 19) |
| GET | `/api/v1/wallet/transfers/{id}` | Detalhes da transferência (visível apenas para ambas as partes, outro 404) (ronda 19) |
...
**Recarga**: `POST /api/v1/wallet/recharge` envia amount (RMB) para criar a ordem de recarga, devolve o hashid da ordem. `POST /api/v1/wallet/recharge/{id}/pay` inicia o pagamento WeChat, a resposta inclui sign_params (mesmo padrão do pagamento de pedidos); o callback de pagamento distingue ordens de recarga de pedidos pelo out_trade_no com prefixo R.
...
| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/store-manager/overview` | Resumo de hoje (n.º de pedidos de hoje/rendimento de hoje/em curso/n.º de técnicos/n.º de verificações) |
| GET | `/api/v1/store-manager/orders` | Lista de pedidos da loja (?status=&page=&limit=) |
| GET | `/api/v1/store-manager/technicians` | Lista de técnicos (inclui agendamentos de hoje) |
| GET | `/api/v1/store-manager/revenue` | Agregação de rendimentos dos últimos 7 dias |
...
| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/growth` | Resumo de crescimento atual (balance/nível/diferença para o próximo escalão/nome do nível) |
| GET | `/api/v1/growth/records` | Movimentos de crescimento paginados (?page=&limit=) |
| GET | `/api/v1/growth/levels` | Lista de escalões (pública, sem login) |
...
| Método | Caminho | Descrição |
|------|------|------|
| POST | `/api/v1/invoices` | Pedir fatura (order_id hashid/order_type: service=serviço/points_exchange=troca de pontos/order_type padrão service; montante e título trazidos pelo servidor, não alteráveis) |
| GET | `/api/v1/invoices` | Lista de faturas (?status=&page=) |
| GET | `/api/v1/invoices/{id}` | Detalhes da fatura (apenas próprio) |
...
| Método | Caminho | Descrição |
|------|------|------|
| POST | `/api/v1/tickets` | Submeter ticket (title/content obrigatórios) |
| GET | `/api/v1/tickets` | Lista de tickets (?status=open/closed&page=) |
| GET | `/api/v1/tickets/{id}` | Detalhes do ticket (apenas próprio, outro 404) |
| POST | `/api/v1/tickets/{id}/close` | Encerrar ticket (apenas próprio/apenas open; rating opcional 1-5 de satisfação, fora do intervalo/não inteiro 422, não fornecido compatível com NULL) |

### 12. Interfaces de calendário de agendamentos (requerem autenticação JWT, ronda 20)

| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/calendar/technician/{id}` | Vista mensal (?month=YYYY-MM): time_slots do agendamento expandidos em faixas horárias + exclusão de já agendados |
| GET | `/api/v1/calendar/technician/{id}/day` | Vista diária (?date=YYYY-MM-DD): detalhes das faixas disponíveis/já agendadas/indisponíveis do dia |

### 13. Interfaces de dados de fatura (requerem autenticação JWT, ronda 21)

| Método | Caminho | Descrição |
|------|------|------|
| POST | `/api/v1/invoice-titles` | Guardar título (title_type: personal/company; company requer tax_no; mesmo título do mesmo utilizador repetido 422; o primeiro é automaticamente predefinido) |
| GET | `/api/v1/invoice-titles` | Lista de títulos (predefinido no topo) |
| PUT | `/api/v1/invoice-titles/{id}` | Editar título (apenas próprio) |
| DELETE | `/api/v1/invoice-titles/{id}` | Eliminar título (apenas próprio; ao eliminar o predefinido, o mais antigo é automaticamente designado) |
| POST | `/api/v1/invoice-titles/{id}/default` | Definir predefinido (zera os outros do mesmo utilizador em transação) |

**Ligação ao pedido**: POST /api/v1/invoices suporta title_id opcional — resolve o título e traz automaticamente invoice_title/tax_no/title_type; sem title_id mantém o caminho de preenchimento manual original.

### 14. Interfaces de histórico de navegação (requerem autenticação JWT, ronda 21)

| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/browse-history` | Serviços navegados recentemente (join do nome/capa/preço/preço original do serviço, ordenação decrescente de viewed_at, per_page padrão 15 máximo 50) |
| DELETE | `/api/v1/browse-history/{item_id}` | Eliminar um (apenas próprio, ilegal/de outro 404) |
| DELETE | `/api/v1/browse-history` | Limpar histórico (apenas próprio) |
...
| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/full-reduction-activities` | Lista de atividades de desconto por valor mínimo em vigor (status=1 e dentro do período de validade, ordenação decrescente por valor da redução; interface pública) |
...
| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/order/ics` | Exportar pedidos válidos dos últimos 90 dias (pending/paid/confirmed/serving) em iCal (RFC5545) |
...
| Método | Caminho | Descrição |
|------|------|------|
| POST | `/api/v1/technician/attendance/check-in` | Registo de ponto de entrada (repetido no dia 422, índice único como salvaguarda de concorrência; >10:00 marca atraso) |
| POST | `/api/v1/technician/attendance/check-out` | Registo de ponto de saída (sem entrada/já com saída 422, bloqueio de linha na concorrência) |
| GET | `/api/v1/technician/attendance` | Lista de assiduidade do mês + resumo de dias de presença/total de horas/média de horas (?month=YYYY-MM, inválido 422) |

### 18. Interfaces de privacidade e conformidade (requerem autenticação JWT, ronda 22)

| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/privacy/data` | Exportação de dados (JSON agrupado personal/orders/points/wallet_txns/reviews/addresses/invoices; os registos do servidor apenas guardam telefone mascarado + contagens) |
| POST | `/api/v1/privacy/close-request` | Pedir cancelamento (saldo ≠ 0/pedidos por concluir/tickets em curso 422; define close_status=1 + close_requested_at) |
| POST | `/api/v1/privacy/close-cancel` | Cancelar o pedido de cancelamento (close_status 1→0) |
| POST | `/api/v1/privacy/close-confirm` | Confirmar cancelamento (apenas após 72h; close_status=2 + close_at + phone/nickname anonimizados em user{id} + status=0) |
...
| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/health-profile` | Consultar a minha ficha de saúde (sem ficha devolve objeto vazio) |
| PUT | `/api/v1/health-profile` | Criar/atualizar (upsert, uma por pessoa; allergies/health_notes limite de 500 caracteres, preferred_technician_id valida a existência; atualiza apenas os campos fornecidos, resposta codificada com hashid) |
| DELETE | `/api/v1/health-profile` | Eliminar a minha ficha (apenas próprio) |
...
| Método | Caminho | Descrição |
|------|------|------|
| POST | `/api/v1/wallet/pay-password/set` | Definir palavra-passe de pagamento (6 dígitos `\d{6}`; já definida requer a palavra-passe antiga, interceptada 422) |
| POST | `/api/v1/wallet/pay-password/verify` | Validar palavra-passe de pagamento (correta/errada devolve booleano, não regista) |
| POST | `/api/v1/wallet/pay-password/check` | Consultar se está definida (set: true/false) |
...
| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/order/{id}/timeline` | Linha do tempo de alterações de estado do pedido (ordem decrescente; apenas próprio, pedido de outro 404 sem revelar existência) |
...
| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/wheel/prizes` | Lista de prémios da roleta (campos sensíveis weight/stock ocultos) |
| POST | `/api/v1/wheel/spin` | Sortear uma vez (Redis NX + bloqueio de linha contra concorrência; extração ponderada random_int; pontos→movimentação earn com data de expiração, saldo→contabilização lockForUpdate, cupão→pending para emissão manual, sem prémio→lose; idempotência client_token) |
| GET | `/api/v1/wheel/records` | Os meus registos de sorteios (paginados) |

### 23. Interfaces de modo convidado (ronda 24)

| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/guest/home` | Agregação da página inicial (carrosséis/anúncios/categorias de serviços/serviços populares, cache Redis svc:guest:home 300s) |
| GET | `/api/v1/guest/services` | Lista de serviços (?category_id=hashid&sort=newest\|sales\|price&page/per_page≤50) |
| GET | `/api/v1/guest/services/{id}` | Detalhes do serviço (inexistente 404) |
| GET | `/api/v1/guest/stores` | Lista de lojas |
| GET | `/api/v1/guest/technicians` | Lista de técnicos (apenas aprovados; ?service_id=hashid filtro; classificação decrescente) |

Entradas de navegação sem login, sem autenticação (interfaces públicas).

### 24. Interfaces de promoções relâmpago (requerem autenticação JWT, ronda 24)

| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/seckill` | Lista de atividades de promoção relâmpago (status=1 e dentro da janela temporal; inclui quantidade vendida = n.º de pedidos com appointment_order.seckill_id, stock restante) |
| GET | `/api/v1/seckill/{id}` | Detalhes da atividade (state=not_started/ongoing/ended) |
| POST | `/api/v1/seckill/{id}/buy` | Encomenda relâmpago (idempotência client_token + Redis NX 30s contra concorrência + validação da atividade; sem pré-dedução de stock) |

**Regras de encomenda (desde 2026-08-26)**: o stock é deduzido uniformemente com bloqueio de linha dentro da transação de `/api/v1/order store()`; buy faz apenas a validação de entrada/idempotência; preço relâmpago = seckill_price (tomado do DB como referência), sem sobreposição de cupões/pontos/cartões de membro; cancelamento do pedido não repõe stock; chamar diretamente `/api/v1/order` com seckill_id também deduz o stock.

### 25. Interfaces de verificação de versões do APP (ronda 24)

| Método | Caminho | Descrição |
|------|------|------|
| GET | `/api/v1/app/version?platform=android|ios` | Verificação da versão mais recente (platform inválido 422; sem versão devolve objeto vazio; interface pública) |
...
Cabeçalho: `Authorization: Bearer <admin_token>`; a versão dos endpoints públicos de autenticação é definida pelo prefixo do URL `/api/v1`
...
IDs de permissão: 416-419. A interface de verificação de atualizações /api/v1/app/version toma a versão mais recente (updated_at/id maior) de status=1.
