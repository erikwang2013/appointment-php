...
| Notificações de mensagens | Notificações internas (erik_notification); página do centro de mensagens: paginação/atualização por arrastar/destaque de lidas/marcar como lida/marcar tudo como lido |
...
| Funcionalidade | Descrição |
|------|------|
| Autorização de subscrição | utils/subscribe.js centraliza a gestão dos IDs de modelo (chaves alinhadas com erik_system_config.wechat_app.template_ids no servidor) |
...
| Marcação do pedido | Colunas promotion_id/participant_id novas em erik_order + índices |
...
| Funcionalidade | Descrição |
|------|------|
| Regra de emissão | Após o primeiro pedido completed do recomendado: montante = paid_amount × reward_rate (erik_system_config referral.reward_rate, padrão 0.05, valor ilegal recai para a constante), só emite se >0 |
...
| Idempotência | Bloqueio de linha erik_user_referral lockForUpdate + verificação de vazio de rewarded_at + reavaliação do primeiro pedido dentro do bloqueio (concorrência/chamadas repetidas emitem apenas uma vez) |
...
| Funcionalidade | Descrição |
|------|------|
| Produtos de troca | erik_points_exchange_goods: type=coupon/gift_card/wallet, points_cost/value (DECIMAL(25,2) contra perda de precisão de IDs avalanche)/stock/status |
...
| Idempotência | Índice único uk_user_goods limita uma vez por utilizador e produto + revalidação dentro do bloqueio + salvaguarda 1062; snapshot do registo de troca em erik_user_points_exchange |
...
| Conclusão | Atualiza service_time + regista erik_order_reschedule (com reason) + liberta o bloqueio do período original/novo detido por este pedido; falha reverte a transação e liberta também o bloqueio do novo período |
...
| Funcionalidade | Descrição |
|------|------|
| Validade | Coluna erik_user_points.expires_at; todos os earn (registo diário/consumo/devolução) preenchem expires_at = agora + points.expiry_days (padrão 365, ≤0 nunca expira); consume/use ficam vazios |
...
| Dados | Coluna replied_at adicionada com idempotência a erik_order_review (a coluna reply já existia na criação da tabela); list/show de avaliações no painel de gestão expõem reply/replied_at via decorate()->toArray() |
...
| Dados | erik_order_review acrescenta três colunas append_content TEXT/append_images JSON/append_at DATETIME (migração idempotente); a resposta expõe os campos append |
...
- Coluna de nível de membro erik_user.member_level (migração 000008)
...
- Tabela erik_order_aftersale (migração 000009): type=refund/exchange, status=pending/approved/rejected/completed
...
- Registos: alterações escritas em erik_technician_tier_log (id/technician_id/old_tier_id/new_tier_id/reason/created_at) + notificação interna (type='tier')
...
- Fonte de dados: time_slots JSON de technician_schedule expandidos em faixas horárias por dia da semana, períodos já agendados de erik_order nesse dia excluídos (status ∈ pending/paid/confirmed/serving), saída das faixas disponíveis restantes
...
- erik_user_growth (movimentações) + erik_growth_level (seed de escalões, 5 níveis: Bronze 0/Prata 100/Ouro 500/Platina 2000/Diamante 5000)
...
- erik_invoice: uk_order_type(order_id,order_type) contra pedidos duplicados de fatura no mesmo pedido (pedido duplicado 422, inclui captura de recurso ao MySQL 1062); idx_user_created/idx_status
...
- erik_ticket: o utilizador submete o ticket (title/content), o painel responde por acréscimo (reply_content/replied_at), o utilizador pode encerrar (closed_at)
...
- erik_ticket acrescenta rating TINYINT NULL + rated_at DATETIME NULL (migração 000303)
...
- erik_browse_history (uk_user_item(user_id, item_id) único, navegação repetida apenas atualiza viewed_at sem inserção duplicada; idx_user_viewed para ordenação)
...
- Interface: GET /api/browse-history (join do nome/capa/preço/preço original de erik_service, ordenação decrescente de viewed_at, per_page padrão 15 máximo 50, item_id com hashid); DELETE /{item_id} (apenas próprio, ilegal/de outro 404); DELETE / (limpar apenas próprio)
...
- erik_technician_attendance (date/check_in_at/check_out_at/status + índice único uk_technician_date contra registo duplicado em concorrência)
...
- AppPushService (config group=push: enabled padrão 0 / provider jpush/getui/placeholder): não ativado degrada em silêncio apenas com log; ativado constrói a estrutura plataforma/título/conteúdo/payload, regista Log + escreve erik_push_log (status=sent); integração com SDKs dos fornecedores fica em TODO (sem credenciais não envia de facto)
...
- erik_profit_sharing (uk_sharing_no único + idx_order); admin GET /admin/profit-sharing lista (join do número do pedido/apelido do técnico, filtros por estado/número do pedido/nome do técnico)
- Permissão: 394
...
- erik_user acrescenta close_status/close_requested_at/close_at (migração ALTER idempotente); AuthController login/loginByCode devolve 403 "Conta cancelada" para close_status=2
...
- Migrações 000503 (erik_lucky_wheel + erik_wheel_record + seeds de demonstração w60/w40) + 000505 (seeds de permissões); LuckyWheelTest admin 3 + service 6 testes
...
- erik_seckill_activity (name/service_id/seckill_price/original_price/stock/start_at/end_at/status); quantidade vendida = número de pedidos com erik_order.seckill_id
...
- Regista em erik_technician_earnings (type=return_customer, status=pending) reutilizando a cadeia de liquidação de comissões, o resumo de earnings do lado do técnico inclui automaticamente; idempotência por order_id+type; chamado dentro da transação com bloqueio de linha de WorkController::complete
