...
| Funcionalidade | Descrição |
|------|------|
| Saldo da carteira | GET /api/v1/wallet saldo + movimentos (tabelas user_wallet/wallet_recharge/wallet_txn) |
| Recarga | POST /api/v1/wallet/recharge cria a ordem de recarga; POST /api/v1/wallet/recharge/{id}/pay pagamento WeChat da recarga, callback com número de ordem com prefixo R |
...
| Funcionalidade | Descrição |
|------|------|
| Os meus cartões de vezes | GET /api/v1/marketing/cards/my calcula em tempo real used_up/expired |
| Dedução de vezes na verificação | POST /api/v1/marketing/cards/use: idempotência Redis NX + bloqueio de linha lockForUpdate, cria diretamente pedido completed + OrderItem + OrderPayment(pay_type='card') |
...
| Os meus cartões-presente | GET /api/v1/marketing/gift-cards/my |
...
| Extratos de pontos | GET /api/v1/marketing/points paginado + filtro type/source, type unificado como earn |
...
| Funcionalidade | Descrição |
|------|------|
| Solicitar pós-venda | POST /api/v1/aftersales: type=refund/exchange, valida pedido próprio/paid+completed/desduplicação no mesmo pedido |
| Os meus pós-vendas | GET /api/v1/aftersales lista paginada + GET /api/v1/aftersales/{id} detalhes |
...
| Funcionalidade | Descrição |
|------|------|
| Lista/detalhes de atividades | GET /api/v1/promotions + /api/v1/promotions/{id}, filtro type group_buy/flash_sale |
| Participação | POST /api/v1/promotions/join/{id}: bloqueio Redis NX contra sobre-venda (flash_sale com max_people como limite de stock), participação repetida 422, bloqueio de group_buy quando cheio, encerramento inativo na expiração sem estar cheio (status definido para 0 em show/join) |
...
| Encomenda de grupo | POST /api/v1/order com promotion_id: valida apenas group_buy/atividade válida/chamador é participante/não cheio/serviço corresponde; preço de grupo = preço original × discount_percent/100, cupões/cartões de vezes/pontos desativados (422) |
...
| Extratos | GET /api/v1/user/referral/earnings paginado (apelido/avatar do recomendado/número do pedido/montante/hora) |
...
| Lista de produtos | GET /api/v1/marketing/points-exchange: produtos em prateleira + stock restante em tempo real + quantidade trocada |
| Troca | POST /api/v1/marketing/points-exchange/{id}: bloqueio Redis NX + bloqueio de linha do produto contra troca excessiva; validação SUM dos pontos (insuficientes 422) + dedução UserPoints type='consume' source='exchange'; coupon emite cupão / wallet credita saldo (WalletTxn points_exchange) / gift_card devolve código |
...
| Funcionalidade | Descrição |
|------|------|
| Interface | POST /api/v1/order/reschedule/{id}: new_service_time (obrigatório) + reason (opcional), altera o horário com o mesmo técnico |
...
| Funcionalidade | Descrição |
|------|------|
| Interface | POST /api/v1/marketing/coupons/transfer (user_coupon_id) gera código de oferta único de 8 caracteres sem caracteres ambíguos (uk_code como salvaguarda, válido 7 dias); POST /api/v1/marketing/coupons/claim (code) para receber; GET /api/v1/marketing/coupons/transfers emitidos (pending/claimed/expired) + recebidos (claimed) paginados |
...
> Substituído pelo canal `/api/v1/seckill` da ronda 24 (o ramo de promoção de store() tem apenas compras em grupo), ver "43. Promoções relâmpago".

| Funcionalidade | Descrição |
|------|------|
| Interface | POST /api/v1/order com promotion_id (tipo flash_sale): preço relâmpago = round(total × (100 − discount_percent)/100, 2), alinhado com o critério de preço relâmpago do PromotionController |
...
| Funcionalidade | Descrição |
|------|------|
| Interface | POST /api/v1/technician/review/reply/{order_id} (middleware de identidade de técnico): avaliação inexistente/não própria unificados 404; resposta já existente 422 (rejeição idempotente sem sobrepor); resposta vazia 422 |
...
| Funcionalidade | Descrição |
|------|------|
| Interface | POST /api/v1/wallet/transfer: desencriptação do hashid do recetor + existência 404, para si próprio 422, montante 0.01-1000/operação 422 (comparação DECIMAL sem float), saldo insuficiente 422, limite diário acumulado de 5000 RMB 422 |
...
| Registos | GET /api/v1/wallet/transfers (direction=out/in paginado) + GET /transfers/{id} (visível apenas para ambas as partes, senão 404) |

### 32. Transferência de pontos (ronda 19)

| Funcionalidade | Descrição |
|------|------|
| Interface | POST /api/v1/user/points/transfer: recetor inexistente 404, para si próprio 422, pontos 1-10000 422, saldo agregado SUM insuficiente 422, limite diário acumulado de 10000 422 |
...
| Funcionalidade | Descrição |
|------|------|
| Interface | GET /api/v1/order/logistics/{id}: apenas pedidos de produtos próprios podem consultar (não próprio/não produto/não expedido unificados 404) |
...
| Interface | GET /api/v1/user/notify-settings devolve os 5 interruptores completos; PUT upsert em lote sem linhas duplicadas |
...
- service /api/v1/store-manager: overview (pedidos de hoje/rendimentos/em curso/número de técnicos/número de verificações) + orders (paginação + filtro de estado) + technicians (com agendamentos de hoje) + revenue (agregação dos últimos 7 dias), requireStoreId() força o isolamento store_id (403 sem loja)
...
- CalendarController vista mensal/diária: GET /api/v1/calendar/technician/{id} (vista mensal) + /day (vista diária)
...
- Interface: GET /api/v1/growth (visão geral do nível atual: balance/level/diferença para o próximo escalão); GET /api/v1/growth/records (movimentações paginadas); GET /api/v1/growth/levels (lista pública de escalões, sem login)
...
- Lado do utilizador: POST /api/v1/invoices (pedido, montante/título trazidos pelo servidor do pedido, não alteráveis); GET /api/v1/invoices (lista); GET /api/v1/invoices/{id} (detalhes)
...
- Lado do utilizador: POST /api/v1/tickets (submissão); GET /api/v1/tickets (lista); GET /api/v1/tickets/{id} (detalhes, apenas próprio); POST /api/v1/tickets/{id}/close (encerramento)
...
- Reutilização de consultas: GrowthLevel::levelForGrowth() fixa o escalão pelo crescimento acumulado, reutilizado no pedido/pagamento; GET /api/v1/growth já devolve benefits e next_gap (implementação do R20, sem alterações)
...
- Interface: POST /api/v1/invoice-titles (guardar, company requer tax_no, duplicado 422); GET (lista, predefinido no topo); PUT /{id} (edição, apenas próprio); DELETE /{id} (eliminação, apenas próprio); POST /{id}/default (definir predefinido, zera os outros do mesmo utilizador em transação)
...
- Interface: GET /api/v1/browse-history (join do nome/capa/preço/preço original de appointment_service, ordenação decrescente de viewed_at, per_page padrão 15 máximo 50, item_id com hashid); DELETE /{item_id} (apenas próprio, ilegal/de outro 404); DELETE / (limpar apenas próprio)
...
- Lado do utilizador GET /api/v1/full-reduction-activities (público, ativas por ordem decrescente de redução)
...
- IcsController GET /api/v1/order/ics: pedidos pending/paid/confirmed/serving dos últimos 90 dias exportados em iCal (RFC5545), apenas próprio
...
- GET /api/v1/privacy/data: exportação de dados (agrupados personal/orders/points/wallet_txns/reviews/addresses/invoices; os registos apenas guardam telefone mascarado + contagens)
...
- GET/PUT/DELETE /api/v1/health-profile: uma por pessoa (índice único uk_user), upsert apenas atualiza os campos fornecidos
...
- POST /api/v1/wallet/pay-password/{set,verify,check}: validação de 6 dígitos, armazenamento de password_hash + pay_password_set_at
...
- POST /api/v1/technician/schedule/batch: período de datas ≤7 dias + filtro weekdays, dias com agendamento existente são ignorados
...
- GET /api/v1/order/{id}/timeline: apenas próprio pode consultar (outro 404), devolução por ordem decrescente; os detalhes do pedido no admin incorporam o array timeline
...
- GET /api/v1/wheel/prizes (weight/stock ocultos); POST /api/v1/wheel/spin: Redis NX + bloqueio de linha contra concorrência, extração ponderada random_int, idempotência client_token
...
- GET /api/v1/wheel/records os meus registos paginados; admin /admin/lucky-wheel CRUD + prateleira + registos (permissões 401-406)
...
- GET /api/v1/guest/{home,services,services/{id},stores,technicians}: entradas de navegação sem login, sem autenticação (interfaces públicas)
...
- GET /api/v1/seckill (status=1 + janela temporal), /{id} (state=not_started/ongoing/ended), POST /{id}/buy: idempotência client_token (8-64 caracteres, SETNX 24h) + Redis NX 30s contra concorrência + validação da atividade (desde 2026-08-26 sem pré-dedução de stock)
- O pedido injeta seckill_id reutilizando OrderController::store; o stock é deduzido uniformemente com bloqueio de linha dentro da transação do store() (chamar diretamente /api/v1/order com seckill_id também deduz o stock), preço relâmpago = seckill_price (tomado do DB como referência), sem sobreposição de cupões/pontos/cartões de membro; cancelamento do pedido não repõe stock; o antigo canal de promoção FLASH_SALE foi removido (o ramo de promoção de store() tem apenas compras em grupo, PromotionController index filtra flash_sale, show/join 400), as promoções relâmpago seguem apenas este canal
...
- GET /api/v1/app/version?platform=android|ios verificação pública de atualizações (platform inválido 422; toma o mais recente de status=1; sem nenhum devolve objeto vazio)
