...
**Комиссионные**: выплачиваются после завершения (completed) первого заказа приглашённого, сумма = paid_amount × reward_rate (erik_system_config referral.reward_rate, по умолчанию 0.05, при недопустимом значении — откат к константе). Тройная идемпотентность: блокировка строк + проверка rewarded_at + повторная проверка первого заказа; зачисление в WalletTxn type=referral_reward.
...
**Переключатели уведомлений**: таблица erik_user_notify_setting (составной уникальный ключ user_id+type, отсутствующая строка = по умолчанию включено). 5 типов: service_reminder напоминание об услуге / card_expiry напоминание об истечении (единый зонт для карт и купонов) / points_expiry истечение баллов / marketing маркетинг (зарезервировано) / system системные (нельзя отключить, PUT принудительно ставит 1). Гейтинг: notifySettingEnabled подключается к 3 процессам-таймерам ServiceReminderTimer/ExpiryReminderTimer/PointsExpiryTimer + маппинг сценариев событий подписки (PAY/REFUND/VERIFIED/RESCHEDULE→system шлётся всегда, REMINDER→service_reminder, EXPIRY→card_expiry); при выключенном типе пропускаются и внутрисистемные уведомления, и сообщения подписки.
...
**Защита цен от подмены (2026-08-26)**: суммы позиций заказа всегда берутся из записей БД (target_type=service запрос в erik_service, product в erik_product), клиентские цены в расчётах не участвуют; неизвестный target_type 422; target_id обязан быть закодированным hashid (передача raw id декодируется в 0 → 422 «商品不存在或已下架»); цены групповых/секундных покупок также по данным БД.
...
**Изоляция по store_id**: requireStoreId() принудительно требует привязку текущего пользователя к филиалу (erik_user.store_id), без филиала 403; все запросы фильтруются по store_id.
...
| Метод | Путь | Описание |
|------|------|------|
| GET | `/api/seckill` | Список активностей распродажи (status=1 и во временном окне; включает проданное = число заказов с erik_order.seckill_id, остаток) |
...
**Автоматическое оценивание**: TierRatingService::evaluate считает в реальном времени (число заказов erik_order со статусом completed + средняя оценка отзывов, округление до 1 знака) и записывает обратно в profile.order_count/rating, сопоставление по erik_technician_tier_config (min_orders/min_rating) от высокого к низкому, без совпадения — самый низкий уровень. Только повышение без понижения (понижение влияет на ставку комиссии и ценовой коэффициент, за ним следит админка вручную; allowDowngrade=true для ручного пересмотра); идемпотентно (при совпадении уровня только синхронизируется статистика); изменение фиксируется в erik_technician_tier_log + внутрисистемное уведомление. Точки запуска: WorkController::complete / запись отзыва ReviewController / ленивая проверка при просмотре профиля ProfileController.
...
ID прав: 412–414. Правила награды: при второй покупке пользователя у одного мастера за 30 дней (заказ завершён) выплачивается бонус = оплаченное × ratio (по умолчанию 0.05), запись в erik_technician_earnings (type=return_customer, status=pending), расчёт вместе с цепочкой комиссионных; по одному заказу идемпотентно — повторно не выплачивается.
...
ID прав: 407–411, 420. Проданное = число заказов с erik_order.seckill_id; запас списывается блокировкой строк, при распродаже — блокировка покупки.
