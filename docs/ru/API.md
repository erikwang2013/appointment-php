...
### 26. Неверсионированные интерфейсы (без префикса `/api/v1`)

| Метод | Путь | Описание |
|------|------|------|
| GET | `/` | Брендовая посадочная страница (HTML; отображается при прямом заходе браузера на порт API, содержит ссылки на документацию API и проверку работоспособности) |
| GET | `/health` | Проверка работоспособности (JSON, для docker-compose healthcheck / проверки живости балансировщиком) |
| GET | `/api/docs` | Полная спецификация бизнес-API (OpenAPI 3.0 JSON) |
| POST | `/payment/wechat-notify` | Колбэк WeChat Pay (проверка подписи + идемпотентность) |

> Клиентские бизнес-интерфейсы всегда идут с префиксом версии `/api/v1/...`; «голые» пути `/api/*` без префикса возвращают 404.
