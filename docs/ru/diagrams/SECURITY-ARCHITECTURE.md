...
flowchart LR
    ...
    subgraph DB存储["Хранение на уровне БД"]
        D1["appointment_user.phone<br/>открытое хранение<br/>вход/поиск дублей зависят от открытых запросов"]
        D2["appointment_technician_profile<br/>.id_card VARCHAR(500)<br/>шифрование encryptable"]
        D3["appointment_user.wx_openid<br/>открытое хранение"]
        D4["appointment_user.real_name<br/>шифрование encryptable"]
    end
