# План реализации системы сервиса предварительной записи
> **Languages**: [中文](../../superpowers/plans/2026-05-26-appointment-system-plan.md) · [English](../../en/plans/2026-05-26-appointment-system-plan.md) · [한국어](../../ko/plans/2026-05-26-appointment-system-plan.md) · [Deutsch](../../de/plans/2026-05-26-appointment-system-plan.md) · [Français](../../fr/plans/2026-05-26-appointment-system-plan.md) · [Español](../../es/plans/2026-05-26-appointment-system-plan.md) · [Português](../../pt/plans/2026-05-26-appointment-system-plan.md) · [हिन्दी](../../hi/plans/2026-05-26-appointment-system-plan.md) · [العربية](../../ar/plans/2026-05-26-appointment-system-plan.md) · [বাংলা](../../bn/plans/2026-05-26-appointment-system-plan.md) · [Bahasa Indonesia](../../id/plans/2026-05-26-appointment-system-plan.md) · [日本語](../../ja/plans/2026-05-26-appointment-system-plan.md)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task.

**Цель:** построить трёхстороннюю систему сервиса предварительной записи: мини-программа WeChat + Flutter APP (переключение роли на одном аккаунте), расширение PC админки.

**Архитектура:** `admin/` (API админки) + `service/` (бизнес-API) — два сервиса разделяют MySQL/Redis/ES. Мини-программа и APP функционально эквивалентны, единый аккаунт поддерживает переключение роли клиента/мастера.

**Технологический стек:** PHP 8.3 + webman v2, MySQL 8.0, Redis, Elasticsearch, нативная мини-программа WeChat, Flutter 3.x (GetX + Dio)

**Технические регламенты:**
- Первичный ключ: `erikwang2013/snowflake-php` BIGINT без автоинкремента
- ID API: шифрование/расшифровка `erikwang2013/hashids`
- JWT: `erikwang2013/jwt-webman`
- Флаги стран: `erikwang2013/season`
- Чувствительные данные API: `erikwang2013/encryption`
- Чувствительные поля БД: `erikwang2013/encryptable`
- Поиск ES: `erikwang2013/webman-scout`
- Обнаружение атак: `erikwang2013/security-php`
- Верификация чувствительных операций: `erikwang2013/poster-php`
- Авторские права: `Copyright (c) 2026 erik <erik@erik.xyz> — https://erik.xyz`
- Глобальные функции без префикса `\`, импорт через `use`
- Файлы конфигурации содержат комментарии на китайском
- Экспорт Excel + визуализация панели + экспорт PDF панели
- Источник операций: web, iPadOS, macOS, Windows, Linux, ios, android, harmonyOS

**Дизайн-спецификация:** `docs/superpowers/specs/2026-05-26-appointment-system-design.md`  
**Структура проекта:** `docs/STRUCTURE.md`  
**Миграция БД:** `admin/database/migrations/2026_05_26_000003_appointment_business_tables.sql`

---

## Phase 0: Foundation — скелет проекта service/

### Task 0.1: Инициализация конфигурации проекта service/
...
- [ ] **Шаг 3: создать config/database.php** — один общий MySQL с admin
...
- [ ] **Шаг 4: создать config/route.php**
...
- [ ] **Шаг 5: создать config/middleware.php**
...
- [ ] **Шаг 6: скопировать общие конфигурационные файлы из admin/config/**

Скопировать `jwt.php`, `hashids.php`, `snowflake.php`, `encryption.php`, `encryptable.php`, `scout.php`, `season.php`, `poster.php` в `service/config/`, добавить объявление авторских прав и комментарии на китайском.

- [ ] **Шаг 7: создать app/functions.php**
...
- [ ] **Шаг 8: создать app/middleware/Cors.php**
...
- [ ] **Шаг 9: создать промежуточное ПО Security**
...
- [ ] **Шаг 10: создать промежуточное ПО Auth**
...
- [ ] **Шаг 11: создать промежуточное ПО TechnicianAuth**
...
- [ ] **Шаг 12: создать BaseController** — единый формат ответа + шифрование/расшифровка hashids
...
## Phase 1: Аутентификация пользователя и управление идентичностью

### Task 1.1: создать модель User
...
### Task 1.2: создать AuthController

- Create: `service/app/api/AuthController.php`
- Реализация: регистрация по телефону (включая реферальный код), вход по паролю, вход по коду, восстановление пароля, переключение роли, вход через WeChat (зарезервирован процесс привязки телефона)
- Гостевой режим: управляется фронтендом, просмотр без входа, при оформлении заказа проверяется JWT

### Task 1.3: создать контроллеры модуля user
...
## Phase 2: Услуги и филиалы

- Create: `service/app/model/ServiceCategory.php`
- Create: `service/app/model/Service.php` (включая конфигурацию индекса ES webman-scout)
...
- Create: `service/app/service/SearchController.php` (поиск ES)
- Create: `service/app/service/StoreController.php`

---

## Phase 3: Заказы и оплата
...
- Create: `service/app/order/OrderController.php` — при оформлении Redis SETNX блокирует мастера на 3 минуты
- Create: `service/app/order/PaymentController.php` (оплата WeChat зарезервирована)
- Create: `service/app/order/VerificationController.php` (списание по QR-коду)
- Create: `service/app/order/ReviewController.php`
- Правила возврата: четыре уровня 100%/90%/80%/0%

---

## Phase 4: Модуль мастера

- Create: `service/app/model/TechnicianProfile.php` + Schedule/Service/Earnings/Withdrawal/Attendance/MemberNote
- Create: `service/app/technician/ProfileController.php` (заявка на вступление + проверка)
...
- Create: `service/app/technician/WithdrawalController.php` (20-е число каждого месяца/T+1)
- Create: `service/app/technician/AttendanceController.php`

---

## Phase 5: Маркетинговый модуль

- Create: `service/app/model/Coupon.php` + UserCoupon/MemberCard/UserMemberCard/MemberCardUsage/Points/GiftCard/Referral
- Create: `service/app/marketing/CouponController.php` (автоматическая выдача купона новым пользователям при регистрации)
- Create: `service/app/marketing/MemberCardController.php` (месячная карта/VIP годовая карта/карта по количеству услуг)
- Create: `service/app/marketing/PointsController.php` (обмен 1:100 на подарочную карту)
- Create: `service/app/marketing/GiftCardController.php`

---

## Phase 6: Контент и уведомления
...
## Phase 7: Расширение админки Admin

- Все контроллеры админки расширяют существующий `admin/app/admin/controller/BaseController.php`
- Чувствительные операции (удаление/проверка/вывод) используют `erikwang2013/poster-php` со случайной верификацией
- Dashboard: карточки статистики в реальном времени + линейные графики Chart.js + быстрая навигация + внутренние сообщения
- Экспорт Excel (пользователи/мастера/заказы/финансы), чувствительные поля автоматически маскируются
- Экспорт PDF панели
- Определение источника операции: промежуточное ПО `OperationLog` уже поддерживает, расширяется до 8 источников
...
- Расширить `admin/app/admin/controller/DashboardController.php` бизнес-статистикой
- Расширить `admin/app/admin/controller/ExportController.php` экспортом бизнес-данных
- Расширить сид-данные прав `admin/database/migrations/` узлами бизнес-прав

---

## Phase 8: Фронтенд

- `apps/wechat/` — разработка страниц мини-программы WeChat (см. структуру страниц в STRUCTURE.md)
- `apps/flutter/` — разработка страниц Flutter APP (структура совпадает с мини-программой)
- `admin/apps/flutter/` — новые бизнес-страницы Flutter Web админки

---

## Справочник технических регламентов

| Регламент | Способ реализации |
|------|----------|
| Шапка авторских прав | Верх всех .php-файлов: `Copyright (c) 2026 erik <erik@erik.xyz> — https://erik.xyz` |
| Глобальные функции | Импорт через `use`, без префикса `\` |
| Шифрование ID | BaseController::encodeIds/decodeIds автоматически кодирует/декодирует hashids |
| Чувствительные поля БД | Трейт encryptable объявляет массив `$encryptable` |
| Чувствительные данные API | Пакет encryption шифрует/расшифровывает на этапе сериализации |
| Поиск ES | Конфигурация webman-scout модели `searchableAs()` + SearchController |
| Обнаружение атак | Глобальное промежуточное ПО security-php, обнаружение 31 типа атак |
| Верификация чувствительных операций | poster-php показывает проверку перед удалением/проверкой/выводом |
| Экспорт Excel | ExportController использует PhpSpreadsheet, чувствительные поля маскируются |
| Экспорт PDF панели | Статистика Dashboard рендерится в PDF |
| Определение источника | Промежуточное ПО OperationLog User-Agent → строка из 8 источников |
