# 预约服务系统 实现计划
> **多语言**：[English](../../en/plans/2026-05-26-appointment-system-plan.md) · [한국어](../../ko/plans/2026-05-26-appointment-system-plan.md) · [Русский](../../ru/plans/2026-05-26-appointment-system-plan.md) · [Deutsch](../../de/plans/2026-05-26-appointment-system-plan.md) · [Français](../../fr/plans/2026-05-26-appointment-system-plan.md) · [Español](../../es/plans/2026-05-26-appointment-system-plan.md) · [Português](../../pt/plans/2026-05-26-appointment-system-plan.md) · [हिन्दी](../../hi/plans/2026-05-26-appointment-system-plan.md) · [العربية](../../ar/plans/2026-05-26-appointment-system-plan.md) · [বাংলা](../../bn/plans/2026-05-26-appointment-system-plan.md) · [Bahasa Indonesia](../../id/plans/2026-05-26-appointment-system-plan.md) · [日本語](../../ja/plans/2026-05-26-appointment-system-plan.md)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task.

**Goal:** 构建三端预约服务系统：用户端微信小程序 + Flutter APP（同账号身份切换），PC管理后台扩展。

**Architecture:** `admin/`（管理后台API）+ `service/`（业务API）双服务共享 MySQL/Redis/ES。用户端小程序与APP功能等价，统一账号支持客户/技师身份切换。

**Tech Stack:** PHP 8.3 + webman v2, MySQL 8.0, Redis, Elasticsearch, 微信小程序原生, Flutter 3.x (GetX + Dio)

**技术规范:**
- 主键: `erikwang2013/snowflake-php` BIGINT 非自增
- API ID: `erikwang2013/hashids` 加解密
- JWT: `erikwang2013/jwt-webman`
- 国家旗帜: `erikwang2013/season`
- API 敏感数据: `erikwang2013/encryption`
- DB 敏感字段: `erikwang2013/encryptable`
- ES 搜索: `erikwang2013/webman-scout`
- 安全检测: `erikwang2013/security-php`
- 敏感操作验证: `erikwang2013/poster-php`
- 版权: `Copyright (c) 2026 erik <erik@erik.xyz> — https://erik.xyz`
- 全局函数不加 `\`，使用 `use` 导入
- 配置文件含中文注释
- Excel 导出 + 面板可视化 + PDF 面板导出
- 操作来源端: web, iPadOS, macOS, Windows, Linux, ios, android, harmonyOS

**设计规范:** `docs/superpowers/specs/2026-05-26-appointment-system-design.md`  
**项目结构:** `docs/STRUCTURE.md`  
**数据库迁移:** `admin/database/migrations/2026_05_26_000003_appointment_business_tables.sql`

---

## Phase 0: Foundation — service/ 项目骨架

### Task 0.1: 初始化 service/ 项目配置
...
- [ ] **Step 3: 创建 config/database.php** — 与 admin 共享同一 MySQL
...
- [ ] **Step 5: 创建 config/middleware.php**
...
- [ ] **Step 6: 从 admin/config/ 复制共享配置文件**

复制 `jwt.php`, `hashids.php`, `snowflake.php`, `encryption.php`, `encryptable.php`, `scout.php`, `season.php`, `poster.php` 到 `service/config/`，添加版权声明和中文注释。

- [ ] **Step 7: 创建 app/functions.php**
...
- [ ] **Step 8: 创建 app/middleware/Cors.php**
...
- [ ] **Step 12: 创建 BaseController** — 统一响应 + hashids 加解密
...
### Task 1.2: 创建 AuthController

- Create: `service/app/api/AuthController.php`
- 实现: 手机号注册（含推荐码）、密码登录、验证码登录、忘记密码、身份切换、微信登录（预留绑定手机号流程）
- 游客模式: 前端控制，未登录可浏览，下单时检查JWT

### Task 1.3: 创建 user 模块控制器
...
- Create: `service/app/model/Service.php` (含 webman-scout ES 索引配置)
...
- Create: `service/app/service/SearchController.php` (ES 搜索)
...
- Create: `service/app/order/OrderController.php` — 下单时 Redis SETNX 锁定技师3分钟
- Create: `service/app/order/PaymentController.php` (微信支付预留)
- Create: `service/app/order/VerificationController.php` (二维码核销)
- Create: `service/app/order/ReviewController.php`
- 退款规则: 100%/90%/80%/0% 四级比例
...
- Create: `service/app/technician/ProfileController.php` (入驻申请 + 审核)
...
- Create: `service/app/technician/WithdrawalController.php` (每月20日/T+1)
...
- Create: `service/app/marketing/CouponController.php` (新用户注册自动发放优惠券)
- Create: `service/app/marketing/MemberCardController.php` (月卡/VIP年卡/次卡)
- Create: `service/app/marketing/PointsController.php` (1:100兑换礼品卡)
...
## Phase 7: Admin 管理后台扩展

- 所有管理端控制器基于现有 `admin/app/admin/controller/BaseController.php` 扩展
- 敏感操作（删除/审核/提现）使用 `erikwang2013/poster-php` 增加随机验证
- Dashboard: 实时统计卡片 + Chart.js折线图 + 快速导航 + 站内消息
- Excel 导出（用户/技师/订单/财务），敏感字段自动脱敏
- PDF 面板导出
- 操作来源端检测: `OperationLog` 中间件已支持，扩展为8端
...
- 扩展 `admin/app/admin/controller/DashboardController.php` 增加业务统计
- 扩展 `admin/app/admin/controller/ExportController.php` 增加业务数据导出
- 扩展权限种子数据 `admin/database/migrations/` 增加业务权限节点

---

## Phase 8: 前端

- `apps/wechat/` — 微信小程序页面开发（参照 STRUCTURE.md 页面结构）
- `apps/flutter/` — Flutter APP 页面开发（与小程序结构一致）
- `admin/apps/flutter/` — 管理后台 Flutter Web 新增业务页面
...
| 规范 | 实现方式 |
|------|----------|
| 版权头 | 所有 .php 文件头部: `Copyright (c) 2026 erik <erik@erik.xyz> — https://erik.xyz` |
| 全局函数 | `use` 导入，不加前置 `\` |
| ID 加解密 | BaseController::encodeIds/decodeIds 自动 hashids 编码解码 |
| DB 敏感字段 | encryptable trait 声明 `$encryptable` 数组 |
| API 敏感数据 | encryption 包在序列化阶段加解密 |
| ES 搜索 | webman-scout 模型配置 `searchableAs()` + SearchController |
| 安全检测 | security-php 全局中间件，31种攻击检测 |
| 敏感操作验证 | poster-php 在删除/审核/提现操作前弹出验证 |
| Excel 导出 | ExportController 使用 PhpSpreadsheet，敏感字段脱敏 |
| PDF 面板导出 | Dashboard 统计渲染为 PDF |
| 来源端检测 | OperationLog 中间件 User-Agent → 8端字符串 |
